...
export function generateMockAnalyticsData(): GetChartsResponse {
	...
	const interactionTrendData: InteractionTrendDataPoint[] = timestamps.map(
		(timestamp) => ({
			timestamp,
	...
	const workflowRunsTrendData: WorkflowRunsTrendDataPoint[] = timestamps.map(
		(timestamp) => ({
			timestamp,
	...
	const activeSubscribersTrendData: ActiveSubscribersTrendDataPoint[] =
