...
const generatePromptSections = (config: WorkflowTriggerPromptConfig) => {
	...
	const languageImplementations: Record<
		PromptLanguage,
					...
					? [
							{
								...
								objective:
									"Collect all required payload fields for the workflow",
 ...
 */
export function generateWorkflowTriggerAIPrompt(
	config: WorkflowTriggerPromptConfig,
): string {
	...
	parts.push(
		formatSection(sections.workflowInfo.title, sections.workflowInfo.content),
	);
	parts.push("\n---\n");

	// Objectives
	parts.push(
		formatSection(sections.objectives.title, sections.objectives.content),
	);
	parts.push("\n---\n");
	...
	parts.push(
		formatSection(
			sections.finalDeliverables.title,
			sections.finalDeliverables.content,
		),
	);
	parts.push("\n---\n");
