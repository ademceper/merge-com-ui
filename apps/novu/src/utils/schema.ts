...
const handleStringPattern = ({
	...
	return value
		...
		.refine(
			(val) =>
				!val || z.string().regex(new RegExp(pattern)).safeParse(val).success,
			{
	...
	if (type === "object") {
		...
		zodValue = zodValue.transform((val) => {
			...
			const hasAnyRequiredEmpty = required?.some(
				(field) =>
					valAsRecord[field] === "" || valAsRecord[field] === undefined,
			);
		...
		const anyOf = jsonSchema.anyOf.map((oneOfObj) =>
			buildDynamicZodSchema(oneOfObj, key),
		);
...
export const buildDynamicZodSchema = (
	...
	if (typeof obj === "object" && obj.type === "object") {
		...
		const keys: Record<string, z.ZodType> = Object.keys(properties).reduce(
			(acc, key) => {
...
export const buildDefaultValues = (
	...
	const properties =
		typeof uiSchema === "object" ? (uiSchema.properties ?? {}) : {};

	const keys: Record<string, unknown> = Object.keys(properties).reduce(
		(acc, key) => {
			...
			if (typeof defaultValue === "object") {
				return {
					...acc,
					[key]: buildDefaultValues({ properties: { ...defaultValue } }),
				};
			}
	...
	for (const [key, propDef] of Object.entries(properties)) {
		...
		if (prop.type === "array" && prop.items) {
			...
			if (Array.isArray(prop.items)) {
				...
				(prop.items as JSONSchemaDto).type === "object"
			) {
...
export const buildDefaultValuesOfDataSchema = (
