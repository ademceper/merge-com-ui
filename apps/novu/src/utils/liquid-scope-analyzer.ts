 ...
 * - assign statements
 */
export function isVariableInLocalContext(
	content: string,
	name: string,
	position: number,
): boolean {
	...
	return (
		availableVariables.includes(rootName) || availableVariables.includes(name)
	);
}
...
export function getVariablesAtPositionWithLoopProperties(
	content: string,
	position: number,
): string[] {
	const availableVariables = getVariablesAtPosition(content, position);
	return availableVariables.filter(
		(v) => !v.startsWith("forloop") && !v.startsWith("tablerowloop"),
	);
}
...
function isInsideBlockScope(content: string, position: number): boolean {
	const blockStarts = [
		LIQUID_PATTERNS.for,
		LIQUID_PATTERNS.tablerow,
		LIQUID_PATTERNS.capture,
	];

	const blockEnds = [
		LIQUID_PATTERNS.endFor,
		LIQUID_PATTERNS.endTablerow,
		LIQUID_PATTERNS.endCapture,
	];
		...
		if (endMatch) {
			...
			if (!isInsideBlockScope(content, start)) {
				...
				for (const scope of scopes) {
					if (
						start >= scope.start &&
						captureEnd <= scope.end &&
						(scope.type === "for" || scope.type === "tablerow")
					) {
