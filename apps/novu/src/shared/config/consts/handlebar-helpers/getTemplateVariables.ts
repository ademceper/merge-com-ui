...
function getTemplateVariables(bod): IMustacheVariable[] {
	...
	const stringVariables: IMustacheVariable[] = bod
		...
		.flatMap((body) => {
			const varName =
				body.params[0]?.original || (body.path.original as string);
			...
			if (body.params?.[0]?.original) {
				if (
					!(Object.values(HandlebarHelpersEnum) as string[]).includes(
						body.path.original,
					)
				) {
					return [];
	...
	const arrayVariables: IMustacheVariable[] = bod
		...
		.flatMap((body) => {
			...
			const nestedVariablesInBlock = getTemplateVariables(
				body.program.body,
			).map((mustVar) => {
				return {
					...mustVar,
