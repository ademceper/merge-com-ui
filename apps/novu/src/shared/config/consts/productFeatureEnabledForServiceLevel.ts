...
} from "./feature-tiers-constants";

const featureAccessAtoFeatureNameMapping: Record<
...
function createProductFeatureMap(): Record<
	ProductFeatureKeyEnum,
	ApiServiceLevelEnum[]
> {
	...
	for (const apiServiceLevel of Object.values(ApiServiceLevelEnum)) {
		for (const [productFeatureKey, featureName] of Object.entries(
			featureAccessAtoFeatureNameMapping,
		)) {
			const typedProductKey =
				productFeatureKey as unknown as ProductFeatureKeyEnum;

			if (Object.values(ProductFeatureKeyEnum).includes(typedProductKey)) {
				const isFeatureEnabled = getFeatureForTierAsBoolean(
					featureName,
					apiServiceLevel,
				);
