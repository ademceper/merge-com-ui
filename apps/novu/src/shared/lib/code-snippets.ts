...
export const createNodeJsSnippet = ({
	identifier,
	to,
	payload,
	secretKey,
	...
	const renderedSecretKey = secretKey
		? `'${secretKey}'`
		: `process.env.${SECRET_KEY_ENV_KEY}`;
	let serverConfig = "";
...
export const createTriggerRequestBody = ({
	...
	return {
		name: workflowId,
		to,
		payload: addDashboardSource
			? { ...parsedPayload, __source: "dashboard" }
			: parsedPayload,
		context,
	};
};
...
export const createFrameworkSnippet = ({
	identifier,
	to,
	payload,
}: CodeSnippet) => {
...
export const createPhpSnippet = ({
	...
	const subscriberId =
		typeof to === "string"
			? to
			: (to as Record<string, unknown>).subscriberId || "subscriber-id";
	...
	const entries = Object.entries(data)
		.map(([key, value]) => {
			...
			if (
				...
				formattedValue = convertJsonToGoMap(
