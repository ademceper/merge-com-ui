...
export function useFormAutosave<
	U extends Record<string, unknown>,
	T extends FieldValues = FieldValues,
>({ form: propsForm, ...saveProps }: UseFormAutosaveProps<U, T>) {
	...
	const onSave = useCallback(
		async (
			data: T,
			options?: { forceSubmit?: boolean; onSuccess?: () => void },
		) => {
			const { save, isReadOnly, shouldClientValidate, previousData } =
				savePropsRef.current;
			if (isReadOnly) {
				return;
			}
			...
			const isDirty =
				form.formState.isDirty || Object.keys(dirtyFields).length > 0;
