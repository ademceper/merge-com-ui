...
function handleNonNamespacedInput(
	...
	if (
		...
		return [
			createJitVariable(
				`${namespace}.${trimmedSearch}.id`,
				isPayloadSchemaEnabled,
				onCreateNewVariable,
				namespace,
			),
			createJitVariable(
				`${namespace}.${trimmedSearch}.data`,
				isPayloadSchemaEnabled,
				onCreateNewVariable,
				namespace,
			),
	...
	if (
		namespace === CONTEXT_NAMESPACE &&
		!isValidContextVariable(suggestedVariableName)
	) {
		return [];
	}
					...
					matchingVariables.length > 0
						? matchingVariables.map((v) =>
								createCompletionOption(
									v.name,
									v.isNewSuggestion && isPayloadSchemaEnabled
										? "new-variable"
										: (v.type ?? "variable"),
									v.boost,
									v.info,
...
function getMatchingVariables(
	...
	const stepPayloadNamespaces = variables.reduce<string[]>(
		(acc, variableItem) => {
	...
	const namespaces = isContextEnabled
		? [...baseNamespaces, CONTEXT_NAMESPACE]
		: baseNamespaces;
	...
	const baseVariables = Array.from(
		new Map(combinedVariables.map((item) => [item.name, item])).values(),
	);
...
export function createAutocompleteSource(
	...
	return (context: CompletionContext) => {
		...
		const scopedVariables = getVariablesAtPositionWithLoopProperties(
			context.state.doc.toString(),
			context.pos,
		);
		const scopedLiquidVariables = scopedVariables.map<LiquidVariable>(
			(variable) => ({
				name: variable,
		...
		if (isTranslationEnabled) {
			options.options = [
				createTranslationNamespaceCompletion(),
				...options.options,
			] as Completion[];
		}
					...
					({
						...
						apply: (
							...
							if (completion.type === "translation") {
								return applyTranslationNamespaceCompletion(
									view,
									completion,
									from,
									to,
