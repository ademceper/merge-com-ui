...
export const SchemaPropertyRow = memo<SchemaPropertyRowProps>(
	function SchemaPropertyRow(props) {
		const {
			control,
			pathPrefix,
			onDeleteProperty,
		...
		const propertyListItem = useWatch({
			control,
			name: pathPrefix,
		}) as PropertyListItem;
		...
		const currentDefinition = propertyListItem?.definition as
			| JSONSchema7
			| undefined;
		...
		return (
			<div
				className={cn(
					...
					className,
					isHighlighted
						? "overflow-hidden rounded-[8px] bg-[rgba(193,221,251,0.50)]"
						: "",
				)}
			>
				<div
