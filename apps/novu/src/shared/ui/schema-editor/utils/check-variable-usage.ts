 ...
 */
function buildFullPropertyPath(
	parentPath: string,
	propertyKey: string,
): string {
...
function isVariableUsedInStep(
	...
	return usedVariables.some((usedVar) => {
		...
		const normalizedUsedVar = usedVar.startsWith("payload.")
			? usedVar.substring(8)
			: usedVar;
		const normalizedKey = variableKey.startsWith("payload.")
			? variableKey.substring(8)
			: variableKey;
		const normalizedFullPath = fullPath.startsWith("payload.")
		...
		return (
			...
			normalizedUsedVar.match(
				new RegExp(
					`^${normalizedKey.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\[\\d+\\]`,
				),
			) ||
			normalizedUsedVar.match(
				new RegExp(
					`^${normalizedFullPath.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\[\\d+\\]`,
				),
			)
		);
	});
