...
const NestedProperty = memo<NestedPropertyProps>(function NestedProperty({
	...
	const nestedVariableUsageInfo = useMemo(() => {
		...
		return onCheckVariableUsage && nestedKeyName
			? onCheckVariableUsage(nestedKeyName, currentFullPath)
			: undefined;
	...
	return (
		<SchemaPropertyRow
			...
			index={nestedIndex}
			pathPrefix={
				`${nestedPropertyListPath}.${nestedIndex}` as Path<SchemaEditorFormValues>
			}
		...
		>
			...
			{isAtMaxDepth && (
				<div className="mt-1 rounded border border-red-200 bg-red-50 p-2 text-xs text-red-600">
					Maximum nesting depth of {MAX_NESTING_DEPTH} levels reached. Cannot
