...
export const VariableList = React.forwardRef<
	VariableListRef,
	VariablesListProps
>(
	(
		...
		const [hoveredOptionIndex, setHoveredOptionIndex] = useState(
		...
		const scrollToOption = useCallback((index: number) => {
			...
			if (optionElement) {
				...
				if (optionTop < listElement.scrollTop) {
					...
					listElement.scrollTop + containerHeight
				) {
		...
		return (
			<div className={cn("bg-background flex flex-col", className)}>
				<header className="flex items-center justify-between gap-1 rounded-t-md border-b border-neutral-100 bg-neutral-50 p-1">
					<span className="text-foreground-400 text-paragraph-2xs uppercase">
						{title}
					</span>
				...
				<footer className="flex items-center gap-1 border-t border-neutral-100 p-1">
					<div className="flex w-full items-center gap-0.5">
						<KeyboardItem>↑</KeyboardItem>
						<KeyboardItem>↓</KeyboardItem>
						<span className="text-foreground-600 text-paragraph-xs ml-0.5">
							Navigate
						</span>
			...
			<TooltipTrigger asChild>
				...
				>
					...
					<Check
						className={cn(
							"ml-auto size-4",
							selectedValue === option.value ? "opacity-50" : "opacity-0",
						)}
					/>
				</li>
			</TooltipTrigger>
