...
type ReorderFilterItemProps = ComponentProps<
	typeof Reorder.Item<FilterWithParam>
> & {
	index: number;
	isLast: boolean;
	variableName: string;
...
export const ReorderFilterItem = (props: ReorderFilterItemProps) => {
	...
	const options = useMemo(() => {
		...
		if (isDigestStepEventsVariable) {
			return variables
				.filter((v) => v.name.startsWith("payload"))
				...
				<div className="flex items-center gap-1">
					...
					<Tooltip>
						<TooltipTrigger className="cursor-pointer" asChild>
							<span>
								<Question
									className="text-text-soft size-4"
									onClick={preventClick}
								/>
							</span>
						</TooltipTrigger>
							...
							<div className="flex flex-col gap-1" key={paramIndex}>
								...
								{param.type === "variable" ? (
									<VariableSelect
										...
										isClearable
										error={
											toSentenceIssue?.filterParam === param.label
												? toSentenceIssue.message
												: undefined
										}
									/>
								) : (
