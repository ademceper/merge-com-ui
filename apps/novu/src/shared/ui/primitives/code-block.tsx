	...
	const maskedCode = useMemo(() => {
		...
		for (const mask of secretMask) {
			...
			if (maskStart !== undefined && maskEnd !== undefined) {
				lines[lineIndex] =
					lineContent.substring(0, maskStart) +
					"•".repeat(maskEnd - maskStart) +
					lineContent.substring(maskEnd);
			} else {
		...
		<div
			className={cn(
				...
				theme === "light"
					...
					: "border-neutral-800/50 bg-[#0d1117] shadow-lg",
				!title && "group",
				className,
			)}
		>
			...
			{!title && (
				<div className="relative">
					...
					>
						...
						{actionButtons ?? (
							<CopyToClipboard content={code} theme={theme} title="Copy code" />
						)}
					</div>
				</div>
			)}
			...
			<div
				className={cn(
					"flex h-full flex-col overflow-hidden px-[4px] pb-[4px]",
					!title && "pt-[4px]",
				)}
			>
				<div
					...
					className={cn(
						...
						theme === "light"
							...
							: "border-neutral-600/50 bg-[#161b22]",
					)}
				>
					{showGradient && (
						<div
