...
function useLocaleFiltering(
	...
	const baseLocales = useMemo(() => {
		if (availableLocales && availableLocales.length > 0) {
			return allLocales.filter((locale) =>
				availableLocales.includes(locale.langIso),
			);
		}
	...
	const filteredLocales = useMemo(() => {
		if (!searchValue.trim()) {
			...
			const common = baseLocales.filter((locale) =>
				COMMON_LOCALES.includes(locale.langIso),
			);
			const others = baseLocales.filter(
				(locale) => !COMMON_LOCALES.includes(locale.langIso),
			);
...
function MultiSelectTrigger({
	...
	const allLocales = getAllLocales();
	const selectedLocales = value
		? allLocales.filter((locale) => value.includes(locale.langIso))
		: [];
	const customLocales = value
		? value.filter(
				(val) => !allLocales.some((locale) => locale.langIso === val),
			)
		: [];
			...
			<div className="flex items-center gap-1.5 overflow-hidden">
				...
				{customLocales.map((locale, index) => (
					<div key={locale} className="flex shrink-0 items-center gap-1">
						...
						{selectedLocales.length + index < totalSelectedCount - 1 && (
							<span className="text-neutral-400">•</span>
						)}
					</div>
				))}
	...
	return (
		<div className="flex items-center gap-1.5">
			<StackedFlagCircles
				locales={[
					...selectedLocales.map((locale) => locale.langIso),
					...customLocales,
				]}
				maxVisible={10}
				size="md"
			/>
			<span className="text-xs font-normal text-neutral-950">
				{totalSelectedCount} locales selected
			</span>
		</div>
	);
}
...
export function LocaleSelect(props: LocaleSelectProps) {
	...
	const { filteredLocales, showSearchLimitMessage } = useLocaleFiltering(
		availableLocales,
		searchValue,
	);
	...
	const handleSelect = (localeValue: string) => {
		if (multiSelect && Array.isArray(value)) {
			const newValue = value.includes(localeValue)
				? value.filter((v) => v !== localeValue)
				: [...value, localeValue];
		...
		<div className="flex items-center gap-1.5">
			<div ref={containerRef} className="relative flex-1">
				<Button
					...
					mode="outline"
					className={cn(
						"flex h-8 w-full items-center justify-between gap-1 rounded-lg px-3 focus:z-10",
						className,
					)}
				...
				>
					...
					<CaretDown
						className={cn(
							...
							disabled || readOnly ? "hidden" : "opacity-100",
						)}
					/>
				</Button>
			...
			{showCimodeWarning && (
				<Tooltip>
					...
					<TooltipContent className="max-w-[260px]">
						<p className="text-xs">
							<span className="font-medium">cimode</span> will return
							translation keys without translating them. This locale is used for
							debugging purposes.
						</p>
					</TooltipContent>
				</Tooltip>
