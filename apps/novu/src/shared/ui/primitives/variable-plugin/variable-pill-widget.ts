...
} from "@/components/variable/utils";
...
export class VariablePillWidget extends WidgetType {
	...
	createBeforeStyles(): CSSProperties {
		return {
			...
			backgroundImage: this.isNotInSchema
				? `url("/images/error-warning-line.svg")`
				: `url("/images/code.svg")`,
	...
	renderFilters(parent: HTMLElement) {
		...
		const { finalParam, firstFilterName } = getFirstFilterAndItsArgs(
			this.filters,
		);
	...
	renderTooltip({
		...
		innerContainer.className =
			"border-stroke-soft/70 text-label-2xs rounded-sm border bg-white p-1";
	...
	getVariableIssues() {
		if (this.isDigestEventsVariable?.(this.variableName)) {
