...
export class VariablePluginView {
	...
	constructor(
		...
		private lastCompletionRef: MutableRefObject<{
			from: number;
			to: number;
		} | null>,
		...
		private getVariableErrorMessage?: (
			variableName: string,
			isAllowed: boolean,
		) => string | undefined,
	) {
	...
	createDecorations(view: EditorView) {
		...
		while ((match = regex.exec(content)) !== null) {
			...
			const isAllowed =
				...
				isVariableInLocalContext(content, name, start);

			// Get error message if variable is not allowed
			const errorMessage =
				!isAllowed && this.getVariableErrorMessage
					? this.getVariableErrorMessage(name, false)
					: undefined;
