...
const inlineToastVariants = cva(
	"flex items-center justify-between gap-3 rounded-lg border px-2 py-1.5",
	{
		variants: {
			variant: {
		...
		<div className={cn(inlineToastVariants({ variant }), className)} {...props}>
			<div className="flex min-w-0 flex-1 items-stretch gap-3">
				...
				<div className="text-foreground-600 min-w-0 flex-1 py-[2px] text-xs">
					{title && (
						<span className="text-foreground-950 font-medium">{title}</span>
					)}
					{title && description && " "}
					{description}
				</div>
