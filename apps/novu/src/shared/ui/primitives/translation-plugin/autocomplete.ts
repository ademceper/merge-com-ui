...
function findMatchingKeys(
	...
	if (!searchText) {
		return translationKeys.map((key) =>
			createCompletionOption(key.name, "translation", ""),
		);
	}
...
function createNewKeySuggestion(
	...
	const keyExists = existingKeys.some(
...
function createApplyFunction(
	translationOption: TranslationCompletionOption,
	config: TranslationAutocompleteConfig,
) {
	return (
		view: EditorView,
...
export function createTranslationAutocompleteSource(
	config: TranslationAutocompleteConfig,
): CompletionSource {
	...
	return (context: CompletionContext) => {
		...
		const lastTranslationStart = beforeCursor.lastIndexOf(
			TRANSLATION_TRIGGER_CHARACTER,
		);
		if (lastTranslationStart === -1) return null;

		const insideTranslation = state.sliceDoc(
			lastTranslationStart + TRANSLATION_TRIGGER_CHARACTER.length,
			pos,
		);
