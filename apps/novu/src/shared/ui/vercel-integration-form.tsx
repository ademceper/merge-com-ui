...
export const VercelIntegrationForm = ({
	...
	const {
		mutate: updateVercelIntegration,
		isPending: isUpdateVercelIntegrationPending,
	} = useUpdateVercelIntegration({
		next,
	});

	const onSubmit = (data: ProjectLinkFormValues) => {
		const payload = data.projectLinkState.reduce<Record<string, string[]>>(
			(prev, curr) => {
	...
	const updateRow = (
		rowIndex: number,
		value: GetVercelConfigurationDetails,
	) => {
		update(rowIndex, value);
	};

	const isDisabledLinkMore =
		projectRowCount >= organizations.length ||
		!!fields.find((el) => el.organizationId === "");
			...
			>
				<div className="flex flex-col gap-4">
					{fields.map((row, index) => {
						const rowOrg = organizations.find(
							(el) => row.organizationId === el.value,
						);
									...
									<SelectContent>
										...
										{organizations
											.filter(
												(org) =>
													!fields.some(
														(field) => field.organizationId === org.value,
													),
											)
											.map((org) => (
