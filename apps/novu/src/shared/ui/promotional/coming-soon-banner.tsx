...
import {
	ToggleGroup,
	ToggleGroupItem,
} from "@merge-rd/ui/components/toggle-group";
...
export function usePromotionalBanner(
	props: UsePromotionalBannerProps,
): UsePromotionalBannerResult {
	...
	const hide = useCallback(() => {
		...
		props.content.feedbackQuestion,
		props.content.telemetryEvent,
		props.content.title,
		track,
	]);
...
function PromotionalBannerContent({
	onDismiss,
	onReactionSelect,
	content,
}: UsePromotionalBannerProps) {
	const track = useTelemetry();
	...
	return (
		...
		>
			...
			<div className="space-y-1">
				<p className="text-foreground-950 text-sm font-medium">
				...
				<CardContent className="p-0">
					...
					>
						{REACTIONS.map((reaction, index) => (
							...
							>
								<ToggleGroupItem
									className="w-full py-2.5 text-xl transition-colors"
