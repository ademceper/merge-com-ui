...
import type { ListTopicSubscriptionsResponse } from "@/api/topics";
import type { DirectionEnum, IEnvironment, ISubscriberResponseDto } from "@/shared";
...
export const deleteSubscriber = async ({
    ...
    const response = await delV2<RemoveSubscriberResponseDto>(
...
export const getSubscriber = async ({
    ...
    const { data } = await getV2<{ data: SubscriberResponseDto }>(
        `/subscribers/${encodeURIComponent(subscriberId)}`,
        {
            environment
        }
...
export const patchSubscriber = async ({
    ...
    const { data } = await patchV2<{ data: SubscriberResponseDto }>(
        `/subscribers/${encodeURIComponent(subscriberId)}`,
        {
            environment,
            body: subscriber
...
export const createSubscriber = async ({
    ...
    const { data } = await postV2<{ data: SubscriberResponseDto }>(
