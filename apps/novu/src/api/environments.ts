...
export async function updateEnvironment({
    ...
    return put<{ data: IEnvironment }>(`/environments/${environment._id}`, {
        body: { name, color }
    });
}
...
export async function getTags({
    ...
    const { data } = await getV2<{ data: ITagsResponse }>(
        `/environments/${environment._id}/tags`
    );
    return data;
}
...
export async function createEnvironment(payload: {
    ...
    const response = await post<{ data: IEnvironment }>("/environments", {
        body: payload
    });
...
export async function diffEnvironments({
    ...
    const { data } = await postV2<{ data: IEnvironmentDiffResponse }>(
        `/environments/${targetEnvironmentId}/diff`,
        {
...
export async function publishEnvironments({
    ...
    const { data } = await postV2<{ data: IEnvironmentPublishResponse }>(
        `/environments/${targetEnvironmentId}/publish`,
        {
            body: {
                sourceEnvironmentId,
                dryRun: false,
