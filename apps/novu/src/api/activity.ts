...
function mapWorkflowRunToActivity(
	workflowRun: GetWorkflowRunResponse | GetWorkflowRunsDto,
): IActivity {
	return {
		_id: workflowRun.id,
		...
		to: {
			subscriberId:
				workflowRun.subscriberId || workflowRun.internalSubscriberId,
		},
...
export async function getWorkflowRunsList({
	...
	const response = await get<GetWorkflowRunsResponseDto>(
		`/activity/workflow-runs?${searchParams.toString()}`,
		{
			environment,
			signal,
		},
...
export async function getNotification(
	...
	const { data } = await get<{ data: IActivity }>(
...
export async function getWorkflowRun(
	...
	const data = await get<GetWorkflowRunResponseDto>(
		`/activity/workflow-runs/${workflowRunId}`,
		{
			environment,
		},
	);
