import type { RulesLogic } from "@novu/js";
...
export const deleteTopic = async ({
    ...
    const response = await delV2<{ acknowledged: boolean }>(
...
export const getTopic = async ({
    ...
    const { data } = await getV2<{ data: Topic }>(
        `/topics/${encodeURIComponent(topicKey)}`,
        {
            environment
        }
...
export const removeSubscribersFromTopic = async ({
    ...
    await delV2<DeleteTopicSubscriptionsResponseDto>(
        `/topics/${encodeURIComponent(topicKey)}/subscriptions`,
        {
            environment,
