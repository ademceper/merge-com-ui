...
function selectEnvironment(
	...
	if (selectedEnvironmentSlug) {
		environment = environments.find(
			(env) => env.slug === selectedEnvironmentSlug,
		);
	}
	...
	if (!environment) {
		environment = environments.find(
			(env) => env.name === DEVELOPMENT_ENVIRONMENT,
		);
	}
...
export function EnvironmentProvider({
	...
	const switchEnvironmentInternal = useCallback(
		(allEnvironments: IEnvironment[], environmentSlug?: string | null) => {
			...
			const isNewEnvironmentDifferent =
				paramsEnvironmentSlug !== selectedEnvironment.slug;
	...
	const { environments: fetchedEnvironments, areEnvironmentsInitialLoading } =
	...
	const oppositeEnvironment = useMemo((): IEnvironment | null => {
		...
		return (
			environments.find((env) => env.name === oppositeEnvironmentName) || null
		);
