...
export function EscapeKeyManagerProvider({
	children,
}: {
	...
	const registerEscapeHandler = useCallback(
		(
			...
			setHandlers((prev) => {
				...
				return [...filtered, { id, handler, priority }].sort(
	...
	return (
		<EscapeKeyManagerContext.Provider value={value}>
			{children}
		</EscapeKeyManagerContext.Provider>
	);
}
