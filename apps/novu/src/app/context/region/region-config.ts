...
function parseRegionsFromEnv(): RegionConfig[] {
	...
	for (const code of regionCodes) {
		...
		const dashboardUrl = isBaseRegion
			? DASHBOARD_URL
			: getEnvVar(`VITE_DASHBOARD_URL_${upperCode}`, "");

		const apiHostname = isBaseRegion
			? API_HOSTNAME
			: getEnvVar(`VITE_API_HOSTNAME_${upperCode}`, "");

		const websocketHostname = isBaseRegion
			? WEBSOCKET_HOSTNAME
			: getEnvVar(`VITE_WEBSOCKET_HOSTNAME_${upperCode}`, "");
		...
		if (!dashboardUrl || !apiHostname || !websocketHostname) {
			if (!isBaseRegion) {
				console.warn(
					`Skipping region ${code}: missing required environment variables`,
				);
				continue;
			}
		}
...
if (REGIONS.length === 0) {
	console.error(
		"No regions configured! Please set VITE_REGIONS environment variable.",
	);
}
