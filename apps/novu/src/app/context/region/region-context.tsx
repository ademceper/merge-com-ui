...
export function useRegion(): RegionContextType {
	...
	if (!context) {
		return {
			...
			setSelectedRegion: () => {},
			getApiHostname: () => apiHostnameManager.getHostname(),
		};
	}
	return context;
}
...
function RegionProvider({ children }: RegionProviderProps) {
	...
	const [orgCreationModal, setOrgCreationModal] =
		useState<OrgCreationModalState>({
			open: false,
			targetRegion: DEFAULT_REGION,
			previousRegion: DEFAULT_REGION,
		});

	const getApiHostname = useCallback(
		() => getApiHostnameForRegion(selectedRegion),
		[selectedRegion],
	);
	...
	}, [currentOrganization, detectRegionFromCurrentOrg, selectedRegion]);
	...
	const handleConfirmOrgCreation = () => {
		...
		const targetDashboardUrl = getDashboardUrlForRegion(
			orgCreationModal.targetRegion,
		);
