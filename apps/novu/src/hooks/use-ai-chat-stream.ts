...
>({
			...
			headers: async () => {
				...
				return {
					...
					...(environmentIdRef.current && {
						"Novu-Environment-Id": environmentIdRef.current,
					}),
				};
			},
			prepareSendMessagesRequest: (options) => {
				const resumeMessage =
					options.messages.length > 0
						...
						: null;
				const isResume =
					(options.requestMetadata as { resume?: boolean })?.resume ?? false;
	...
	const { messages, sendMessage, status, error, stop, setMessages } =
		useChatStream<UIMessage<unknown, D, T>>({
			id,
	...
	const reasoningParts = useMemo(() => {
		return messages
			...
			.flatMap((m) => m.parts.filter((p) => p.type === "reasoning"));
	}, [messages]);
	...
	const dataParts: DataUIPart<D>[] = useMemo(() => {
		return messages
			.filter((m) => m.role === "assistant")
			.flatMap((m) =>
				m.parts.filter((p) => p.type.startsWith("data-")),
			) as DataUIPart<D>[];
	}, [messages]);
