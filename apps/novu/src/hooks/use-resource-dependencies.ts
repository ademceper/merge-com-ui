...
export function useResourceDependencies(
	diffData: IEnvironmentDiffResponse | undefined,
): UseResourceDependenciesResult {
	const { workflows, layouts, dependencyMap } = useMemo(() => {
		if (!diffData?.resources) {
			return { workflows: [], layouts: [], dependencyMap: new Map() };
		}

		const workflowResources = diffData.resources.filter(
			(r: IResourceDiffResult) => r.resourceType === "workflow",
		);
		const layoutResources = diffData.resources.filter(
			(r: IResourceDiffResult) => r.resourceType === "layout",
		);
						...
						resourceDependencies.forEach((dep: IResourceDependency) => {
							if (dep.isBlocking) {
								...
								Object.entries(updated).forEach(
									([depResourceId, depResourceState]) => {
										const depResource = depResourceState.resource;
										const depResourceActualId =
											depResource.sourceResource?.id ||
											depResource.targetResource?.id;
							...
							dependencies.forEach((dep: IResourceDependency) => {
								...
								Object.entries(updated).forEach(([layoutId, layoutState]) => {
									if (
										...
										const layoutResourceId =
											layoutResource.sourceResource?.id ||
											layoutResource.targetResource?.id;
