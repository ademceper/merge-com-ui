...
import { ContextDrawerButton } from "@/pages/contexts/ui";
import { SubscriberDrawerButton } from "@/pages/subscribers/ui/subscriber-drawer";
import { TopicDrawerButton } from "@/pages/topics/ui/topic-drawer";
...
export function ActivityOverview({ activity }: ActivityOverviewProps) {
	...
	const renderTopicsContent = () => {
		...
		if (activity.topics.length === 1) {
			return (
				<TopicDrawerButton
					topicKey={activity.topics[0].topicKey}
					readOnly
	...
	const renderContextKeysContent = () => {
		...
		if (activity.contextKeys.length === 1) {
			return (
				<ContextDrawerButton
					contextKey={activity.contextKeys[0]}
					readOnly
						...
						{activity.contextKeys.map((contextKey, index) => (
							<React.Fragment key={contextKey}>
								...
								<ContextDrawerButton
									...
									readOnly
									className="group inline-block bg-transparent p-0"
								>
									<span className="cursor-pointer group-hover:underline">
										{contextKey}
									</span>
			...
			<div className="mb-2 flex flex-col gap-[12px]">
				...
				>
					<Link
						...
						className={cn(
							"text-foreground-600 cursor-pointer font-mono text-xs group-hover:underline",
							{
								"text-foreground-300 cursor-not-allowed":
									!activity.template?._id,
							},
						)}
					>
						{activity.template?.triggers?.[0]?.identifier || "Deleted workflow"}
				...
				<OverviewItem
					label="Triggered at"
					value={format(new Date(activity.createdAt), "MMM d yyyy, HH:mm:ss")}
				>
				...
				<OverviewItem label="Status">
					<span
						className={cn(
							...
							`text-${JOB_STATUS_CONFIG[status]?.color}`,
						)}
