...
import { ActivityJobItem } from "@/pages/activity/ui/activity-job-item";
	...
	return (
		<>
			<motion.div
				{...fadeIn}
				className={cn(
					"flex items-center justify-between border-neutral-100 p-2 px-3 pt-0",
					className,
				)}
			>
			...
			<motion.div
				{...fadeIn}
				className={cn(
					"flex items-center justify-between border-t border-neutral-100 p-2 px-3",
					className,
				)}
			>
			...
			>
				<PopoverContent className="w-[400px] p-0" align="center" side="left">
					<div className="flex items-center justify-between border-b border-neutral-100 p-3">
						<h3 className="text-foreground-950 text-sm font-medium">
							Request payload
						</h3>
					</div>
				</PopoverContent>
			...
			<motion.div
				{...fadeIn}
				className="flex flex-1 flex-col gap-6 overflow-y-auto bg-white p-3"
			>
				{jobs.map((job, index) => (
					<ActivityJobItem
						key={job._id}
				...
				<DialogContent className="flex max-h-[90vh] w-[90%] flex-col overflow-hidden p-0 [&>button.absolute.right-4.top-4]:hidden">
					<DialogHeader className="flex-none border-b border-neutral-100 p-3">
						<div className="flex items-center justify-between">
							<DialogTitle className="text-foreground-950 text-sm font-medium">
								Request payload
							</DialogTitle>
							<DialogClose asChild>
