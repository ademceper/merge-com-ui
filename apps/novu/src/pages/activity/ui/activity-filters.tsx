...
export function ActivityFilters({
	...
	return (
		<Form {...form}>
			<FormRoot
				className={cn(
					"w-full flex flex-wrap items-center gap-2 pb-2.5",
					className,
				)}
			>
						...
						render={({ field }) => (
							<FormItem>
								<FacetedFormFilter
									...
									selected={field.value}
									onSelect={(values) =>
										setValue("channels", values as ChannelTypeEnum[])
									}
								/>
							</FormItem>
						)}
						...
						render={({ field }) => (
							<FormItem>
								<FacetedFormFilter
									...
									selected={field.value}
									onSelect={(values) =>
										setValue("severity", values as SeverityLevelEnum[])
									}
								/>
							</FormItem>
						)}
