...
import { useDeleteLayout } from "@/pages/layouts/api/use-delete-layout";
import { useUpdateLayout } from "@/pages/layouts/api/use-update-layout";
import { TranslationToggleSection } from "@/pages/workflows/ui/workflow-editor/translation-toggle-section";
...
export const LayoutEditorSettingsDrawer = forwardRef<
	HTMLDivElement,
	LayoutEditorSettingsDrawerProps
>(({ isOpen, onOpenChange }, forwardedRef) => {
	...
	const isReadOnly =
		layout?.origin === ResourceOriginEnum.EXTERNAL ||
	...
	const blocker = useBlocker({
		shouldBlockFn: ({ next }) => {
			...
			const isLeavingEditor =
				!next.pathname.startsWith(layoutEditorBasePath);
	...
	return (
		<>
			<Sheet
				modal={false}
				open={isOpen}
				onOpenChange={(open) => checkUnsavedChanges(() => onOpenChange(open))}
			>
								...
								<FormField
									...
									render={({ field }) => (
										<TranslationToggleSection
											...
											showDrawer={
												!!(layout?.layoutId && layout?.isTranslationEnabled)
											}
										/>
									)}
								/>
