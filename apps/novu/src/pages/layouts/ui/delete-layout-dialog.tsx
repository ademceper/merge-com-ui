...
import { useFetchLayoutUsage } from "@/pages/layouts/api/use-fetch-layout-usage";
...
export const DeleteLayoutDialog = ({
	...
	const getDescription = () => {
		...
		return (
			<>
				...
				in <b>{currentEnvironment?.name}</b> and may cause breaking behavior.
				Please review dependent workflows before proceeding.
				<br />
				<br />
								...
								{usage?.workflows.map((workflow, index) => (
									...
									>
										<div className="flex h-5 w-5 items-center justify-center">
											<Path
												weight="fill"
												className="text-feature h-3.5 w-3.5"
											/>
										</div>
	...
	return (
		<ConfirmationModal
			...
			confirmButtonText={
				usage?.workflows.length === 0 || isUsagePending
					? "Delete layout"
					: "Proceed"
			}
