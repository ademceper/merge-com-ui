...
import { useLayoutPreview } from "@/pages/layouts/lib/use-layout-preview";
import {
	type UpdateLayoutParameters,
	useUpdateLayout,
} from "@/pages/layouts/api/use-update-layout";
...
import { usePersistedPreviewContext } from "@/pages/workflows/ui/workflow-editor/steps/hooks/use-persisted-preview-context";
...
function useLocaleSynchronization({
	...
	useEffect(() => {
		...
		if (selectedLocaleChanged && selectedLocale !== subscriberLocale) {
			...
			subscriberLocaleChanged &&
			subscriberLocale &&
			subscriberLocale !== selectedLocale &&
			onLocaleChange
		) {
...
export const LayoutEditorContext = createContext<LayoutContextType>(
...
export const LayoutEditorProvider = ({
	...
	const { data: organizationSettings, isLoading: isOrgSettingsLoading } =
		useFetchOrganizationSettings();
	const { currentEnvironment } = useEnvironment();
	const defaultValues = useMemo(
		() => (layout ? getLayoutControlsDefaultValues(layout) : {}),
		[layout],
	);
	const values = useMemo(
		() => (layout?.controls.values.email ?? {}) as Record<string, unknown>,
		[layout],
	);
	...
	const blocker = useBlocker({
		shouldBlockFn: ({ next }) => {
			return !next.pathname.startsWith(location.pathname);
		},
	...
	const { previewData, isPending: isPreviewPending } =
		useLayoutPreview(layoutPreviewParams);
	...
	const { updateLayout, isPending: isUpdating } = useUpdateLayout({
		...
		onError: (error) => {
			if (
				...
				"controls" in (error.rawError as any)
			) {
				const controlIssues = (error.rawError as any).controls;
				setFormIssues(controlIssues);

				const firstControlError = getFirstErrorMessage(
					{ controls: controlIssues },
					"controls",
				);
				showErrorToast(
	...
	const isNovuCloud =
		layout?.origin === ResourceOriginEnum.NOVU_CLOUD &&
		Boolean(layout?.controls.uiSchema);
	const isExternal = layout?.origin === ResourceOriginEnum.EXTERNAL;
	const isLayoutEditable =
		isExternal || (isNovuCloud && Boolean(layout?.controls.uiSchema));

	const setPreviewContextValueSafe = useCallback(
	...
	useEffect(() => {
		...
		const contextToUse =
			storedContextData && Object.keys(storedContextData).length > 0
				? storedContextData
				: {};
