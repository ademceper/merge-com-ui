...
import type { HelpTextInfo } from "@/pages/workflows/ui/conditions-editor/field-type-editors";
import { shouldUseRelativeDateEditor } from "@/pages/workflows/ui/conditions-editor/field-type-editors";
import { HelpIcon } from "@/pages/workflows/ui/conditions-editor/help-icon";
...
export const ValueEditor = (props: ValueEditorProps) => {
	...
	const stringValueAsArray = valueAsArray.map((v) =>
		typeof v === "string" ? v : `${v}`,
	);
...
function RelativeDateEditor({
	...
	const handleAmountChange = (newAmount: string) => {
		...
		const finalAmount =
			!Number.isNaN(amount) && amount > 0 ? amount : newAmount;
	...
	const handleUnitChange = (newUnit: string) => {
		const newValue = {
			...parsedValue,
			unit: newUnit as RelativeDateValue["unit"],
		};
		const jsonValue = JSON.stringify(newValue);
		onChange(jsonValue);
	};
