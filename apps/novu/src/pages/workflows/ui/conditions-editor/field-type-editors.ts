...
import type { EnhancedField } from "@/pages/workflows/ui/conditions-editor/conditions-editor";
...
export function getHelpTextForField(
	...
	if (operator === "withinLast") {
		return {
			...
			description:
				"Check if the date occurred within the last specified amount of time. Excludes future dates.",
			examples: [
				"within last 7 days",
	...
	if (operator === "notWithinLast") {
		return {
			...
			description:
				"Check if the date did NOT occur within the last specified amount of time.",
	...
	if (operator === "in" || operator === "notIn") {
		const action =
			operator === "in" ? "matches any of" : "does not match any of";
	...
	switch (dataType) {
		...
		case "number":
			return {
				...
				description:
					"Compare the number with the provided value. You can also use dynamic values from the payload.",
		...
		case "object":
			return {
				...
				description:
					"Compare object values using JSON format. You can also use dynamic values from the payload.",
