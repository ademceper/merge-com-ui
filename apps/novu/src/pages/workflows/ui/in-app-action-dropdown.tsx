...
import { URLInput } from "@/pages/workflows/ui/workflow-editor/url-input";
...
export const InAppActionDropdown = ({
	...
	const primaryActionRedirectUrl = getFieldState(
		`${primaryActionKey}.redirect.url`,
	);
	const secondaryActionLabel = getFieldState(`${secondaryActionKey}.label`);
	const secondaryActionRedirectUrl = getFieldState(
		`${secondaryActionKey}.redirect.url`,
	);
	const error =
		...
		<>
			<DropdownMenu modal={false}>
				...
				>
					<DropdownMenuItem
						onClick={() => {
							setValue(primaryActionKey, null, {
								shouldDirty: true,
								shouldValidate: true,
							});
							setValue(secondaryActionKey, null, {
								shouldDirty: true,
								shouldValidate: true,
							});
							onMenuItemClick?.();
			...
			{error && (
				<FormMessagePure hasError={!!error}>
...
const ConfigureActionPopover = (
	...
	return (
		<Popover>
			<PopoverTrigger {...rest} />
			<PopoverContent
				className="max-w-72 overflow-visible"
				side="bottom"
				align="end"
			>
