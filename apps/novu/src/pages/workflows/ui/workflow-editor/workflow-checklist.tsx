...
export function WorkflowChecklist({ steps, workflow }: WorkflowChecklistProps) {
	...
	const { environments = [] } = useFetchEnvironments({
		organizationId: currentEnvironment?._id,
	});
	...
	useEffect(() => {
		...
		const isFinishedLoading =
			currentEnvironment && workflow && integrations && environments;
		...
		<Popover open={isOpen} onOpenChange={handleOpenChange} modal={false}>
				...
				<button type="button" className="absolute bottom-[18px] left-[18px]">
					...
					>
						...
						<span className="text-xs">
							{checklistItems.filter((item) => item.isCompleted(steps)).length}/
			...
			>
				<div className="flex items-start justify-between">
					<div>
						<h3 className="text-foreground-900 text-label-sm mb-1 font-medium">
							Actions Recommended
						</h3>
						<p className="text-text-soft text-paragraph-xs mb-3">
							Let's make sure you have everything you need to send notifications
							to your users
						</p>
					</div>
		...
		() => [
				...
				isCompleted: (steps: Step[]) =>
					steps?.filter(
						(step) =>
							...
							![StepTypeEnum.DIGEST, StepTypeEnum.DELAY].includes(step.type),
					).length > 0,
				onClick: () => {
					telemetry(TelemetryEvent.WORKFLOW_CHECKLIST_STEP_CLICKED, {
						stepTitle: "Add a step",
					});

					if (steps.length === 0) {
						const addStepButton = document.querySelector(
							'[data-testid="add-step-menu-button"]',
						);
			...
			},
			{
				...
				isCompleted: (steps: Step[]) =>
					steps.some(
						(step: Step) =>
							step.type !== StepTypeEnum.TRIGGER && isStepContentComplete(step),
					),
				onClick: () => {
					telemetry(TelemetryEvent.WORKFLOW_CHECKLIST_STEP_CLICKED, {
						stepTitle: "Add notification content",
					});
					const stepToConfig = steps.find(
				...
				? [
						{
							...
							onClick: () => {
								...
								navigate({
									to: `${ROUTES.INBOX_EMBED}?environmentId=${currentEnvironment?._id}`,
								});
							},
						},
					]
	...
	return (
		...
		>
			...
			<div className="text-label-xs text-text-sub">
				<span className={cn(item.isCompleted(steps) && "line-through")}>
					{item.title}
				</span>
			</div>
			...
			<CaretDoubleRight
				weight="fill"
				className="text-text-soft ml-auto h-4 w-4 opacity-0 transition-opacity duration-200 group-hover:opacity-100"
			/>
		</button>
	);
}
