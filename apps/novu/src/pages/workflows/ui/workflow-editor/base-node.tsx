...
import { StepPreview } from "@/pages/workflows/ui/step-preview-hover-card";
...
export const NodeBody = ({
	...
	const isPreviewEnabled = useFeatureFlag(
		FeatureFlagsKeysEnum.IS_WORKFLOW_NODE_PREVIEW_ENABLED,
	);
			...
			{(isPreviewEnabled || showPreview) && (
				...
				>
					{type !== StepTypeEnum.TRIGGER && (
						<HoverCardContent
							side="left"
							className="border-stroke-soft bg-bg-weak w-[450px] p-1"
							sideOffset={15}
						>
		...
		<Popover open={isPopoverOpen}>
			<PopoverTrigger asChild>
				...
				>
					<WarningCircle
						weight="fill"
						className="border-destructive fill-destructive bg-foreground-0 rounded-full border p-px"
					/>
				</span>
			</PopoverTrigger>
			<PopoverPortal>
				<PopoverContent
					className="flex min-w-min max-w-[200px] rounded-xl p-2"
					side="right"
				>
					<PopoverArrow />
