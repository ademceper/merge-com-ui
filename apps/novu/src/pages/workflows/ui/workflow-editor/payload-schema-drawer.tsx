...
import { SchemaEditor } from "@/widgets/schema-editor-widget/schema-editor";
import { convertSchemaToPropertyList } from "@/widgets/schema-editor-widget/utils";
import { checkVariableUsageInWorkflow } from "@/widgets/schema-editor-widget/utils/check-variable-usage";
import {
	detectSchemaChanges,
	type SchemaChanges,
} from "@/widgets/schema-editor-widget/utils/schema-change-detection";
...
} from "./payload-schema/components";
...
export function PayloadSchemaDrawer({
	...
	const [drawerSchema, setDrawerSchema] = useState<JSONSchema7 | undefined>(
	...
	const [pendingChanges, setPendingChanges] = useState<SchemaChanges | null>(
		null,
	);
	...
	}, [workflow?.payloadSchema, drawerSchema]);
	...
	const hasPayloadSchema =
		fields.length > 0 ||
		(workflow?.payloadSchema &&
			Object.keys(workflow.payloadSchema.properties || {}).length > 0);
	...
	return (
		<>
			<Sheet open={isOpen} onOpenChange={handleSheetOpenChange}>
				<SheetContent
					ref={protectionRef}
					className="bg-bg-weak flex w-[600px] flex-col p-0 sm:max-w-3xl"
				>
											...
											<div className="rounded-4 border mb-2 flex items-center justify-between border-neutral-100 bg-white p-1.5">
												<div className="text-text-strong text-label-xs flex items-center gap-1">
													...
													<Tooltip>
														...
														<TooltipContent>
															<p>
																When enabled, the workflow will validate
																incoming payloads against the defined schema and
																reject invalid requests during the trigger http
																request.
															</p>
														</TooltipContent>
													</Tooltip>
								...
								{hasPayloadSchema && (
									<>
										...
										<Hint className="text-text-soft p-2 px-3">
											<HintIcon as={Info} />
											Modifying a variable&apos;s type can break step behavior
											if the variable is used in logic or expressions.
										</Hint>
									</>
								)}
							</SheetMain>
