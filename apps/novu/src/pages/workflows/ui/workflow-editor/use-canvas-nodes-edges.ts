...
export const useCanvasNodesEdges = ({
	...
	const [currentNodes, setNodes] = useNodesState<
		Node<NodeData, keyof typeof nodeTypes>
	>([]);
	const [currentEdges, setEdges] = useEdgesState<AddNodeEdgeType>([]);
	const [currentSelectedNodeId, setSelectedNodeId] = useState<
		string | undefined
	>();
	const [draggedNodeId, setDraggedNodeId] = useState<string | null>(null);
	const [intersectingNodeId, setIntersectingNodeId] = useState<string | null>(
		null,
	);
	const [intersectingEdgeId, setIntersectingEdgeId] = useState<string | null>(
		null,
	);
		...
		timeoutRef.current = setTimeout(() => {
			setEdges(
				createEdges(
					...
					dataRef.current.isTemplateStorePreview,
				),
			);
	...
	const insertStep = useCallback(
		(
			...
			const stepNodes = nodes.filter(
				(node) => node.type !== "trigger" && node.type !== "add",
			);
				...
				},
				{
					onSuccess: (newWorkflow) => {
						...
						if (newStep && environment?.slug) {
							const isTemplateConfigurable =
								TEMPLATE_CONFIGURABLE_STEP_TYPES.includes(newStep.type);

							if (isTemplateConfigurable) {
								navigate({
									to: buildRoute(ROUTES.EDIT_STEP_TEMPLATE, {
										stepSlug: newStep.slug,
									}),
								});
							} else if (
								INLINE_CONFIGURABLE_STEP_TYPES.includes(newStep.type)
							) {
								navigate({
									to: buildRoute(ROUTES.EDIT_STEP, {
										stepSlug: newStep.slug,
									}),
								});
							}
	...
	const copyNode = useCallback(
		(copyIndex: number) => {
			...
			const currentStep = workflow.steps.find(
				(step) => step.slug === copyNode.data.stepSlug,
			);
			...
			update(
				{
					...
					steps: workflow.steps.filter(
						(s) => s.slug !== nodeToRemove.data.stepSlug,
					),
				},
				{
					onSuccess: () => {
						const newNodes = [...dataRef.current.nodes].filter(
						...
						if (
							selectedNodeId === nodeToRemove.id &&
							environment?.slug &&
							workflow?.slug
						) {
							navigate({
		...
		(
			...
			const selectedNode = newNodes.find(
				(node) =>
					...
					getIdFromSlug({
						slug: node.data.stepSlug ?? "",
						divider: STEP_DIVIDER,
					}),
			);
			if (selectedNode) {
			...
			setNodes(
				recalculatePositionAndIndex(newNodes, dataRef.current.containerWidth),
			);
			removeEdges();
				...
				},
				{
					onSuccess: () => {
						const finalNodes = recalculatePositionAndIndex(
							newNodes.map((node) => ({
								...node,
								data: { ...node.data, isPending: false },
							})),
							dataRef.current.containerWidth,
						);
	...
	const handleNodeDragMove = useCallback(() => {
		...
		const draggableNode = document.querySelector(
			`[data-draggable-node-id="${draggedNodeId}"]`,
		);
		...
		for (const node of nodes) {
			...
			const currentNode = document.querySelector(
				`[data-droppable-node-id="${node.id}"]`,
			);
		...
		if (addNode && isIntersecting(addNode, draggableNode) && !isLastNode) {
			setIntersectingNodeId(
				addNode.getAttribute("data-droppable-add-node-id") ?? null,
			);
			setIntersectingEdgeId(null);
			foundNodeIntersection = true;
		}
		...
		for (const edge of edges) {
			...
			const edgeNode = document.querySelector(
				`[data-droppable-edge-id="${edge.id}"]`,
			);
	...
	const handleNodeDragEnd = useCallback(() => {
		...
		const draggedStepIndex = steps.findIndex(
			(s) =>
				...
				getIdFromSlug({
					slug: draggedNode?.data.stepSlug ?? "",
					divider: STEP_DIVIDER,
				}),
		);
		...
		if (intersectingNodeId && !isLastAddNode) {
			...
			if (hoveredNode?.data.stepSlug) {
				const hoveredStepIndex = steps.findIndex(
					(s) =>
						getIdFromSlug({ slug: s.slug, divider: STEP_DIVIDER }) ===
						getIdFromSlug({
							slug: hoveredNode.data.stepSlug ?? "",
							divider: STEP_DIVIDER,
						}),
				);
				const hoveredNodeIndex = nodes.findIndex(
					(n) => n.id === intersectingNodeId,
				);
				...
				if (hoveredStepIndex !== -1 && hoveredStepIndex !== draggedStepIndex) {
					...
					newNodes[draggedNodeIndex] = {
						...hoveredNode,
						data: { ...hoveredNode.data, isPending: true },
					};
					newNodes[hoveredNodeIndex] = {
						...draggedNode,
			...
			newNodes.push({
				...tempNode,
				data: { ...tempNode.data, isPending: true },
			});
			if (addNode) {
		...
		if (intersectingEdgeId) {
			...
			if (hoveredEdge) {
				...
				const sourceNodeIndex = nodes.findIndex(
					(n) => n.id === hoveredEdge.source,
				);
				const targetNodeIndex = nodes.findIndex(
					(n) => n.id === hoveredEdge.target,
				);
				...
				if (insertNodeIndex !== -1 && draggedNodeIndex !== insertNodeIndex) {
					...
					const adjustedInsertStepIndex =
						draggedStepIndex < insertStepIndex
							? insertStepIndex - 1
							: insertStepIndex;
					const adjustedInsertNodeIndex =
						draggedNodeIndex < insertNodeIndex
							? insertNodeIndex - 1
							: insertNodeIndex;
	...
	const selectNode = useCallback(
		(id: string, goto: "editor" | "view" = "editor") => {
			...
			if (goto === "editor") {
				const isTemplateConfigurable =
					TEMPLATE_CONFIGURABLE_STEP_TYPES.includes(
						NODE_TYPE_TO_STEP_TYPE[
							potentialNode?.type as keyof typeof NODE_TYPE_TO_STEP_TYPE
						],
					);
				if (isTemplateConfigurable) {
					navigate({
	...
	useEffect(() => {
		...
		const timeout = setTimeout(() => {
			...
			const previousPosition =
				newNodes[newNodes.length - 1]?.position ?? triggerNode.position;
			const addNode =
				nodes.find((node) => node.type === "add") ??
				createAddNode(previousPosition, newNodes);
			const finalNodes = [triggerNode, ...newNodes, addNode].filter(
				(node) => node !== undefined,
			);
			...
			setNodes(
				recalculatePositionAndIndex(finalNodes, dataRef.current.containerWidth),
			);
			updateEdges();
		}, 0);
