...
type RequiredFieldStrategy = "none" | "all" | "heuristic" | "custom";
 ...
 */
function shouldBeRequired(value: unknown, _threshold: number = 0.8): boolean {
...
function determineSchemaType(
	...
	if (Array.isArray(value)) {
		return {
			...
			items:
				value.length > 0
					? determineSchemaType(value[0], options)
					: { type: "string" },
		};
	}
