...
import { UserPreferencesFormSchema } from "@/pages/workflows/ui/workflow-editor/schema";
...
const checkHasEveryChannelSameValue = (
	...
	return Object.values(channels).every(
		(channel) => channel.enabled === checkForEnabled,
	);
};
...
export const ChannelPreferencesForm = (props: ConfigureWorkflowFormProps) => {
	...
	const isDefaultPreferences = useMemo(
		() => workflow.preferences.user === null,
		[workflow.preferences.user],
	);
	const isDashboardWorkflow = useMemo(
	...
	const formDataToRender = useMemo(() => {
		...
		const defaultPreferences = isDefaultPreferences
		...
		const channelsInUse = allChannelsArr.filter((channel) =>
			steps.has(channel as StepTypeEnum),
		);
		const channelsNotInUse = allChannelsArr.filter(
			(channel) => !steps.has(channel as StepTypeEnum),
		);
	...
	const updateUserPreference = (
		...
		const value =
			userPreferences === null ? workflow.preferences.default : userPreferences;
		form.reset({
			user: value,
	...
	const handleChannelToggle = (channel: ChannelTypeEnum, value: boolean) => {
		...
		if (
			areAllChannelsSameValue ||
			(value && !updatedUserPreferences.all.enabled)
		) {
			...
			>
				...
				<SidebarContent size="md">
					<p className="text-xs text-neutral-400">
						Set default channel preferences for subscribers and specify which
						channels they can customize.
					</p>
				</SidebarContent>
											...
											<FormControl>
												<Switch
													...
													onCheckedChange={(checked) => {
														...
														track(
															TelemetryEvent.WORKFLOW_PREFERENCES_OVERRIDE_USED,
															{
																new_status: checked,
															},
														);
				...
				<Form {...form}>
					<FormRoot>
													...
													<div className="mt-2 flex flex-col gap-2">
														<div>
															...
															<ul className="text-text-sub text-2xs list-disc pl-4">
																<li>
																	Severity is a way to classify the importance
																	of a notification — from high-priority to
														...
														<span className="text-text-sub text-2xs">
															This value is stored in the Workflow Properties
															and exposed via the Data Object.{" "}
															<Link
						...
						<div className="flex items-center justify-between gap-1.5 bg-neutral-50 px-3 py-0.5">
							<span className="text-2xs uppercase text-neutral-400">
								All channels
							</span>
							<FormField
								control={form.control}
									...
									<FormControl className="m-1">
										<Checkbox
											...
											disabled={
												!override ||
												isReadOnly ||
												formDataToRender?.channelsInUse.length === 0
											}
										/>
									</FormControl>
								)}
											...
											render={({ field }) => (
												<FormItem className="mt-2 flex w-full items-center justify-between">
													<div className="flex items-center gap-2">
														<Step
															variant={
																STEP_TYPE_TO_COLOR[channel as StepTypeEnum]
															}
															className="size-5"
														>
															<Icon />
													...
													<Tooltip>
														...
														<TooltipContent className="w-64" align="end">
															<span className="text-2xs">
																Add the channel to your workflow to control its
						...
						<Card className="bg-information/10 border-information/40 border px-2.5 py-2">
							<CardContent className="flex flex-nowrap items-center gap-2 p-0">
								<div className="size-5">
									<Info
										weight="fill"
										className="text-information m-0.5 size-4"
									/>
								</div>
								<span className="text-2xs">
									Preferences defined in code have been overridden. Disable
									overrides to restore original.
								</span>
							</CardContent>
						</Card>
					</SidebarContent>
