...
import { useUpdateWorkflow } from "@/pages/workflows/api/use-update-workflow";
...
type WorkflowContextType = {
	...
	) => Promise<QueryObserverResult<WorkflowResponseDto, Error>>;
...
export const WorkflowProvider = ({ children }: { children: ReactNode }) => {
	...
	const { workflowSlug = "", stepSlug = "" } = useParams({ strict: false });
	...
	const isStepAfterDigest = useMemo(() => {
		...
		const hasDigestStepInBetween = workflow?.steps
			.slice(0, index)
			.some((s) => s.type === "digest");
	...
	const digestStepBeforeCurrent = useMemo(() => {
		...
		const digestStep = stepsBeforeCurrent
			.reverse()
			.find((step) => step.type === "digest");
	...
	const isUpdatePatchPending =
		isPatchPending || isUpdatePending || hasPendingItems;
	...
	const blocker = useBlocker({
		shouldBlockFn: ({ next }) => {
			...
			const isLeavingEditor = !next.pathname.startsWith(
				workflowEditorBasePath,
			);
...
const SavingChangesDialog = ({
	...
	return (
		<Dialog
			modal
			open={isOpen}
			onOpenChange={(open) => !open && isUpdatePatchPending && onCancel()}
		>
			<DialogPortal>
				<DialogOverlay />
				<DialogContent
						...
						>
							<div className="transition-opacity duration-300">
								{isUpdatePatchPending ? (
									<Warning
										weight="fill"
										className="text-warning animate-in fade-in size-6"
									/>
								) : (
