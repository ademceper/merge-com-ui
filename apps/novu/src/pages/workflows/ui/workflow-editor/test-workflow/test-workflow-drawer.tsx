...
import { SubscriberDrawer } from "@/pages/subscribers/ui/subscriber-drawer";
import { useFetchSubscriber } from "@/pages/subscribers/api/use-fetch-subscriber";
import type {
	PayloadData,
	PreviewSubscriberData,
} from "@/pages/workflows/ui/workflow-editor/steps/types/preview-context.types";
import { TestWorkflowActivityDrawer } from "@/pages/workflows/ui/workflow-editor/test-workflow/test-workflow-activity-drawer";
import { TestWorkflowContent } from "@/pages/workflows/ui/workflow-editor/test-workflow/test-workflow-content";
...
const getContextSpread = (contextData: Partial<ContextPayload> | null) => {
	return contextData && Object.keys(contextData).length > 0
		? { context: contextData }
		: {};
};

export const TestWorkflowDrawer = forwardRef<
	HTMLDivElement,
	TestWorkflowDrawerProps
>((props, forwardedRef) => {
	...
	const [subscriberData, setSubscriberData] =
		useState<PreviewSubscriberData | null>(null);
	...
	useEffect(() => {
		if (
			...
			subscriberData?.subscriberId === fetchedSubscriberData.subscriberId
		) {
			setSubscriberData({
						...
						<>
							...
							<div className="flex flex-col gap-2">
								...
								<span className="text-foreground-600 inline">
									...
									cannot be triggered. Ensure that it is active and requires not
									further actions.
								</span>
							</div>
	...
	const handleClearPersistedPayload = useCallback(() => {
		...
		const newPayload: PayloadData =
			isPayloadSchemaEnabled && workflow?.payloadExample
				? (workflow.payloadExample as PayloadData)
				: {};
				...
				<div className="flex h-full flex-col">
					<TestWorkflowContent
						...
						onClearPersistedPayload={
							workflow?.workflowId && currentEnvironment?._id
								? handleClearPersistedPayload
								: undefined
						}
						onClearPersistedSubscriber={
							workflow?.workflowId && currentEnvironment?._id
								? handleClearPersistedSubscriber
								: undefined
						}
						onClearPersistedContext={
							workflow?.workflowId && currentEnvironment?._id
								? handleClearPersistedContext
								: undefined
						}
			...
			<TestWorkflowActivityDrawer
				...
				payload={
					currentFormData?.payload
						? JSON.stringify(currentFormData.payload, null, 2)
						: undefined
				}
			/>
