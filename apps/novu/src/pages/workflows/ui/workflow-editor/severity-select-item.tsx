...
const TOOLTIP_CONTENT_LOOKUP: Record<SeverityLevelEnum, string> = {
	...
	[SeverityLevelEnum.LOW]:
		"No hue by default, but styling can be customized via API. Respects user preferences.",
	[SeverityLevelEnum.NONE]: "",
};

const TOOLTIP_IMAGE_LOOKUP: Record<SeverityLevelEnum, React.ReactNode> = {
	[SeverityLevelEnum.HIGH]: (
		<img src="/images/severity/high.webp" alt="high severity" />
	),
	[SeverityLevelEnum.MEDIUM]: (
		<img src="/images/severity/medium.webp" alt="medium severity" />
	),
	[SeverityLevelEnum.LOW]: (
		<img src="/images/severity/low.webp" alt="low severity" />
	),
...
const TOOLTIP_BADGE_COLOR_LOOKUP: Record<
	SeverityLevelEnum,
	BadgeRootProps["color"]
> = {
...
export const SeveritySelectItem = ({
	severity,
}: {
	...
	return (
		<Tooltip>
			...
			<TooltipContent
				className={
					"bg-background max-w-48 rounded-lg shadow-md whitespace-pre-wrap p-1"
				}
				side="left"
			>
				<div className="flex flex-col gap-1">
					<div className="border rounded-sm border-bg-soft">
						{TOOLTIP_IMAGE_LOOKUP[severity]}
					</div>
					<div className="flex flex-col gap-2 p-[2px] items-start">
						...
						<span className="text-text-sub text-2xs">
							{TOOLTIP_CONTENT_LOOKUP[severity]}
						</span>
					</div>
				</div>
			</TooltipContent>
