...
import type {
	JSONSchema7,
	JSONSchema7TypeName,
} from "@/widgets/schema-editor-widget/json-schema";
import { useSchemaForm } from "@/widgets/schema-editor-widget/use-schema-form";
import { convertSchemaToPropertyList } from "@/widgets/schema-editor-widget/utils";
import type {
	PropertyListItem,
	SchemaEditorFormValues,
} from "@/widgets/schema-editor-widget/utils/validation-schema";
...
function getSchemaPropertyByKeyInternal(
	keyPath: string,
	...
	for (const part of parts) {
		...
		if (nodeType === "object") {
			...
			if (
				!currentSchemaNode.items ||
				typeof currentSchemaNode.items === "boolean"
			) {
				return undefined;
			}
...
export interface UseWorkflowSchemaManagerReturn {
	...
	addProperty: (
		propertyData?: Partial<PropertyListItem>,
...
export function useWorkflowSchemaManager({
	...
	const [internalValidatePayload, setInternalValidatePayload] = useState(
