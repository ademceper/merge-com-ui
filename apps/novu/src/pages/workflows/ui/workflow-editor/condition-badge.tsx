...
export const ConditionBadge = ({
	...
	const displayVariableName = useMemo(() => {
		...
		return variableParts.length >= 3
			? `..${variableParts.slice(-2).join(".")}`
			: firstConditionField;
	}, [firstConditionField]);
	...
	const handleClick = (e: React.MouseEvent) => {
		e.preventDefault();
		e.stopPropagation();
		navigate({ to: buildRoute(ROUTES.EDIT_STEP_CONDITIONS, { stepSlug }) });
	};
	...
	return (
		...
		>
			<div className="flex items-center gap-1.5 text-foreground-600 font-code tracking-[-0.24px]">
				<span className="text-foreground-400 text-xs italic font-serif">
					if
				</span>
				<span className="bg-bg-weak border-stroke-soft font-code inline-flex items-center gap-1 rounded-md border px-1.5 py-px font-medium">
					...
					<span
						className="text-label-xs text-[#6C6E73] max-w-[24ch] truncate "
						title={displayVariableName}
					>
						{displayVariableName}
					</span>
				</span>
				{moreText && (
					<span className="text-text-soft font-code text-center font-medium leading-4 text-2xs">
						{moreText}
					</span>
				)}
			</div>
		...
		>
			<div className="flex items-center gap-1.5 text-foreground-600 font-code tracking-[-0.24px]">
				...
				<span className="bg-bg-weak border-stroke-soft font-code inline-flex items-center gap-1 rounded-md border px-1.5 py-px font-medium">
					<Code2 className="text-feature size-3.5 min-w-3.5" />
					<span
						className="leading-1 text-[#6C6E73] max-w-[24ch] truncate "
						title={displayVariableName}
					>
						{displayVariableName}
					</span>
