...
import {
	...
	parseJsonValue,
} from "./utils/preview-context.utils";
...
export function PreviewContextPanel({
	...
	const schemas = useMemo(
		() => ({
			...
			subscriber: previewSchema?.properties?.subscriber as
				| JSONSchema7
				| undefined,
	...
	const hasDigestStep = useMemo(() => {
		return (
			workflow?.steps?.some((step) => step.type === StepTypeEnum.DIGEST) ??
			false
		);
	}, [workflow?.steps]);
	...
	const handleClearPersistedPayload = useCallback(() => {
		...
		const newPayload: PayloadData =
			workflow?.payloadExample && isPayloadSchemaEnabled
		...
		<>
			...
			>
				<PreviewPayloadSection
					...
					onClearPersisted={
						canClearPersisted ? handleClearPersistedPayload : undefined
					}
