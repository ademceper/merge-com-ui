...
import { TIME_UNIT_OPTIONS } from "@/pages/workflows/ui/workflow-editor/steps/time-units";
...
function deriveLookbackType(lookBackWindow?: {
	amount?: number;
	unit?: string;
}): LookbackType {
	...
	if (
		lookBackWindow.amount === 5 &&
		lookBackWindow.unit === TimeUnitEnum.MINUTES
	) {
		return "5min";
	}

	if (
		lookBackWindow.amount === 30 &&
		lookBackWindow.unit === TimeUnitEnum.MINUTES
	) {
		return "30min";
	}
...
export const LookbackWindow = ({ isReadOnly }: { isReadOnly: boolean }) => {
	...
	const handleLookbackTypeChange = async (value: LookbackType) => {
		if (value === "immediately") {
			setValue("controlValues.lookBackWindow", undefined, {
				shouldDirty: true,
			});
		} else if (value === "5min") {
			...
			if (
				...
				setValue(LOOKBACK_UNIT_KEY, TimeUnitEnum.MINUTES, {
					shouldDirty: true,
				});
			}
		}
				...
				render={() => (
					<FormItem>
						<FormControl>
							<Select
								value={lookbackType}
								onValueChange={handleLookbackTypeChange}
								disabled={isReadOnly}
							>
			...
			{lookbackType === "custom" && (
				<div className="flex items-center justify-between">
					<span className="text-foreground-600 text-xs font-medium">
						When events repeat within
					</span>
					<AmountInput
						fields={{
							inputKey: LOOKBACK_AMOUNT_KEY,
							selectKey: LOOKBACK_UNIT_KEY,
						}}
