...
import { useUpdateTranslationValue } from "@/pages/translations/api/use-update-translation-value";
							...
							<Tooltip>
								...
								<TooltipContent side="top" className="max-w-xs">
									<p className="text-label-xs">
										A unique identifier for this translation. Keys are added to
										the default language ({DEFAULT_LOCALE}). Use dot notation
										for nested keys (e.g., "welcome.title" or "buttons.submit").
									</p>
								</TooltipContent>
							</Tooltip>
						</label>
						...
						>
							...
							>
								<span className="underline">
									{isCreatingKey ? "Adding..." : "Add translation key ↗"}
								</span>
							</LinkButton>
						</FormMessagePure>
					)}
				...
				<div className="flex items-center justify-between">
					<label className="text-text-sub text-label-xs flex items-center gap-1">
						...
						<Tooltip>
							...
							<TooltipContent side="top" className="max-w-xs">
								<p className="text-label-xs">
									The translated text content. Use {"{{"} to insert dynamic
									variables from your workflow payload or step data.
								</p>
							</TooltipContent>
						</Tooltip>
					</label>
...
export const EditTranslationPopover: React.FC<EditTranslationPopoverProps> = ({
	...
	const id = useId();
	const { translationKeys, isLoading, translationData } =
	...
	const handleClose = useCallback(() => {
		...
		if (
			editor.hasUserEditedKey &&
			onReplaceKey &&
			trimmedKey &&
			trimmedKey !== editor.initialKeyOnOpen
		) {
