...
import {
	...
	TYPE_KEY,
	UNIT_KEY,
} from "@/pages/workflows/ui/workflow-editor/steps/digest-delay-tabs/keys";
import { LookbackWindow } from "@/pages/workflows/ui/workflow-editor/steps/digest-delay-tabs/lookback-window";
import { RegularType } from "@/pages/workflows/ui/workflow-editor/steps/digest-delay-tabs/regular-type";
import { ScheduledType } from "@/pages/workflows/ui/workflow-editor/steps/digest-delay-tabs/scheduled-type";
import { EVERY_MINUTE_CRON } from "@/pages/workflows/ui/workflow-editor/steps/digest-delay-tabs/utils";
...
export const DigestDelayTabs = ({
	isDigest = true,
}: {
	isDigest?: boolean;
}) => {
	const { workflow } = useWorkflow();
	const { control, getFieldState, setValue, setError, getValues, trigger } =
		useFormContext();
	...
	const [preservedFormValuesByType, setPreservedFormValuesByType] =
		useState<PreservedFormValuesByType>({
			regular: undefined,
			scheduled: undefined,
		});
	...
	const handleTypeChange = async (value: string) => {
		...
		setPreservedFormValuesByType((old) => ({
			...old,
			[type]: { ...controlValues },
		}));
		setType(value);
		...
		if (preservedFormValues) {
			...
			setValue(
				TYPE_KEY,
				isDigest ? DigestTypeEnum.TIMED : DelayTypeEnum.TIMED,
				{ shouldDirty: true },
			);
		} else {
			setValue(
				AMOUNT_KEY,
				isDigest ? DEFAULT_CONTROL_DIGEST_AMOUNT : DEFAULT_CONTROL_DELAY_AMOUNT,
				{
					shouldDirty: true,
				},
			);
			...
			setValue(
				TYPE_KEY,
				isDigest ? DigestTypeEnum.REGULAR : DelayTypeEnum.REGULAR,
				{ shouldDirty: true },
			);
		}
						...
						<TabsList className="w-full">
							<Tooltip delayDuration={POPOVER_DURATION_MS}>
								...
								<TooltipContent className="max-w-56" side="top" sideOffset={10}>
									{isDigest ? (
										<span>
											Set the amount of time to digest events for. Once the
											defined time has elapsed, the digested events are sent.
										</span>
									) : (
										<span>
											Delays workflow execution for the set time, then proceeds
											to the next step.
										</span>
									)}
								</TooltipContent>
							...
							<Tooltip delayDuration={POPOVER_DURATION_MS}>
								...
								<TooltipContent className="max-w-56" side="top" sideOffset={10}>
									{isDigest ? (
										<span>
											Schedule the digest on a repeating basis (every 3 hours,
											every Friday at 6 p.m., etc.) to get full control over
											when your digested events are processed and sent.
										</span>
									) : (
										<span>
											Delays workflow execution until a specific scheduled time
											(e.g. until Friday at 6 p.m.), then proceeds to the next
											step.
										</span>
									)}
								</TooltipContent>
