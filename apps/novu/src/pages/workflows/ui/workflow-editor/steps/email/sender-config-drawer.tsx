...
import { usePrimaryEmailIntegration } from "@/pages/integrations/lib/use-primary-email-integration";
...
export function SenderConfigDrawer({
	open,
	onOpenChange,
}: SenderConfigDrawerProps) {
	const { getValues, setValue } = useFormContext();
	const { saveForm } = useSaveForm();
	const { senderEmail: integrationEmail, senderName: integrationName } =
		usePrimaryEmailIntegration();
		...
		<Sheet open={open} onOpenChange={onOpenChange}>
			<SheetContent className="flex w-[400px] flex-col p-0 sm:max-w-[400px]">
				<SheetHeader className="space-y-1 px-3 py-4">
					<SheetTitle className="text-label-lg flex items-center gap-2 mb-0">
					...
					<div className="rounded-4 flex items-center justify-between bg-white mt-1.5">
						<div className="text-text-strong text-label-xs flex items-center gap-1">
							...
							<Tooltip>
								...
								<TooltipContent>
									<p>
										When enabled, the email will use the sender name and email
										from your configured email integration.
									</p>
								</TooltipContent>
							</Tooltip>
						</div>
										...
										<ControlInput
											placeholder={
												localUseDefaults
													...
													: integrationName || "e.g. Acme Security"
											}
							...
							<FormLabel className="flex items-center gap-1">
								...
								<Tooltip>
									...
									<TooltipContent className="max-w-[280px]">
										<p>
											The email address shown as "From" in the received email.
											Make sure this email is part of your provider's
