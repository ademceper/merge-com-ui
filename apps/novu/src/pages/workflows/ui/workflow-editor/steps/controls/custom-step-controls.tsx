...
export const CustomStepControls = (props: CustomStepControlsProps) => {
	...
	const [isRestoreDefaultModalOpen, setIsRestoreDefaultModalOpen] =
		useState(false);
	const { step, workflow, update } = useWorkflow();
	const [isOverridden, setIsOverridden] = useState(
		() => Object.keys(step?.controls.values ?? {}).length > 0,
	);
									...
									<div className="flex w-full flex-col items-center gap-4">
										...
										<div className="flex flex-col items-center justify-center gap-1">
											...
											<span className="text-neutral-alpha-600 w-3/4 text-center text-xs">
												Define step controls to render fields here. This lets
												your team collaborate and ensure changes are validated
												in code.
											</span>
										</div>
									</div>
		...
		<SidebarContent size="md" className="p-0">
			<ConfirmationModal
				...
				onConfirm={async () => {
					...
					const defaultValues = buildDefaultValuesOfDataSchema(
						step.controls.dataSchema ?? {},
					);
					reset(defaultValues);
					update(
						updateStepInWorkflow(workflow, step.stepId, {
							controlValues: null,
						}),
					);
			...
			<div className="mb-3 mt-2 flex w-full items-center justify-between">
				<div className="flex flex-col justify-center gap-1">
					...
					<span className="text-xs text-neutral-400">
						Code-defined defaults are read-only by default, you can allow
						overrides using this toggle.
					</span>
				</div>
				<Switch
	...
	return (
		<motion.div layout {...fadeAnimation} className="relative min-h-10">
			{isOverridden ? (
				...
				>
					<Question className="size-4" /> Learn more about code-defined
					controls.
				</Link>
			)}
		</motion.div>
