...
import { ConditionsEditor } from "@/pages/workflows/ui/conditions-editor/conditions-editor";
import { isRelativeDateOperator } from "@/pages/workflows/ui/conditions-editor/field-type-operators";
import { updateStepInWorkflow } from "@/pages/workflows/ui/workflow-editor/step-utils";
		...
		z
			...
			.superRefine(({ field, operator, value }, ctx) => {
				...
				const isPayloadField =
					field.startsWith(PAYLOAD_FIELD_PREFIX) &&
					field.length > PAYLOAD_FIELD_PREFIX.length;
				const isSubscriberDataField =
					field.startsWith(SUBSCRIBER_DATA_FIELD_PREFIX) &&
					field.length > SUBSCRIBER_DATA_FIELD_PREFIX.length;
				const isContextField =
					field.startsWith(CONTEXT_FIELD_PREFIX) &&
					field.length > CONTEXT_FIELD_PREFIX.length;
				...
				const isValidContextField = isContextField
					? isAllowedVariableFn({ name: field })
					: false;
...
export const EditStepConditionsForm = () => {
	...
	const { variables, isAllowedVariable, enhancedVariables, namespaces } =
		useParseVariables(step?.variables, digestStepBeforeCurrent?.stepId, true);

	const isVariableAllowedInConditions = (
		variable: EnhancedLiquidVariable,
	): boolean => {
		...
		const isTopLevelNamespace = namespaces.some(
			(ns) => ns.name === variable.name,
		);
	...
	const filteredEnhancedVariables = enhancedVariables.filter(
		isVariableAllowedInConditions,
	);

	const fields = filteredEnhancedVariables.map(
		(enhancedVariable: EnhancedLiquidVariable) => ({
	...
	const form = useForm({
		...
		resolver: standardSchemaResolver(
			getConditionsSchema(fields, isAllowedVariable),
		),
		defaultValues: {
			query: query as unknown as z.infer<
				ReturnType<typeof getConditionsSchema>
			>["query"],
		},
	});
			...
			update(updateStepInWorkflow(workflow, step.stepId, updateStepData), {
				onSuccess: () => {
					...
					if (!hasConditions) {
						...
						const oldConditionsCount = countConditions(
						...
						track(TelemetryEvent.STEP_CONDITIONS_UPDATED, {
							...
							operators: uniqueOperators,
							type:
								newConditionsCount < oldConditionsCount ? "deletion" : "update",
						});
					}
				},
			});
		...
		if (stepConditionIssues && stepConditionIssues.length > 0) {
			stepConditionIssues.forEach((issue) => {
				...
				if (issue.issueType === ContentIssueEnum.MISSING_VALUE) {
					form.setError(
						`${queryPath}.value` as keyof typeof form.formState.errors,
						{
					...
					form.setError(
						`${queryPath}.field` as keyof typeof form.formState.errors,
						{
							message: issue.message,
						},
					);
