...
import { MAILY_EMAIL_WIDTH } from "@/widgets/maily-editor/maily-config";
...
export const EmailPreviewHeader = (props: EmailPreviewHeaderProps) => {
	...
	const displaySenderName =
		previewFrom?.name || fromName || senderName || "Acme Inc.";
	const displaySenderEmail =
		previewFrom?.email || fromEmail || senderEmail || "noreply@novu.co";
					...
					<div>
						{isLoading ? (
							...
							>
								...
								{onEditSenderClick && (
									<PencilSimple className="text-foreground-600 size-3.5" />
								)}
							</button>
						)}
					</div>
...
export const EmailPreviewBody = (props: EmailPreviewBodyProps) => {
	...
	const processBody = useCallback(
		(shadowRoot: ShadowRoot, bodyToProcess: string) => {
	...
	return (
		<div
			{...rest}
			className={cn(
				`bg-background mx-auto flex w-full flex-col max-w-[${MAILY_EMAIL_WIDTH}px]`,
				className,
			)}
		>
			<div
...
export const EmailPreviewContentMobile = (
	props: EmailPreviewContentMobileProps,
) => {
