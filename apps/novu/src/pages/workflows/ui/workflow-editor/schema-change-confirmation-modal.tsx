...
function VariableChangeSection({
	title,
	changes,
	icon,
}: VariableChangeSectionProps) {
							...
							<div className="flex min-w-0 flex-1 items-center gap-2">
								...
								{change.type === "typeChanged" && (
									<div className="text-text-sub flex items-center gap-1.5 text-xs">
										...
										<span>→</span>
										<span className="text-information-base">
											{change.newType}
										</span>
									</div>
								)}
								...
								{change.type === "requiredChanged" && (
									<div className="text-text-sub text-xs">
										{change.originalRequired ? "Required" : "Optional"} →{" "}
										{change.newRequired ? "Required" : "Optional"}
									</div>
								)}
							</div>
								...
								<div className="text-warning-base flex items-center gap-1.5 text-xs">
									...
									{change.usageInfo.usedInSteps.length === 1 ? (
										...
										<span
											...
											title={change.usageInfo.usedInSteps
...
export function SchemaChangeConfirmationModal({
	...
	const totalChanges =
		...
		changes.requiredChanged.length;
	...
	].filter((change) => change.usageInfo.isUsed).length;
	...
	return (
		<Dialog modal open={isOpen} onOpenChange={(open) => !open && onClose()}>
			<DialogPortal>
				<DialogOverlay />
				<DialogContent
					className="min-w-[512px] max-w-[640px] gap-4 rounded-lg p-4 overflow-hidden"
					hideCloseButton
				>
						...
						<DialogDescription className="text-foreground-600">
							...
							{usedChanges > 0 && (
								<span className="text-warning">
									{" "}
									• {usedChanges} affecting existing steps
								</span>
							)}
						</DialogDescription>
					</div>
