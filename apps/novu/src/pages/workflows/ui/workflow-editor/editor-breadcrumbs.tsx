...
export function EditorBreadcrumbs() {
	...
	const { workflow } = useFetchWorkflow({
		workflowSlug: !isNewWorkflowSlug ? workflowSlug : undefined,
	});
	...
	const isOnStepRoute =
		isOnStepEditingRoute(stepSlug, location.pathname) && step;
		...
		<div className="flex items-center gap-1">
			...
			{showSavingIndicator && (
				<SavingStatusIndicator
					isSaving={isUpdatePatchPending}
					hasError={!!lastSaveError}
				/>
			)}
		</div>
	);
							...
							{shouldShowAsPage ? (
								<BreadcrumbPage className="flex items-center gap-1">
									{isWorkflowBreadcrumb ? (
										<WorkflowBreadcrumbContent
											workflow={workflow}
											label={label}
											showSavingIndicator={!isOnStepRoute}
										/>
									) : (
										label
									)}
