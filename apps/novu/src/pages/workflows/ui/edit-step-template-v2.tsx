...
import { StepEditorLayout } from "@/pages/workflows/ui/workflow-editor/steps/step-editor-layout";
...
export function EditStepTemplateV2Page() {
	...
	const defaultValues = useMemo(
		() => (step ? getControlsDefaultValues(step) : {}),
		[step],
	);
	...
	const { onBlur, saveForm } = useFormAutosave({
		...
		save: (data, { onSuccess }) => {
			...
			const { stepResolverHash: _stripped, ...controlValues } = data as Record<
				string,
				unknown
			> & {
	...
	const hasStepControlValues =
		stepControlValues && Object.keys(stepControlValues).length > 0;
