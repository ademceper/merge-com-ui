...
export function useSyncWorkflow(
			...
			toast.error("Failed to sync workflow", {
				description:
					error instanceof Error
						? error.message
						: "There was an error syncing the workflow.",
			});
		}
	};
		...
		return showToast({
			...
			children: ({ close }) => (
				<SuccessButtonToast
					...
					description={
						<>
							Workflow <span className="font-bold">{workflow.name}</span> has
							been successfully synced to {environment?.name}.
						</>
					}
					...
					onAction={() => {
						...
						navigate({
							to: buildRoute(ROUTES.WORKFLOWS, { environmentSlug: targetSlug }),
						});
					}}
					onClose={close}
				/>
			...
			loadingToast = toast.loading(
				<>
					...
					<span className="text-sm">
						Syncing workflow <span className="font-bold">{workflow.name}</span>{" "}
						to {targetEnvironment?.name}...
					</span>
				</>,
			);
		},
		onSuccess: ({ workflow, environment }) =>
			onSyncSuccess(workflow, environment),
		onError: onSyncError,
	});
	...
	return {
		...
		PromoteConfirmModal: () => {
			const targetEnvironment = environments.find(
				(env) => env._id === targetEnvironmentId,
			);
			...
			return (
				<ConfirmationModal
					...
					description={
						<>
							Workflow{" "}
							<TruncatedText className="max-w-[32ch] font-bold">
								{workflow.name}
							</TruncatedText>{" "}
							already exists in {targetEnvironment?.name}.<br />
							<br />
