...
export function useCreateWorkflow({
	...
	const submit = async (
		...
		const isFromTemplateStore =
			template?.__source === WorkflowCreationSourceEnum.TEMPLATE_STORE;
		const hasEmailWithoutLayout = steps.some(
			(s) =>
				s.type === StepTypeEnum.EMAIL &&
				(!s.controlValues ||
					(s.controlValues as Record<string, unknown>).layoutId == null),
		);
		...
		if (isFromTemplateStore && hasEmailWithoutLayout && currentEnvironment) {
			try {
				...
				const defaultLayoutId = layouts.layouts.find(
					(l) => l.isDefault,
				)?.layoutId;
				if (defaultLayoutId) {
					steps = steps.map((s) => {
						if (s.type !== StepTypeEnum.EMAIL) return s;
						const controlValues = { ...(s.controlValues || {}) } as Record<
							string,
							unknown
						>;
						if (controlValues.layoutId == null)
							controlValues.layoutId = defaultLayoutId;
