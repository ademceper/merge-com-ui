...
export function useUploadMasterJson({
	onSuccess,
	onError,
}: UseUploadMasterJsonProps = {}) {
		...
		onSuccess: async (result) => {
			...
			if (success) {
				if (successful?.length && failed?.length) {
					...
					showSuccessToast(
						`${message} (${successful.length} resource${successful.length !== 1 ? "s" : ""})`,
					);
				} else {
				...
				if (failed?.length) {
					showErrorToast(
						`${message} (${failed.length} resource${failed.length !== 1 ? "s" : ""} failed)`,
					);
				} else {
...
function getImportSummary(result: ImportMasterJsonResponseDto) {
