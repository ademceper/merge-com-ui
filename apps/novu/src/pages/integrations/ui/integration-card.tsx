...
export function IntegrationCard({
	...
	const handleConfigureClick = (e: React.MouseEvent<HTMLElement>) => {
		if (
			...
			navigate({ to: `${ROUTES.INBOX_EMBED}?environmentId=${environment._id}` });
		} else {
			onClick({
		...
		>
				...
				<div className="flex items-center gap-1">
					{integration.primary && (
						<Tooltip>
							...
							<TooltipContent>
								This is your primary integration for the {provider.channel}{" "}
								channel.
							</TooltipContent>
						</Tooltip>
					)}
			...
			<div className="flex items-center gap-2">
				{isDemo && (
					<Tooltip>
						...
						<TooltipContent>
							<p>
								This is a demo provider for testing purposes only and capped at
								300 {provider.channel === "email" ? "emails" : "sms"} per month.
			...
			<div className="mt-auto flex items-center gap-2">
				...
				!integration.connected ? (
					...
					<StatusBadge
						variant="light"
						status={integration.active ? "completed" : "disabled"}
					>
