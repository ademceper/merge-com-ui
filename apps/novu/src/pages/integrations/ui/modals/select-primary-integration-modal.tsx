...
type SelectPrimaryIntegrationModalProps = {
	isOpen: boolean;
...
export function SelectPrimaryIntegrationModal({
	...
	const description =
		mode === "switch" ? (
			<>
				<p>
					This will change the primary integration from{" "}
					<span className="font-medium">{currentPrimaryName}</span> to{" "}
					<span className="font-medium">{newPrimaryName}</span>.
				</p>
				<p>
					The current primary integration will be disabled and all future
					notifications will be sent through the new primary integration.
				</p>
			</>
		) : (
			<>
				...
				<p>
					All future notifications will be sent through the selected
					integration.
				</p>
				<div className="mt-4">
					<Select
	...
	return (
		<ConfirmationModal
			...
			onConfirm={() =>
				onConfirm(mode === "select" ? selectedIntegrationId : undefined)
			}
			title={
				mode === "switch"
					? "Change Primary Integration"
					: "Select Primary Integration"
			}
