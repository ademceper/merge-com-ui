...
export function IntegrationSettings({
	...
	const isSlackTeamsEnabled = useFeatureFlag(
		FeatureFlagsKeysEnum.IS_SLACK_TEAMS_ENABLED,
		false,
	);
			...
			>
				<div className="flex items-center justify-between gap-2 p-3">
					...
					<div
						className={cn(
							"w-full",
							mode === "update" ? "max-w-[160px]" : "max-w-[260px]",
						)}
					>
						<EnvironmentDropdown
						...
						<AccordionContent>
							<GeneralSettings
								...
								onAutoConfigureSuccess={(updatedIntegration) => {
									...
									setValue(
										"configurations",
										updatedIntegration.configurations as Record<string, string>,
									);
									setValue(
										"credentials",
										updatedIntegration.credentials as Record<string, string>,
									);
              ...
              we need to replace it with dedicated view per integration channel */}
						...
						!integration.connected ? (
							<InlineToast
								...
								onCtaClick={() =>
									navigate({
										to: `${ROUTES.INBOX_EMBED}?environmentId=${integration._environmentId}`,
									})
								}
							/>
						) : (
