...
import { useAutoConfigureIntegration } from "@/pages/integrations/lib/use-auto-configure-integration";
...
export function InboundWebhookGroup({
	...
	const { mutateAsync: autoConfigureIntegration } =
		useAutoConfigureIntegration();
	...
	const [autoConfigureState, setAutoConfigureState] = useState<
		"idle" | "loading" | "success" | "error"
	>("idle");
	const [autoConfigureMessage, setAutoConfigureMessage] = useState<string>("");

	// Find the enabler configuration (toggle field)
	const enablerConfig = enabler
		? configurations.find((config) => config.key === enabler)
		: null;
	const nonEnablerConfigs = configurations.filter(
		(config) => config.key !== enabler,
	);
	...
	useEffect(() => {
		const handleIntegrationCreationOrUpdate = async () => {
			...
			const wasToggleJustEnabled =
				prevIsEnabledRef.current === false && isEnabled === true;
				...
				try {
					...
					if (response.success) {
						...
						setAutoConfigureMessage(
							response.message || "Configuration completed successfully",
						);
				...
				<>
					<CredentialSection
						...
						disabledSwitchMessage={
							!integrationId
								? "To enable Email activity tracking, create the integration first"
								: undefined
						}
