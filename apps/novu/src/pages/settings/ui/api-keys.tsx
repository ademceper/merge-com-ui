...
} from "@/pages/settings/api/use-fetch-api-keys";
...
export function ApiKeysPage() {
	...
	const canRegenerateApiKeys = has({
		permission: PermissionsEnum.API_KEY_WRITE,
	});
	...
	const region =
		getRegionConfig(selectedRegion)?.name || selectedRegion.toUpperCase();

	return (
		<>
			<PageMeta
				title={`API Keys for ${currentEnvironment?.name} environment`}
			/>
					...
					<Card className="w-full overflow-hidden shadow-none">
						<CardHeader>
							...
							<p className="text-foreground-500 mt-1 text-xs font-normal">
								...
								<ExternalLink
									href="https://docs.novu.co/platform/inbox/overview"
					...
					<Card className="w-full overflow-hidden shadow-none">
						<CardHeader>
							...
							<p className="text-foreground-500 mt-1 text-xs font-normal">
								...
								<ExternalLink
									href="https://docs.novu.co/platform/sdks/overview"
									className="text-foreground-500"
								>
									Learn more
								</ExternalLink>
				...
				{isLoading ? (
					...
					<>
						<Input
							...
							value={
								secret ? (showSecret ? value : maskSecret(value ?? "")) : value
							}
							readOnly={readOnly}
