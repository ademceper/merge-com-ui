...
import { useCreateEnvironment } from "@/pages/settings/model/use-environments";
...
function getRandomColor(existingEnvironments: IEnvironment[] = []) {
	const usedColors = new Set(
		existingEnvironments.map((env) => (env as any).color).filter(Boolean),
	);
	const availableColors = ENVIRONMENT_COLORS.filter(
		(color) => !usedColors.has(color),
	);

	// If all colors are used, fall back to the original list
	const colorPool =
		availableColors.length > 0 ? availableColors : ENVIRONMENT_COLORS;
