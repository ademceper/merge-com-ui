...
export const VercelIntegrationPage = () => {
	...
	const { isPending: isCreateVercelIntegrationPending, data } =
		useCreateVercelIntegration();
	const {
		data: vercelIntegration,
		isLoading: isFetchVercelIntegrationLoading,
	} = useFetchVercelIntegration({
		configurationId,
		options: {
			enabled: !!configurationId && isEditMode && !!currentEnvironment,
		},
	});
	const {
		data: vercelIntegrationProjects,
		isLoading: isFetchVercelIntegrationProjectsLoading,
				...
				<Card className="max-w-[700px] overflow-hidden shadow-none">
					...
					<CardContent className="h-fit rounded-b-xl border-t bg-neutral-50 bg-white p-4">
						<p className="text-foreground-500 mb-6 mt-1 text-xs font-normal">
							Choose the projects to link with your organizations. This action
							will perform a sync of the projects with your Novu environments as
							their bridge url.
						</p>
				...
				<CardContent className="h-fit rounded-b-xl border-t bg-neutral-50 bg-white p-4">
					...
					<VercelIntegrationForm
						...
						currentOrganizationId={
							organization.publicMetadata.externalOrgId as string
						}
						projects={projects}
