...
import { useTopic } from "@/pages/topics/lib/use-topic";
import { useTopicSubscriptions } from "@/pages/topics/lib/use-topic-subscribers";
...
import {
	TopicOverviewForm,
	TopicOverviewSkeleton,
} from "./topic-overview-form";
...
const TopicSubscribers = (props: TopicSubscribersProps) => {
	...
	return (
		...
		>
			...
			>
				{!readOnly && (
					<AddSubscriberForm topicKey={topicKey} contextKeys={contextKeys} />
				)}
			</div>
			<div
				...
				>
					{Array.from({ length: 5 }).map((_, index) => (
						<motion.div
							...
							className="border-b-stroke-soft flex w-full border-b"
						>
			...
			>
				<header className="border-bg-soft flex h-12 w-full flex-row items-center gap-3 border-b px-3 py-4">
					<div className="flex flex-1 items-center gap-1 overflow-hidden text-sm font-medium">
						...
						<TruncatedText className="flex-1 pr-10">
							Topic - {topicKey}
						</TruncatedText>
					</div>
				</header>
				...
				>
					...
					<TabsTrigger
						value="subscribers"
						className={cn(tabTriggerClasses, "flex items-center")}
					>
						Subscriptions
						{subscriptionCount && (
...
const TopicListBlank = () => {
	return (
		<div className="mt-[100px] flex h-full w-full flex-col items-center justify-center gap-6">
			...
			<div className="flex flex-col items-center gap-2 text-center">
				<p className="text-text-soft text-paragraph-sm max-w-[60ch]">
					No subscribers added yet, Add subscribers via the API or manually to
...
export const TopicDrawer = forwardRef<HTMLDivElement, TopicDrawerProps>(
	(props, forwardedRef) => {
