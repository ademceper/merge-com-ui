...
import { EditContextFormSchema } from "@/pages/contexts/ui/schema";
import { useContextsNavigate } from "@/pages/contexts/model/use-contexts-navigate";
import { useDeleteContext } from "@/pages/contexts/api/use-delete-context";
import { useFetchContext } from "@/pages/contexts/api/use-fetch-context";
import { useUpdateContext } from "@/pages/contexts/api/use-update-context";
	...
	const onSubmit = async (formData: z.infer<typeof EditContextFormSchema>) => {
		...
		try {
			...
			await updateContext({
				...
				id: context.id,
				data:
					parsedData && Object.keys(parsedData).length > 0 ? parsedData : {},
			});
		} catch {
