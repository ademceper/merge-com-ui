...
import {
	StepIdEnum,
	useOnboardingSteps,
} from "@/pages/onboarding/lib/use-onboarding-steps";
...
import {
	...
	stepsList,
	textItem,
} from "./progress-section.animations";
			...
			>
				...
				<motion.div
					className={cn(
						...
						isNewHomePageEnabled ? "p-3 pt-0" : "p-6",
					)}
					variants={stepsList}
				>
...
function HomePageHeader() {
	return (
		<motion.div variants={leftSection} className="p-3">
			<h2 className="text-label-xs text-text-strong">
				You're doing great work! 💪
			</h2>
			<p className="text-label-xs text-text-soft">
				Set up Novu to send notifications your users will love.
			</p>
		</motion.div>
	);
}
...
function StepItem({ step, environmentSlug }: StepItemProps) {
	...
	return (
		<motion.div
			className="flex max-w-[370px] items-center gap-1.5"
			variants={stepItem}
		>
			<div
			...
			>
				...
				>
					<CardContent className="flex flex-col rounded-[6px] bg-[#FBFBFB] px-2 py-1.5">
						...
						<p className="text-foreground-400 text-[10px] leading-[14px]">
							{step.description}
						</p>
					</CardContent>
				</Card>
	...
	return (
		...
		>
			<div className="flex w-full flex-col gap-2">
				<motion.h2
					variants={textItem}
					className="font-label-medium text-base font-medium"
				>
					You're doing great work! 💪
				</motion.h2>
				...
				<div className="text-foreground-400 flex flex-col gap-6 text-sm">
					...
					<motion.p variants={textItem}>
						Streamline all your customer messaging in one tool and delight them
						at every touchpoint.
					</motion.p>
				</div>
			</div>
			...
			<motion.div
				variants={textItem}
				className="space-between flex items-center gap-0"
			>
