...
import { useFirstTriggerDetection } from "@/pages/workflows/lib/use-first-trigger-detection";
import { useInitDemoWorkflow } from "@/pages/workflows/lib/use-init-demo-workflow";
...
function WorkflowIntegrationSteps({
	userId,
	apiKey,
}: {
	userId: string;
	...
	const curl = useMemo(
		() => generateCurlSnippet(userId, apiKey),
		[userId, apiKey],
	);
...
export function InboxConnectedGuide({
	subscriberId,
	environment,
}: InboxConnectedGuideProps) {
	...
	const hasValidApiKey =
		!apiKeysQuery.isLoading && !apiKeysQuery.error && apiKey;
	...
	useEffect(() => {
		if (isTriggerDetectionError && triggerDetectionError) {
			...
			showErrorToast(
				"Failed to detect API trigger. Please refresh the page and try again.",
				"Detection Error",
			);
		}
	...
	async function handleCompleteOnboarding() {
		...
		try {
			...
			navigate({ to: ROUTES.INBOX_EMBED_SUCCESS });
		} catch (error) {
			console.error("Failed to navigate after onboarding completion:", error);
			showErrorToast(
				"Failed to complete onboarding. Please try refreshing the page.",
				"Navigation Error",
			);
		}
	}
							...
							<div className="flex flex-col gap-3">
								<div className="flex items-center gap-2">
									<span className="text-success text-sm font-medium">
										In-App Channel Integration Activated
									</span>
								</div>
								<p className="text-foreground-400 text-xs">
									You've initialized your Inbox. The last step is to make an API
