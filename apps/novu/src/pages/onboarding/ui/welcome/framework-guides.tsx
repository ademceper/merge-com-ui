...
import type {
	Framework,
	InstallationStep,
} from "./framework-guides.instructions";
...
function StepContent({
	...
	const handlePackageManagerChange = (value: string) => {
		track(TelemetryEvent.INBOX_CUSTOMIZATION_CHANGED, {
			...
			switch (packageManager) {
				case "pnpm":
					return code.replace(
						...
						"pnpm dlx add-inbox@latest",
					);
				case "yarn":
					return code.replace(
						"npx add-inbox@latest",
						"yarn dlx add-inbox@latest",
					);
				default:
					return code;
			}
		...
		<motion.div {...codeBlockAnimation(index)} className="w-full max-w-[500px]">
			<div className="flex flex-col gap-3">
				...
				>
					...
					>
						...
						<span className="px-1 w-[98px] h-4 flex flex-row justify-center items-center flex-none order-1">
							<span className="w-[90px] h-4 flex items-center flex-none order-0">
								{buttonText}
							</span>
						</span>
					</div>
				...
				<p className="text-foreground-400 text-xs">
					(No terminal, no docs — just let your pair programmer handle it.)
				</p>
			</div>
		</motion.div>
	);
...
export function FrameworkCliInstructions({
	framework,
}: {
	framework: Framework;
}) {
