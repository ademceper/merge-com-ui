...
import {
	type SearchField,
	useSubscriberSearch,
} from "@/pages/subscribers/lib/use-subscriber-search";
...
export function SubscriberAutocomplete({
	...
	// Core state
	const [internalSearchField, setInternalSearchField] =
		useState<SearchField>("subscriberId");
	...
	// Get search results
	const { subscribers, isLoading, hasSearched } = useSubscriberSearch(
		value,
		searchField,
	);
	...
	const subscriberItems: SubscriberAutocompleteItem[] = subscribers.map(
		(subscriber) => ({
			...subscriber,
	...
	return (
		<Autocomplete
			...
			onSelectItem={(item) => {
				const originalSubscriber = subscribers.find(
					(s) => s.subscriberId === item.id,
				);
