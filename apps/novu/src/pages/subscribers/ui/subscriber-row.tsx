...
import { getSubscriberTitle } from "@/pages/subscribers/ui/utils";
...
export const SubscriberRow = ({
	subscriber,
	subscribersCount,
	firstTwoSubscribersInternalIds,
}: SubscriberRowProps) => {
	const { currentEnvironment } = useEnvironment();
	const [isDeleteModalOpen, setIsDeleteModalOpen] = useState(false);
	const subscriberTitle = getSubscriberTitle(subscriber);
	const queryClient = useQueryClient();
	const { navigateToSubscribersFirstPage, navigateToEditSubscriberPage } =
					...
					children: () => (
						<>
							...
							<span className="text-sm">
								Deleted subscriber{" "}
								<span className="font-bold">{subscriberTitle}</span>.
							</span>
						</>
					),
	...
	const handleDeletion = async () => {
		...
		const hasTwoSubscribersInternalIds =
			firstTwoSubscribersInternalIds.length === 2 && !hasSingleSubscriber;
		const firstSubscriberInternalId = firstTwoSubscribersInternalIds[0];
		const isFirstSubscriberBeingDeleted =
			subscriber._id === firstSubscriberInternalId;
	...
	return (
		<>
			...
			>
				<SubscriberTableCell>
					<div className="flex items-center gap-3">
						...
						<div className="flex flex-col">
							<TruncatedText className="text-text-strong max-w-[36ch] font-medium">
								{subscriberTitle}
							</TruncatedText>
				...
				<SubscriberTableCell>
					<TruncatedText className="relative z-10 max-w-[28ch]">
						{subscriber.email || "-"}
					</TruncatedText>
				</SubscriberTableCell>
			...
			<ConfirmationModal
				...
				description={
					<span>
						...
						<TruncatedText className="max-w-[20ch] font-bold">
							{subscriberTitle}
						</TruncatedText>
