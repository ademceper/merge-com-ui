...
const SubscriptionOverview = ({
	...
	return (
		...
		>
			<span className="text-text-soft font-code shrink-0 text-xs font-medium">
				{label}
			</span>
			<div className="relative flex min-w-0 items-center gap-2 overflow-hidden">
				{isCopyable && value && (
					<CopyButton
						valueToCopy={value}
						size="2xs"
						className="h-1 shrink-0 p-0.5"
					/>
				)}
				<span
					className="text-foreground-600 truncate font-mono text-xs"
					title={value}
				>
					{children ?? value}
				</span>
	...
	return (
		<>
				...
				<div className="flex min-h-0 flex-1 flex-col">
					...
					<div className="flex flex-col gap-2 p-3 overflow-auto">
						{subscription.preferences.map((preference) => (
							<SubscriptionPreferenceRule
								key={preference.workflow.id}
			...
			<TopicDrawer
				...
				onOpenChange={setOpenTopicDrawer}
				topicKey={topicKey}
				readOnly
			/>
		</>
