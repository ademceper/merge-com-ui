...
import { useDeleteSubscription } from "@/pages/subscribers/api/use-delete-subscription";
import { useFetchSubscriberSubscriptions } from "@/pages/subscribers/api/use-fetch-subscriber-subscriptions";
import { TopicDrawer } from "@/pages/topics/ui/topic-drawer";
...
export function SubscriberSubscriptions({
	...
	const [selectedSubscription, setSelectedSubscription] =
		useState<TopicSubscription | null>(null);
	const [isDeleteModalOpen, setIsDeleteModalOpen] = useState(false);
	const [isTopicDrawerOpen, setIsTopicDrawerOpen] = useState(false);
	const [
		isSubscriptionPreferencesDrawerOpen,
		setIsSubscriptionPreferencesDrawerOpen,
	] = useState(false);
	...
	const handleConfirmDeleteSubscription = async () => {
		...
		await deleteSubscription(
			...
			},
			{
				...
				onError: (error) => {
					showErrorToast(
						`Error deleting subscription: ${(error as Error).message}`,
					);
				},
			},
		);
	...
	const handleViewSubscriptionPreferences = (
		...
		<>
			...
			>
				{isContextPreferencesEnabled && (
					<SidebarContent
						size="md"
						className="min-h-max overflow-x-auto border-b border-neutral-200 py-2 px-2"
					>
							...
							<motion.div key={subscription._id} variants={itemVariants}>
								<SubscriptionItem
									...
									onViewSubscriptionPreferences={
										handleViewSubscriptionPreferences
									}
								/>
							</motion.div>
						))}
