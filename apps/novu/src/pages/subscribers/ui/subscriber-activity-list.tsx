...
import { ActivityEmptyState } from "@/pages/activity/ui/activity-empty-state";
import { JOB_STATUS_CONFIG } from "@/pages/activity/ui/constants";
import { getActivityStatus } from "@/pages/activity/ui/helpers";
...
const statusToTooltipStyles: Record<string, string> = {
	completed:
		"before:bg-success-lighter before:border before:border-success-lighter text-success-base",
	pending:
		"before:bg-warning-lighter before:border before:border-warning-lighter text-warning-base",
	failed:
		"before:bg-error-lighter before:border before:border-error-lighter text-error-base",
	disabled:
		"before:bg-faded-lighter before:border before:border-faded-light text-faded-base",
};
	...
	return (
		...
		>
			{activities.map((activity) => {
				...
				JOB_STATUS_CONFIG[JobStatusEnum.PENDING];
							...
							<div className="flex w-full flex-col gap-0.5">
								<TruncatedText
									className={cn("text-label-xs", {
										"text-foreground-400": !activity.template,
									})}
								>
									{activity.template?.name ?? "Deleted workflow"}
								</TruncatedText>
								<TruncatedText className="text-paragraph-2xs text-text-soft">
									{activity.template?.triggers
										.map((trigger) => trigger.identifier)
										.join(", ")}
								</TruncatedText>
							</div>
						</div>
