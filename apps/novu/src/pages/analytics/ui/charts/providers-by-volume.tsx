...
import { ProviderIcon } from "@/pages/integrations/ui/provider-icon";
...
function ProviderVolumeTooltip(props: ProviderVolumeTooltipProps) {
	...
	return (
		<NovuTooltip
			active={props.active}
			label={data.displayName || data.providerId}
			rows={rows}
			showTotal={false}
		/>
	);
...
function CustomTick({
	...
	const formattedText =
		payload.value === "novu" ? "Novu Inbox" : formatProviderName(payload.value);
	const text =
...
export function ProvidersByVolume({ data, isLoading }: ProvidersByVolumeProps) {
	...
	const chartData = useMemo(() => {
		return data?.map((dataPoint, index) => {
			const formattedName =
				dataPoint.providerId === "novu"
					? "Novu Inbox"
					: formatProviderName(dataPoint.providerId);
			return {
	...
	const hasDataChecker = useCallback(
		createVolumeBasedHasDataChecker<ProviderChartData>(
			(dataPoint: ProviderChartData) => {
