...
function WorkflowVolumeTooltip(props: WorkflowVolumeTooltipProps) {
	...
	return (
		<NovuTooltip
			active={props.active}
			label={data.displayName || data.workflowName}
			rows={rows}
			showTotal={false}
		/>
	);
...
export function WorkflowsByVolume({ data, isLoading }: WorkflowsByVolumeProps) {
		...
		return data?.map((dataPoint, index) => ({
			...
			displayName:
				dataPoint.workflowName.length > 20
					? `${dataPoint.workflowName.substring(0, 20)}...`.replace(
							/\b\w/g,
							(char: string) => char.toUpperCase(),
						)
					: dataPoint.workflowName.replace(/\b\w/g, (char: string) =>
							char.toUpperCase(),
						),
	...
	const hasDataChecker = useCallback(
		createVolumeBasedHasDataChecker<WorkflowChartData>(
			(dataPoint: WorkflowChartData) => {
