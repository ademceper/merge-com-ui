...
import { uploadTranslations } from "@/entities/translation/api/translations";
import {
	showErrorToast,
	showSuccessToast,
	showWarningToast,
...
type UploadTranslationsParameters = OmitEnvironmentFromParameters<
	typeof uploadTranslations
>;
	...
	return useMutation({
		...
		onSuccess: async (data, variables) => {
			...
			if (data.failedUploads > 0) {
				...
				showSuccessToast(
					`All ${data.totalFiles} translation files uploaded successfully`,
				);
			}
