...
function TranslationStatusSection({
	updatedAt,
	outdatedLocales,
}: TranslationStatusSectionProps) {
	return (
		<div className="flex flex-col items-start gap-3 self-stretch border-b border-neutral-100 p-4">
			<div className="flex w-full items-center justify-between">
			...
			<div className="flex w-full items-center justify-between">
				...
				<TimeDisplayHoverCard
					date={updatedAt}
					className="font-code text-xs text-neutral-400"
				>
					...
					<Tooltip>
						...
						<TooltipContent>
							<span className="text-xs">
								Some keys in this target language don't match the default
								language. Add missing keys or remove extra ones to sync
								translations.
							</span>
						</TooltipContent>
					</Tooltip>
...
export function LocaleList({
	...
	const sortedLocales = useMemo(() => {
		...
		return [
			actualDefaultLocale,
			...locales.filter((locale) => locale !== actualDefaultLocale),
		];
