			...
			<div className="flex items-center gap-1 text-paragraph-2xs text-text-soft">
				...
				<Tooltip>
					...
					<TooltipContent className="max-w-xs">
						Live environments are read only. Use them for staging, QA, previews.
						Great for safe reviews and testing!
					</TooltipContent>
				</Tooltip>
			</div>
		</TableCell>
...
export function EnvironmentsList({
	...
	const { mutateAsync: deleteEnvironmentAction, isPending: isDeletePending } =
		useDeleteEnvironment();

	const groupedEnvironments = useMemo(() => {
		const devEnvironments = environments.filter(
			(env) => env.type === EnvironmentTypeEnum.DEV,
		);
		const liveEnvironments = environments.filter(
			(env) => env.type === EnvironmentTypeEnum.PROD,
		);
	...
	const renderEnvironmentRow = (environment: IEnvironment) => (
		<TableRow key={environment._id} className="group relative isolate">
			...
			<TableCell
				className={cn("text-foreground-600 min-w-[180px] text-sm font-medium")}
			>
						...
						<>
							...
							{groupedEnvironments.liveEnvironments.length > 0 && (
								<>
									...
									{groupedEnvironments.liveEnvironments.map(
