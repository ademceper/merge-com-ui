...
export const DayScheduleCopy = ({
	day,
	schedule,
	disabled,
	onScheduleUpdate,
}: DayScheduleCopyProps) => {
	const [isOpen, setIsOpen] = useState<boolean>(false);
	const [selectedDays, setSelectedDays] = useState<Array<keyof WeeklySchedule>>(
		[day],
	);
	const [isAllSelected, setIsAllSelected] = useState<boolean>(false);
	const allWeekDaysSelected = useMemo(
									...
									onCheckedChange={(value) =>
										setSelectedDays(
											value
												...
												: selectedDays.filter((d) => d !== weekDay),
										)
									}
						...
						<div className="flex justify-end border-t border-neutral-alpha-100 pt-2">
							<Button
								onClick={async () => {
									...
									const daysToCopy = selectedDays.filter(
										(day) => day !== currentDay,
									);
