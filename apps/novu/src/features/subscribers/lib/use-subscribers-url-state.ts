...
export function useSubscribersUrlState(
	...
	const filterValues = useMemo(
		() => ({
			...
			limit: parseInt(
				searchParams.get("limit") || defaultSubscribersFilter.limit.toString(),
				10,
			),
			after: searchParams.get("after") || "",
			before: searchParams.get("before") || "",
			orderBy:
				(searchParams.get("orderBy") as SubscribersSortableColumn) ||
				defaultSubscribersFilter.orderBy,
			orderDirection:
				(searchParams.get("orderDirection") as DirectionEnum) ||
				DirectionEnum.DESC,
	...
	const handleNavigationAfterDelete = (afterCursor: string) => {
		...
		if (isUnderSubscribersPage) {
			navigate(
				`${buildRoute(ROUTES.SUBSCRIBERS, { environmentSlug: currentEnvironment?.slug ?? "" })}?${newParams}`,
				{
					replace: true,
				},
			);
