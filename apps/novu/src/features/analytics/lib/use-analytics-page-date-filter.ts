...
function buildDateFilterOptions({
	...
	return HOME_PAGE_DATE_RANGE_OPTIONS.map((option) => {
		...
		return {
			...
			label:
				exceedsAnalyticsLimit && !exceedsRetentionLimit
					? `${option.label} (Coming soon)`
					: option.label,
			value: option.value,
			disabledDueToAnalyticsLimit:
				exceedsAnalyticsLimit && !exceedsRetentionLimit,
		};
	});
}
...
function getDefaultDateRange({
	...
	const fallbackOption = availableFilters.find(
		(option) => !option.disabled && !option.disabledDueToAnalyticsLimit,
	);
	if (fallbackOption) {
...
function getChartsDateRange(selectedDateRange: string) {
	const rangeMs =
		HOME_PAGE_DATE_RANGE_OPTIONS.find(
...
export function useHomepageDateFilter({
	organization,
	subscription,
	upgradeCtaIcon,
}: UseHomepageDateFilterParams) {
	...
	const maxDateAnalyticsDays = useNumericFeatureFlag(
		FeatureFlagsKeysEnum.MAX_DATE_ANALYTICS_ENABLED_NUMBER,
		0,
	);
	const maxDateAnalyticsMs =
		maxDateAnalyticsDays > 0 ? maxDateAnalyticsDays * 24 * 60 * 60 * 1000 : 0;
	...
	const [selectedDateRange, setSelectedDateRange] =
		useState<string>(defaultDateRange);
	...
	const dateFilterOptions = useMemo(() => {
		...
		return buildDateFilterOptions({
			...
			...option,
			icon:
				option.disabled && !option.disabledDueToAnalyticsLimit
					? upgradeCtaIcon
					: undefined,
		}));
	...
	const chartsDateRange = useMemo(
		() => getChartsDateRange(selectedDateRange),
		[selectedDateRange],
	);
