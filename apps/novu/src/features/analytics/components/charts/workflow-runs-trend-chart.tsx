		...
		<div className="h-[160px] w-full relative px-4">
			<div className="absolute inset-0 flex items-end justify-between px-2">
				{Array.from({ length: 35 }).map((_, i) => {
					const baseHeight = 40;
					const successHeight =
						baseHeight + Math.sin(i * 0.3) * 30 + Math.random() * 20;
...
function LegacyWorkflowRunsTrendChart({
	data,
	isLoading,
	error,
}: WorkflowRunsTrendChartProps) {
	...
	const hasDataChecker = useCallback(
		createDateBasedHasDataChecker<WorkflowRunsChartData>(
			(dataPoint: WorkflowRunsChartData) =>
				(dataPoint.completed || 0) > 0 ||
				(dataPoint.processing || 0) > 0 ||
				(dataPoint.error || 0) > 0,
		),
		[],
	);
	...
	const renderChart = useCallback(
		(chartDataToRender: WorkflowRunsChartData[], includeTooltip = true) => {
			return (
				<ChartContainer config={legacyChartConfig} className="h-[160px] w-full">
					<LineChart accessibilityLayer data={chartDataToRender}>
						...
						{includeTooltip && (
							<ChartTooltip
								cursor={false}
								content={<NovuTooltip showTotal={false} />}
							/>
						)}
						<Line
							dataKey="completed"
...
function FinalStatusWorkflowRunsTrendChart({
	data,
	isLoading,
	error,
}: WorkflowRunsTrendChartProps) {
	...
	const hasDataChecker = useCallback(
		createDateBasedHasDataChecker<WorkflowRunsChartData>(
			(dataPoint: WorkflowRunsChartData) =>
				(dataPoint.completed || 0) > 0 || (dataPoint.error || 0) > 0,
		),
		[],
	);
...
export function WorkflowRunsTrendChart(props: WorkflowRunsTrendChartProps) {
	const isFinalStatusOnly = useFeatureFlag(
		FeatureFlagsKeysEnum.IS_WORKFLOW_RUN_TREND_FROM_TRACES_ENABLED,
	);
