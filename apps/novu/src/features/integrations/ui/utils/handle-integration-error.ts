...
export function handleIntegrationError(
	...
	if (error?.message?.code === CheckIntegrationResponseEnum.INVALID_EMAIL) {
		...
		error?.message?.code === CheckIntegrationResponseEnum.BAD_CREDENTIALS
	) {
		showErrorToast(
			error.message?.message,
			"Invalid credentials or credentials expired",
		);
	} else {
		...
		showErrorToast(
			error?.message?.message ||
				error?.message ||
				`There was an error ${operation}ing the integration.`,
