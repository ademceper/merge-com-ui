					...
					<SelectTrigger>
						<SelectValue
							placeholder={
								...
								`Select ${credential.displayName.toLowerCase()}`
							}
						/>
					</SelectTrigger>
					<SelectContent>
		...
		<FormItem className="mb-2" key={credential.key}>
			<div className="space-y-3">
				...
				<InlineToast
					...
					onCtaClick={() => {
						window.open(
							"https://docs.novu.co/platform/integrations/push/push-activity-tracking",
							"_blank",
						);
					}}
				/>
...
function InputControl({
	...
	if (credential.type === "pushResources") {
		return (
			<PushResources credential={credential} integrationId={integrationId} />
		);
	}
		...
		<FormField
			...
			rules={{
				required: credential.required
					? `${credential.displayName} is required`
					: false,
					...
					<InputControl
						...
						tooltip={
							credential.tooltip?.text ? (
								<DescriptionWithLinks
									description={credential.tooltip?.text}
									links={credential.links}
								/>
							) : undefined
						}
