...
export function useAutoConfigureIntegration() {
	...
	return useMutation<
		AutoConfigureIntegrationResponse,
		Error,
		AutoConfigureIntegrationVariables
	>({
		...
		onSuccess: (data) => {
			if (data.success) {
				...
				showErrorToast(
					data.message || "Auto-configuration failed",
