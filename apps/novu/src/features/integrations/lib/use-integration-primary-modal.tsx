...
import type { IntegrationFormData } from "../components/types";
import { handleIntegrationError } from "../components/utils/handle-integration-error";
...
type UseIntegrationPrimaryModalProps = {
	...
	mode: "create" | "update";
	setPrimaryIntegration?: UseMutateAsyncFunction<
		unknown,
		Error,
		SetPrimaryIntegrationParams
	>;
};
...
export function useIntegrationPrimaryModal({
	...
	const [pendingData, setPendingData] = useState<IntegrationFormData | null>(
		null,
	);
	...
	const isChannelSupportPrimary =
		CHANNELS_WITH_PRIMARY.includes(currentChannel);
	...
	const existingPrimaryIntegration = filteredIntegrations.find(
		(el) => el.primary,
	);
	const hasOtherProviders = filteredIntegrations.length;
	const hasSameChannelActiveIntegration = filteredIntegrations.find(
		(el) => el.active,
	);
