...
const PROVIDER_TYPE_PRIORITIES: ChannelTypeEnum[] = [
...
function isActiveIntegration(
	...
	const isConnected =
		providerType === ChannelTypeEnum.IN_APP ? !!integration.connected : true;
...
export function useOnboardingSteps(): OnboardingStepsResult {
	...
	const hasCreatedWorkflow = useMemo(() => {
		return (
			(workflows?.data?.workflows ?? []).filter(
				(workflow) => workflow.workflowId !== ONBOARDING_DEMO_WORKFLOW_ID,
			).length > 0
		);
	...
	const providerType = useMemo(() => {
		...
		return (
			PROVIDER_TYPE_PRIORITIES.find((type) => useCases.includes(type)) ??
			useCases[0]
		);
	...
	const steps = useMemo((): Step[] => {
		const allSteps: Step[] = [
			{
				...
				description:
					"We know it's not always easy — take a moment to celebrate!",
				status: "completed",
			},
			{
				...
				description:
					"Workflows in Novu, orchestrate notifications across channels.",
			...
			},
			{
				...
				status: integrations?.some((integration) =>
					isActiveIntegration(integration, providerType),
				)
					? "completed"
					: "pending",
			},
		...
		if (IS_SELF_HOSTED) {
			return allSteps.filter(
				(step) => step.id !== StepIdEnum.INVITE_TEAM_MEMBER,
			);
		}
