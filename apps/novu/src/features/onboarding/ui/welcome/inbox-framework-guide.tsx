...
const FRAMEWORKS_WITH_MANUAL_ONLY = [
	"Remix",
	"Native",
	"Angular",
	"JavaScript",
...
export function InboxFrameworkGuide({
	...
	const [selectedFrameworkName, setSelectedFrameworkName] = useState<string>(
		() => {
			return (
				frameworks.find((f) => f.selected)?.name ?? frameworks[0]?.name ?? ""
			);
		},
	);
	...
	const effectiveInstallationMethod = useMemo<InstallationMethod>(
		() =>
			FRAMEWORKS_WITH_MANUAL_ONLY.includes(selectedFrameworkName)
				? "manual"
				: installationMethod,
	...
	const updatedFrameworks = useMemo(() => {
		if (!currentEnvironment?.identifier || !subscriberId)
	...
	const selectedFramework = useMemo(
		() =>
			updatedFrameworks.find((f) => f.name === selectedFrameworkName) ||
			updatedFrameworks[0],
	...
	const handleInstallationMethodChange = useCallback(
