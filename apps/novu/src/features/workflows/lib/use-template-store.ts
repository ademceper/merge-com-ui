...
function normalizeStepType(input: unknown): StepTypeEnum {
	if (typeof input === "string") {
		...
		if (StepTypeEnum[upper])
			return StepTypeEnum[upper as keyof typeof StepTypeEnum];
	}

	return StepTypeEnum.IN_APP;
}

function normalizeControlValuesForType(
	type: StepTypeEnum,
	...
	const coalesceText = (...keys: string[]) => {
		for (const key of keys) {
			if (key in nextValues && nextValues[key] != null)
				return nextValues[key] as unknown;
		}
		return undefined;
	};
	...
	if (type === StepTypeEnum.EMAIL) {
		...
		if ("layoutId" in nextValues)
			delete (nextValues as { layoutId?: unknown }).layoutId;
		if ("layout" in nextValues)
			delete (nextValues as { layout?: unknown }).layout;
...
function mapApiWorkflowsToQuickTemplates(items: unknown[]): QuickTemplate[] {
	...
	return (Array.isArray(items) ? items : []).map((rawItem) => {
		...
		const workflowId =
			...
			rawWorkflow._id ||
			"";
		const rawSteps = Array.isArray(rawWorkflow.steps)
			? (rawWorkflow.steps as Array<{ type?: unknown }>)
			: [];
		const steps = rawSteps.map((rawStep) =>
			normalizeStepType(rawStep?.type as unknown),
		);
...
function mapApiWorkflowsToSuggestions(items: unknown[]): IWorkflowSuggestion[] {
	...
	return (Array.isArray(items) ? items : []).map((rawItem) => {
		...
		const steps: StepCreateDto[] = rawSteps.map((rawStep, index) => {
			...
			const baseValues =
				rawStepData?.controlValues ?? rawStepData?.controls?.values ?? {};
		...
		const workflowTags = Array.isArray(rawWorkflow.tags)
			? rawWorkflow.tags
			: [];
		...
		const payloadSchema =
			rawPayloadSchema &&
			typeof rawPayloadSchema === "object" &&
			!Array.isArray(rawPayloadSchema)
		...
		return {
			...
			workflowDefinition: {
				...
				tags: workflowTags,
				active:
					typeof rawWorkflow.active === "boolean"
						? rawWorkflow.active
						: rawWorkflow.status === "ACTIVE",
		...
		const load = async () => {
			try {
				const templatesApiUrl =
					...
					"https://templates-novuhq.vercel.app";
