...
export function useFirstTriggerDetection({
	...
	useEffect(() => {
		if (workflowsData?.workflows) {
			const demoWorkflow = workflowsData.workflows.find(
				(w) => w.workflowId === ONBOARDING_DEMO_WORKFLOW_ID,
			);
	...
	const {
		...
		queryFn: () => {
			const environment = requireEnvironment(
				currentEnvironment,
				"Environment or workflow slug not available",
			);
		...
		enabled:
			enabled && !!currentEnvironment?._id && !!workflowSlug && !notFound,
		refetchInterval:
			isWaitingForTrigger && !hasDetectedFirstTrigger && !notFound
				? 2000
				: false,
		refetchOnWindowFocus: false,
		staleTime: 0,
		...
		if (workflow.lastTriggeredAt) {
			...
			try {
				...
				if (!Number.isFinite(triggerTime) || Number.isNaN(triggerTime)) {
					console.error(
						"Invalid lastTriggeredAt timestamp:",
				...
				if (visitTime > now) {
					console.warn(
						"First visit timestamp is in the future, ignoring detection:",
						firstVisitTimestamp,
					);
					setIsWaitingForTrigger(false);
					return;
				...
				if (triggerTime > now) {
					console.warn(
						"Trigger timestamp is in the future, treating as not detected:",
						workflow.lastTriggeredAt,
					);
