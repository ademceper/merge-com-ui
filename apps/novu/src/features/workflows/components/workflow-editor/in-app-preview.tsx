...
export const InAppPreviewBell = (props: InAppPreviewBellProps) => {
	...
	return (
		<div
			className={cn(
				"flex items-center justify-end p-2 text-neutral-300",
				className,
			)}
			{...rest}
		>
...
export const InAppPreviewAvatar = (props: InAppPreviewAvatarProps) => {
	...
	return (
		<img
			src={src}
			alt="avatar"
			className={cn("bg-background size-7 rounded-full")}
			{...rest}
		/>
	);
};
...
export const InAppPreviewNotification = (
	props: InAppPreviewNotificationProps,
) => {
	const { className, ...rest } = props;
...
export const InAppPreviewNotificationContent = (
	props: InAppPreviewNotificationContentProps,
) => {
	const { className, ...rest } = props;
...
export const InAppPreviewSubject = (props: InAppPreviewSubjectProps) => {
	...
	return (
		<Markdown
			className={cn(
				"text-foreground-600 truncate text-xs font-medium",
				className,
			)}
			{...rest}
...
export const InAppPreviewBody = (props: InAppPreviewBodyProps) => {
	...
	return (
		<Markdown
			className={cn(
				"text-foreground-400 whitespace-pre-wrap text-xs font-normal",
				className,
			)}
			{...rest}
...
export const InAppPreviewActions = (props: InAppPreviewActionsProps) => {
	...
	return (
		<div
			className={cn("mt-3 flex flex-wrap gap-1 py-px", className)}
			{...rest}
		/>
	);
};
...
export const InAppPreviewPrimaryAction = (
	props: InAppPreviewPrimaryActionProps,
) => {
...
export const InAppPreviewSecondaryAction = (
	props: InAppPreviewSecondaryActionProps,
) => {
	const { className, isPending, children, ...rest } = props;
...
type MarkdownProps = Omit<HTMLAttributes<HTMLParagraphElement>, "children"> & {
	children?: string;
};
...
const Markdown = (props: MarkdownProps) => {
	...
	const tokens = useMemo(
		() => parseMarkdownIntoTokens(children || ""),
		[children],
	);
