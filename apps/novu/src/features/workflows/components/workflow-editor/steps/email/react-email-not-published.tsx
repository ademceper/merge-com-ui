...
const buildSteps = (workflowId: string, stepId: string) => {
	...
	const publishCommand = publishArgs
		? `npx novu email publish ${publishArgs}`
	...
	return [
		{
			...
			description:
				"The npm package needed to initialize and publish React Email templates.",
...
export const ReactEmailNotPublished = ({
	workflowId,
	stepId,
}: ReactEmailNotPublishedProps) => {
			...
			<div className="flex flex-col">
							...
							<div key={step.label} className="flex gap-0">
								<div className="flex flex-col items-center">
									<div className="z-10 flex size-5 shrink-0 items-center justify-center rounded-full bg-[#f4f5f6] shadow-[0px_0px_0px_1px_white,0px_0px_0px_2px_#e1e4ea]">
										<span className="text-label-xs text-[#0e121b]">
											{index + 1}
										</span>
									</div>
									{!isLast && (
				...
				<div className="flex flex-col gap-2 py-4">
					...
					<p className="text-label-xs text-[#99a0ae]">
						Once your React Email template is published, you'll be able to
						preview it here and trigger your first notification.
					</p>
				</div>
			</div>
		</div>
