...
import {
	type TranslationKeyItem,
	TranslationSuggestionsListView,
} from "./translation-suggestions-list-view";
		...
		() =>
			InlineDecoratorExtension.configure({
				...
				isPatternMatch: (value: string) => {
					return (
						value.startsWith(TRANSLATION_DELIMITER_OPEN) &&
						value.endsWith(TRANSLATION_DELIMITER_CLOSE)
					);
				},
				...
				suggestion: {
					...getInlineDecoratorSuggestionsReact(
						TRANSLATION_TRIGGER_CHARACTER,
						propsRef.current.translationKeys,
					),
					...
					items: ({ query }) => {
						...
						const filteredKeys =
							translationKeys?.filter((key) =>
								key.name.toLowerCase().includes(clearedQuery.toLowerCase()),
							) || [];
						...
						const shouldOfferNewKey =
							clearedQuery.trim() &&
							!existingKeys.some(
								(key) => key.toLowerCase() === clearedQuery.toLowerCase(),
							);
					...
					return (
						<TranslationSuggestionsListView
							...
							onSelectItem={(item) => {
								...
								const existingKeys =
									translationKeys?.map((key) => key.name) || [];
