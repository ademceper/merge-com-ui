...
export const WorkflowIssuesPopover = ({
	children,
	steps,
	className,
}: WorkflowIssuesPopoverProps) => {
	const [isOpen, setIsOpen] = useState(false);
	const [hoverTimeout, setHoverTimeout] = useState<NodeJS.Timeout | null>(null);
	const stepsWithIssues = steps.filter(
		(step) => step.issues && countIssues(step.issues as any) > 0,
	);
	...
	const totalIssues = stepsWithIssues.reduce(
		(acc, step) => acc + countIssues(step.issues as any),
		0,
	);
					...
					<div className="space-y-1">
						{stepsWithIssues.map((step, index) => (
							<div key={step.slug}>
								...
								{index < stepsWithIssues.length - 1 && (
...
const StepIssueItem = ({ step }: StepIssueItemProps) => {
	...
	return (
		<div className="space-y-1.5">
			<span className="text-xs font-medium capitalize text-foreground-700">
			...
			<div className="space-y-1">
				{allIssues.slice(0, 3).map((issue, index) => (
					<div key={index} className="flex items-start gap-1.5">
						...
						<div className="text-xs text-foreground-600 leading-snug">
							{issue.message}
						</div>
					</div>
				))}
				{allIssues.length > 3 && (
					<div className="text-xs text-foreground-400 pl-2.5">
