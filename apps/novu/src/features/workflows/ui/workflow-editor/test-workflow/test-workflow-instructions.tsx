...
const LANGUAGE_TO_SNIPPET_UTIL: Record<
	SnippetLanguage,
	(props: CodeSnippet) => string
> = {
	...
	return (
		<div className="space-y-3">
			<div className="text-foreground-400 text-xs">
				A trigger is the starting point of every workflow — an action or event
				that kicks it off. To initiate this, you call the Novu API using
				workflow_id.
			</div>
			<div className="text-foreground-400 text-xs">
				With the trigger, you can pass a custom payload object to the workflow,
				and use it in the workflow steps.
			</div>
			<InlineToast
				variant="tip"
				title="Tip"
...
export function TestWorkflowInstructions({
	...
	const getSnippetForLanguage = (language: SnippetLanguage) => {
		...
		const secretKey =
			language === "shell" && canReadApiKeys && apiKey ? apiKey : undefined;
		const payloadString =
		...
		<Sheet open={isOpen} onOpenChange={onClose}>
			<SheetContent className="flex w-full max-w-2xl flex-col">
				...
				<SheetMain className="min-h-0 flex-1 overflow-y-auto p-0">
					...
					>
						<TabsList
							className="shrink-0 w-full overflow-x-auto px-6"
							variant="regular"
						>
							...
							<TabsContent value="typescript" className="mt-0 min-w-0">
								<TimelineContainer>
									...
									<InstructionStep
										...
										secretMask={
											canReadApiKeys
												? [{ line: 1, maskStart, maskEnd }]
												: undefined
										}
									>
										Use this key to authenticate your API requests. Keep it
												...
												? [
														{
															...
															maskEnd:
																`export NOVU_SECRET_KEY=${apiKey}`.length - 4,
														},
													]
												: undefined
										}
