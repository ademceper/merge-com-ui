...
export const TestWorkflowActivityDrawer = forwardRef<
	HTMLDivElement,
	TestWorkflowActivityDrawerProps
>((props, forwardedRef) => {
	const { isOpen, onOpenChange, transactionId, workflow, to, payload } = props;
	const [parentActivityId, setParentActivityId] = useState<string | undefined>(
		undefined,
	);
	...
	const activityId: string | undefined =
		parentActivityId ?? activities?.[0]?._id;
	const {
	...
	const _handleTransactionIdChange = useCallback((newTransactionId: string) => {
