		...
		<AccordionItem value="payload" className={ACCORDION_STYLES.item}>
						...
						<div className="flex items-center gap-0.5">
							...
							<Tooltip>
								...
								<TooltipContent className="max-w-xs">
									The data that will be sent to your workflow when triggered.
									This can include dynamic values and variables.
								</TooltipContent>
							</Tooltip>
						</div>
					</div>
			...
			<AccordionContent className="flex flex-col gap-2">
				<div className="flex flex-1 flex-col gap-2 overflow-auto">
					...
					/>
					{errors.payload && (
						<p className="text-destructive text-xs">{errors.payload}</p>
					)}
				</div>
				{onManageSchema &&
					workflow?.origin === ResourceOriginEnum.NOVU_CLOUD && (
