...
export const ThrottleWindow = () => {
	...
	React.useEffect(() => {
		if (!type) {
			setValue("controlValues.type", FIXED_THROTTLE_TYPE, {
				shouldDirty: false,
			});
		}
	}, [type, setValue]);
	...
	const handleThrottleTypeChange = async (value: string) => {
		...
		if (preservedFormValues) {
			...
			setValue("controlValues.unit", TimeUnitEnum.MINUTES, {
				shouldDirty: true,
			});
