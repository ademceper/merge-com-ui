...
export function toCronFields(fields: UiCronFields): CronFields {
	return {
		second: (fields.second.length === 0
			? EVERY_SECOND
			: fields.second) as SixtyRange[],
		minute: (fields.minute.length === 0
			? EVERY_MINUTE
			: fields.minute) as SixtyRange[],
		hour: (fields.hour.length === 0 ? EVERY_HOUR : fields.hour) as HourRange[],
		dayOfWeek: (fields.dayOfWeek.length === 0
			? EVERY_DAY_OF_WEEK
			: fields.dayOfWeek) as DayOfTheWeekRange[],
		dayOfMonth: (fields.dayOfMonth.length === 0
			? EVERY_DAY_OF_MONTH
			: fields.dayOfMonth) as DayOfTheMonthRange[],
		month: (fields.month.length === 0
			? EVERY_MONTH
			: fields.month) as MonthRange[],
	};
}
