	...
	const handleStepDataChange = useCallback(
		(stepId: string, updatedStepData: any) => {
			...
			if (stepType === "digest") {
				...
				finalStepData = synchronizeDigestStepData(
					updatedStepData,
					currentStepData,
	...
	const getCurrentStepData = useCallback(
		(stepId: string, stepData: any) => {
			...
			if (
				stepType === "digest" &&
				stepData &&
				"eventCount" in stepData &&
				"events" in stepData
			) {
				return synchronizeDigestStepData(
					stepData,
					{},
					workflow?.payloadExample,
				);
				...
				<div className="flex items-center gap-0.5">
					...
					<Tooltip>
						...
						<TooltipContent className="max-w-xs">
							Output data from previous steps in the workflow that can be used
							in subsequent steps.
						</TooltipContent>
					</Tooltip>
				</div>
								...
								<div key={stepId}>
									...
									>
										<div className="flex flex-1 items-center gap-2">
											...
											<span className="text-label-2xs text-left font-medium">
												{stepName}
											</span>
											{isCurrentStep && (
												<span className="text-label-2xs text-neutral-500">
													(current step)
												</span>
											)}
											...
											<div className="pb-3">
												<EditableJsonViewer
													...
													onChange={(updatedStepData) =>
														handleStepDataChange(stepId, updatedStepData)
													}
												...
												{stepType === "digest" && (
													<div className="pt-2">
														<div className="text-text-soft flex items-center gap-1.5 text-[10px] font-normal leading-[13px]">
															...
															<span>
																Event count and events array are synchronized
																automatically. The event payload is originating
																from the workflow trigger payload.
															</span>
														</div>
													</div>
												)}
