...
export function PushPreview({
	...
	const subject = isValidPushPreview
		? ((previewData?.result.preview as PushRenderOutput)?.subject ?? "")
		: "";
	const body = isValidPushPreview
		? ((previewData?.result.preview as PushRenderOutput)?.body ?? "")
		: "";
...
export const PushSubjectPreview = ({
	subject,
	isPending,
	className,
	...rest
}: PushSubjectPreviewProps) => {
	if (isPending) {
	...
	return (
		<div
			className={cn("flex items-center gap-1.5 whitespace-pre-wrap", className)}
			{...rest}
		>
			<div className="flex-1">
				<span className="line-clamp-1 min-h-4 text-xs font-medium">
					{subject}
				</span>
			</div>
...
export const PushBodyPreview = ({
	body,
	isPending,
	className,
	...rest
}: PushBodyPreviewProps) => {
	if (isPending) {
		return (
...
export const PushContentContainerPreview = ({
	children,
	className,
	...rest
}: HTMLMotionProps<"div">) => {
	return (
		<motion.div
			...
			className={cn(
				"flex w-full flex-col gap-0.5 rounded-md bg-neutral-50 p-1.5",
				className,
			)}
			layout
			{...rest}
...
export const PushBackgroundWithPhone = ({
	children,
	className,
	...rest
	...
	return (
		<div className="flex items-center justify-center w-full">
			<div
				className={cn(
					"relative h-60 w-full max-w-72 bg-[url('/images/phones/iphone-push.svg')] bg-cover",
					className,
				)}
				{...rest}
			>
...
export const PushNotificationContainer = ({
	...
	return (
		<div
			className={cn(
				"absolute bottom-5 left-1/2 z-10 w-11/12 -translate-x-1/2 p-2",
				className,
			)}
			{...rest}
		>
