...
export const useEditorPreview = ({
	...
	const {
		...
		enabled: Boolean(
			workflowSlug && stepSlug && currentEnvironment && parsedEditorPayload,
		),
		staleTime: 0,
		retry: false,
	...
	useEffect(() => {
		...
		const shouldUpdateEditor =
			...
			!areKeysEqual(serverKeys, lastServerKeysRef.current);
	...
	return {
		...
		isFetching,
		isTransitioning:
			JSON.stringify(controlValues) !== JSON.stringify(debouncedControlValues),
	};
};
