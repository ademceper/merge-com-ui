export function synchronizeDigestStepData(
	updatedData: any,
	previousData: any,
	payloadExample?: any,
): any {
	...
	if (currentEventCount !== prevEventCount) {
		const newCount = Math.max(0, parseInt(currentEventCount, 10) || 0);
		...
		if (newCount > syncedEvents.length) {
			...
			while (syncedEvents.length < newCount) {
				syncedEvents.push(
					generatePlaceholderEvent(syncedEvents.length, payloadExample),
				);
			}
	...
	if (
		...
		currentEvents.length !== prevEvents.length
	) {
