...
export const DelayWindow = () => {
	...
	React.useEffect(() => {
		if (!type) {
			setValue("controlValues.type", DelayTypeEnum.REGULAR, {
				shouldDirty: false,
			});
		}
	}, [type, setValue]);
	...
	const handleDelayTypeChange = async (value: string) => {
		...
		setPreservedFormValuesByType((old) => ({
			...old,
			[delayType]: { ...controlValues },
		}));
		setDelayType(value);

		const preservedFormValues =
			preservedFormValuesByType[value as keyof PreservedFormValuesByType];

		if (preservedFormValues) {
			setValue("controlValues.type", preservedFormValues.type, {
				shouldDirty: true,
			});
			setValue("controlValues.amount", preservedFormValues.amount, {
				shouldDirty: true,
			});
			setValue("controlValues.unit", preservedFormValues.unit, {
				shouldDirty: true,
			});
			setValue("controlValues.cron", preservedFormValues.cron, {
				shouldDirty: true,
			});
			setValue("controlValues.dynamicKey", preservedFormValues.dynamicKey, {
				shouldDirty: true,
			});
		} else if (value === DYNAMIC_TYPE) {
			setValue("controlValues.type", DelayTypeEnum.DYNAMIC, {
				shouldDirty: true,
			});
			...
			setValue("controlValues.type", DelayTypeEnum.TIMED, {
				shouldDirty: true,
			});
			setValue("controlValues.amount", undefined, { shouldDirty: true });
			setValue("controlValues.unit", undefined, { shouldDirty: true });
			...
			setValue("controlValues.unit", TimeUnitEnum.SECONDS, {
				shouldDirty: true,
			});
			setValue("controlValues.cron", undefined, { shouldDirty: true });
			setValue("controlValues.dynamicKey", undefined, { shouldDirty: true });
	...
	return (
		<div className="flex flex-col gap-2">
			<FormLabel
				required
				tooltip="Defines how long the workflow execution should be delayed before proceeding."
			>
				Delay type
			</FormLabel>
			...
			<Tabs value={delayType} onValueChange={handleDelayTypeChange}>
				...
				<TabsContent value={SCHEDULED_TYPE} className="mt-3">
					<ScheduledType
						...
						onError={() => {
							setError("controlValues.cron", {
								message: "Failed to parse cron",
							});
						}}
