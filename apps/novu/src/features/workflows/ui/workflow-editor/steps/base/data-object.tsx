...
const InnerDataObject = ({ field }: { field: FieldValues }) => {
	...
	const updateParentForm = () => {
		const uniquePairLength = new Set<string>(
			currentPairs.map((pair) => pair.key),
		).size;
	...
	const handleUpdatePair = (
		...
		const newPairs = currentPairs.map((pair, i) =>
			i === index ? { ...pair, [fieldType]: newValue } : pair,
		);
		setCurrentPairs(newPairs);
	};
	...
	const handleRemovePair = (index: number) => {
		...
		setTimeout(() => {
			...
			const uniquePairLength = new Set<string>(
				updatedPairs.map((pair) => pair.key),
			).size;
				...
				<CardContent className="flex flex-col gap-1 p-2">
					<div className="flex flex-col gap-1">
						{currentPairs.map((pair, index) => {
							const isDuplicate =
								currentPairs.findIndex((p) => p.key === pair.key) < index;
							...
							return (
								<div className="flex flex-col gap-1" key={index}>
									...
									<FormMessage keyName={isDuplicate ? "" : pair.key}>
										{isDuplicate
											? `The key ${pair.key} is already used. Please choose another key.`
											: null}
									</FormMessage>
								</div>
							);
...
export const DataObject = () => {
	...
	return (
		<FormField
			control={control}
			name={dataObjectKey}
			render={({ field }) => <InnerDataObject field={field} />}
		/>
	);
};
