...
export const countIssues = (issues?: {
	...
	if (issues.controls) {
		const controlIssues = Object.values(issues.controls).reduce(
			(acc, issueArray) => acc + issueArray.length,
			0,
		);
	...
	if (issues.integration) {
		const integrationIssues = Object.values(issues.integration).reduce(
			(acc, issueArray) => acc + issueArray.length,
			0,
		);
...
export const flattenIssues = (
	...
	const controlIssuesFlat: Record<string, RuntimeIssue[]> = flatten(
		{ ...controlIssues },
		{ safe: true },
	);
		...
		steps: workflow.steps.map((step) => {
			if (step.stepId === stepId) {
				...
				const updatedControlValues =
					updateStep.controlValues !== undefined
