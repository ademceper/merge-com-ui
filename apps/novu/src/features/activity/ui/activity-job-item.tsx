...
export function ActivityJobItem({
	job,
	isFirst,
	isLast,
}: ActivityJobItemProps) {
			...
			<Card className="border flex-1 overflow-hidden border-neutral-200 p-1 shadow-xs">
				...
				>
					<div className="flex items-center gap-1.5">
						...
						<span className="text-foreground-950 text-xs capitalize">
							{job?.step?.name || formatJobType(job.type)}
						</span>
					</div>
...
function getStatusMessage(job: IActivityJob): string | React.ReactNode {
	...
	if (
		...
		const lastExecutionDetail =
			job.executionDetails[job.executionDetails.length - 1];

		return lastExecutionDetail ? (
			<div className="flex items-center gap-2">
				{lastExecutionDetail.raw ? (
					<TraceTooltip
						message={lastExecutionDetail.detail}
						raw={lastExecutionDetail.raw}
						variant="info"
					/>
				) : (
					<span
	...
	switch (job.type?.toLowerCase()) {
		...
		case StepTypeEnum.DIGEST:
			...
			if (job.status === JobStatusEnum.DELAYED) {
				...
				if (untilDate) {
					const untilDateFormatted = format(
						new Date(untilDate),
						"MMM d yyyy, HH:mm:ss",
					);
		...
		case StepTypeEnum.DELAY: {
			...
			if (job.status === JobStatusEnum.DELAYED) {
				...
				const untilDate = (job.digest as IDigestTimedMetadata)?.timed
					?.untilDate;
				if (untilDate) {
					const untilDateFormatted = format(
						new Date(untilDate),
	...
	return (
		<Tooltip>
			<TooltipTrigger asChild>
				<button
					type="button"
					className="flex items-center gap-1 text-left hover:cursor-default"
				>
					<span
						className={cn(
							"text-destructive",
							variant === "error" ? "text-destructive" : "text-text-soft",
						)}
					>
						{message}
					</span>
			...
			<TooltipContent
				side="right"
				className="max-w-[400px] border border-neutral-200 bg-white p-3 shadow-lg"
			>
...
function getJobIcon(type?: StepTypeEnum) {
	const Icon =
		STEP_TYPE_TO_ICON[type?.toLowerCase() as keyof typeof STEP_TYPE_TO_ICON] ??
		Path;
...
function JobStatusIndicator({ status }: JobStatusIndicatorProps) {
	const { icon: Icon, animationClass } =
		JOB_STATUS_CONFIG[status] || JOB_STATUS_CONFIG[JobStatusEnum.PENDING];

	return (
		<div className="relative shrink-0">
			<div className="flex h-6 w-6 items-center justify-center rounded-full bg-white shadow-xs">
				<div
					className={`${getJobClasses(status)} flex items-center justify-center`}
				>
