...
export function ActivityDetailCard({
	...
	return (
		<div className="border w-full overflow-hidden rounded-lg border border-neutral-100">
			<div
				className={cn(
					"group flex w-full items-center px-3 py-2 hover:bg-neutral-50",
					expandable && "cursor-pointer",
				)}
				onClick={expandable ? () => setInternalOpen(!internalOpen) : undefined}
			>
				<span className="text-foreground-950 flex-1 text-left text-xs font-medium">
					{title}
				</span>
				<div className="flex items-center gap-2 pl-3">
					...
					{expandable && (
						<CaretDown
							className={cn(
								"h-4 w-4 text-[#717784] transition-transform",
				...
				<>
					...
					{footer && (
						<div className="flex gap-2 items-center border-t border-neutral-200 bg-transparent py-1 px-2">
							...
							<span
								className="text-label-xs text-text-soft truncate"
								title={footer}
							>
								{footer}
							</span>
