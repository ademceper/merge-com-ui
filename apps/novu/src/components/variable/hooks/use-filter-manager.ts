...
export function useFilterManager({
	...
	const [filters, setFilters] = useState<FilterWithParam[]>(
		initialFilters.filter((t) => t.value !== "default"),
	);
	...
	const handleParamChange = useCallback(
		(index: number, params: string[]) => {
			setFilters((current) => {
				...
				const filterDef = liquidFilters.find(
					(def) => def.value === newFilters[index].value,
				);
				...
				const formattedParams = params.map((param, paramIndex) => {
					...
					if (paramType === "number") {
						...
						return Number.isNaN(Number(numericValue)) ? "" : numericValue;
					}
