...
function getDefaultRegion(): RegionType {
	...
	return window.location.hostname.includes("eu.")
		? REGION_MAP.EU
		: REGION_MAP.US;
}
			...
			<div className="text-xs font-medium leading-none text-neutral-400">
				...
				<TooltipProvider delayDuration={100}>
					<Tooltip>
						...
						<TooltipContent>
							Novu offers data residency in Europe (Germany) and the United
							States. Data residency cannot be modified after sign-up.
						</TooltipContent>
					</Tooltip>
				</TooltipProvider>
			</div>
							...
							<SelectItem key={option} value={option} className="w-full">
								<div className="flex items-center gap-[6px]">
									{option === REGION_MAP.US ? (
										<USFlag className="h-3 w-3" />
									) : (
										<EuFlag className="h-3 w-3" />
									)}{" "}
									{option}
								</div>
							</SelectItem>
