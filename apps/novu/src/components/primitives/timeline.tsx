...
export function TimelineLine({
	...
	if (variant === "continuous") {
		return (
			<div
				...
				style={{
					background:
						"linear-gradient(to bottom, transparent 0%, #E1E4EA 15%, #E1E4EA 85%, transparent 100%)",
				}}
			/>
		);
	}
	...
	return (
		<div className="absolute left-3 top-6 h-[calc(100%+2rem)] w-px -translate-x-1/2 bg-neutral-100" />
	);
}
...
export function TimelineStep({
	...
	if (layout === "grid") {
		return (
			<motion.div
				{...stepAnimation(index)}
				className="grid grid-cols-[400px_320px] gap-6"
			>
				{/* Left side - Step info */}
				<div className="flex gap-3">
					...
					<div className="flex w-full flex-col items-start gap-2 pr-5">
						...
						{description && (
							<p className="text-text-soft text-xs">{description}</p>
						)}
						{leftExtraContent}
					</div>
				</div>
	...
	return (
		<motion.div
			{...stepAnimation(index)}
			className="relative flex min-w-0 gap-6"
		>
			...
			<div className="min-w-0 flex-1">
				...
				{description && (
					<div className="text-label-xs text-text-soft mt-2">{description}</div>
				)}
				{children}
			</div>
		</motion.div>
