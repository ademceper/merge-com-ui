...
export function AnalyticsCard({
	...
	if (showSkeleton) {
		return (
			<div
				className={cn(
					"bg-bg-white rounded-12 p-3 shadow-box-xs w-full min-h-[108px]",
					className,
				)}
			>
				<div className="flex flex-col gap-1.5">
					<div className="flex items-center justify-between">
						<div className="flex items-center gap-1">
							{IconComponent && (
								<IconComponent className="size-4 text-icon-sub" />
							)}
	...
	return (
		<div
			className={cn(
				"bg-bg-white rounded-12 p-3 shadow-box-xs w-full",
				className,
			)}
		>
			<div className="flex flex-col gap-1.5">
				...
				{description && (
					<div className="text-paragraph-xs text-text-soft">{description}</div>
				)}
			</div>
		</div>
	);
