...
export function isTypingTranslation(content: string, pos: number): boolean {
	...
	const hasOpenPattern = beforeCursor.includes(
		TRANSLATION_DELIMITER_OPEN + TRANSLATION_NAMESPACE_SEPARATOR,
	);
	if (!hasOpenPattern) return false;

	// Find the last occurrence of translation start
	const lastPatternStart = beforeCursor.lastIndexOf(
		TRANSLATION_DELIMITER_OPEN + TRANSLATION_NAMESPACE_SEPARATOR,
	);
...
export function isInsideVariableContext(context: CompletionContext): boolean {
	...
	if (lastClose === -1 || lastOpen > lastClose) {
		...
		const contentAfterOpen = beforeCursor
			.slice(lastOpen + TRANSLATION_DELIMITER_OPEN.length)
			.trim();
...
export function parseTranslation(
	translation: string,
): { key: string; fullExpression: string } | undefined {
