...
export function VariableEditor({
	...
	const variableCompletionSource = useMemo(() => {
		return (context: CompletionContext) => {
			return createAutocompleteSource(
				...
				(completion: CompletionOption) =>
					callbacksRef.current.onVariableSelect(completion),
				async (variableName: string) =>
					callbacksRef.current.onCreateNewVariable(variableName),
		...
		return [
			autocompletion({
				...
				optionClass: (completion) => {
					const classes = [];
					if (completion.type === "new-variable")
						classes.push("cm-new-variable-option");
					if (completion.type === "new-translation-key")
						classes.push("cm-new-translation-option");
					return classes.join(" ");
				},
			}),
			...
			EditorView.updateListener.of(() => {
				document
					.querySelectorAll('li[role="option"]:not([title])')
					.forEach((item) => {
	...
	const variablePluginExtension = useMemo(() => {
		const getVariableError = (
			...
			return getVariableErrorMessage({
				...
				isAllowed: false,
				isInSchema: false,
				isPayloadSchemaEnabled: isPayload
					? callbacksRef.current.isPayloadSchemaEnabled
					: undefined,
			});
		};
		...
		return createVariableExtension({
			viewRef,
			lastCompletionRef,
			onSelect: (value: string, from: number, to: number) =>
				callbacksRef.current.handleVariableSelect(value, from, to),
			isAllowedVariable: (variable: LiquidVariable) =>
				callbacksRef.current.isAllowedVariable(variable),
			isDigestEventsVariable: (variableName: string) =>
				callbacksRef.current.isDigestEventsVariable(variableName),
	...
	const editorExtensions = useMemo(() => {
		...
		const baseExtensions = [
			...(callbacksRef.current.multiline ? [EditorView.lineWrapping] : []),
			variablePillTheme,
		];
