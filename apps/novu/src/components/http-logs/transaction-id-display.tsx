...
export function TransactionIdDisplay({
	...
	if (transactionIds.length <= 1) {
		return (
			<div className="flex items-center gap-1">
				<CopyButton
					valueToCopy={transactionId}
					className="text-text-soft size-6 p-1"
					size="2xs"
				/>
				<span
					className={cn(
						"text-text-sub font-mono text-xs font-normal tracking-[-0.24px]",
						className,
					)}
				>
					{transactionId}
								...
								<div className="grid gap-1">
									{transactionIds.map((id, index) => (
										...
										>
											...
											<CopyButton
												valueToCopy={id}
												className="text-text-soft size-4 shrink-0 p-0.5"
												size="2xs"
											/>
										</div>
									))}
