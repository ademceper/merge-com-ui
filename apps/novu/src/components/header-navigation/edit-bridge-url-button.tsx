...
export const EditBridgeUrlButton = () => {
	...
	const { validateBridgeUrl, isPending: isValidatingBridgeUrl } =
		useValidateBridgeUrl();
	const { updateBridgeUrl, isPending: isUpdatingBridgeUrl } =
		useUpdateBridgeUrl();
	...
	const onSubmit = async ({ bridgeUrl }: z.infer<typeof formSchema>) => {
		...
		if (isValid) {
			...
			setError("bridgeUrl", {
				message: "The provided URL is not the Novu Endpoint URL",
			});
		}
	};
									...
									render={({ field }) => (
										<FormItem>
											...
											<FormControl>
												<Input
													leadingIcon={Link}
													id={`bridgeUrl-${field.name}`}
													{...field}
													readOnly={isReadOnly}
												/>
											</FormControl>
											<FormMessage>
												URL (e.g., https://your.api.com/api/novu)
											</FormMessage>
										</FormItem>
									)}
								/>
