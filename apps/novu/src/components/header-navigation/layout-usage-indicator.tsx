...
export function LayoutUsageIndicator({
	layoutResource,
	allWorkflows,
	dependencies,
}: LayoutUsageIndicatorProps) {
	const layoutName =
		layoutResource.sourceResource?.name || layoutResource.targetResource?.name;
	const layoutId =
		layoutResource.sourceResource?.id || layoutResource.targetResource?.id;
	...
	const workflowsUsingLayout = useMemo(() => {
		...
		dependencies.forEach((deps, workflowId) => {
			const workflow = allWorkflows.find(
				(w) =>
					w.sourceResource?.id === workflowId ||
					w.targetResource?.id === workflowId,
			);
			...
			if (
				...
				const workflowName =
					workflow.sourceResource?.name || workflow.targetResource?.name;
		...
		<Tooltip>
			<TooltipTrigger asChild>
				<div className="relative flex cursor-pointer items-center gap-1 p-0">
					<span className="text-xs font-medium leading-3 text-gray-400">
						Used in
					</span>
					<UsageDisplay />
				</div>
			...
			<TooltipContent
				side="top"
				className="rounded-lg border border-gray-200 bg-white p-1.5 pb-1 pt-1.5 shadow-lg"
			>
				<div className="flex flex-col gap-1">
					<div className="mb-1 text-xs font-medium leading-3 text-gray-400">
						Used in
					</div>
					{workflowsUsingLayout.map((workflow, index) => (
						<div
							key={index}
							className="flex min-w-[175px] items-center gap-1.5 rounded bg-gray-50 px-1 py-0.5"
						>
							<Path weight="fill" className="text-icon-sub h-3.5 w-3.5" />
