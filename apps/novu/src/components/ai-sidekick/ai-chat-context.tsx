...
export function AiChatProvider({
	...
	const [isFirstMessageRevertDialogOpen, setFirstMessageRevertDialogOpen] =
		useState(false);
	...
	const { areEnvironmentsInitialLoading, currentEnvironment } =
		useEnvironment();
	...
	const lastUserMessageId = useMemo(() => {
		const userMessages = messages.filter(
			(m) => m.role === AiMessageRoleEnum.USER,
		);
	...
	const handleSendMessage = useCallback(
		async (message: string) => {
			const { resourceType, resourceId, isAborted, latestChat, messages } =
				dataRef.current;
			const isLastUserMessage =
				messages.length > 0 &&
				messages[messages.length - 1].role === AiMessageRoleEnum.USER;
			...
			if (!latestChat) {
				...
				const lastUserMessage = messages
					.filter((m) => m.role === AiMessageRoleEnum.USER)
					.pop();
				sendPrompt({
	...
	const handleLastUserMessage = useCallback((chat?: AiChatResponseDto) => {
		if (
			...
			chat.messages[chat.messages.length - 1].role === AiMessageRoleEnum.USER
		) {
			const promptText =
				chat.messages[chat.messages.length - 1].parts?.find(
					(p) => p.type === "text",
				)?.text ?? "";
	...
	const isLoading =
		isResourceLoading || isFetchingAiChat || areEnvironmentsInitialLoading;
	...
	return (
		<AiChatContext.Provider value={value}>
			...
			<ConfirmationModal
				...
				confirmButtonText={
					pendingRevertAction?.type === "tryAgain" ? "Try again" : "Revert"
				}
