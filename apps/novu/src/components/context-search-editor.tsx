...
export function ContextSearchEditor({
	value,
	onUpdate,
	schema,
	error,
}: ContextSearchEditorProps) {
					...
					return (
						<div className="flex flex-col items-start gap-0.5">
							...
							{originalContext.data &&
								Object.keys(originalContext.data).length > 0 && (
									<span className="text-xs text-foreground-400">
										{Object.keys(originalContext.data).join(", ")}
									</span>
								)}
						</div>
					);
