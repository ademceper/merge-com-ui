...
function carryOverCommonKeywords(
	originalSchema: JSONSchema7,
	newSchema: Partial<JSONSchema7>,
) {
	if (originalSchema.title !== undefined)
		newSchema.title = originalSchema.title;
	if (originalSchema.description !== undefined)
		newSchema.description = originalSchema.description;
	if (originalSchema.default !== undefined)
		newSchema.default = originalSchema.default; // Type compatibility should be ensured by caller or validation
	if (originalSchema.examples !== undefined)
		newSchema.examples = originalSchema.examples;
	if (originalSchema.$id !== undefined) newSchema.$id = originalSchema.$id;
	if (originalSchema.$schema !== undefined)
		newSchema.$schema = originalSchema.$schema;

	if (originalSchema.readOnly !== undefined)
		newSchema.readOnly = originalSchema.readOnly;
	if (originalSchema.writeOnly !== undefined)
		newSchema.writeOnly = originalSchema.writeOnly;
	...
	if (originalSchema.allOf !== undefined)
		newSchema.allOf = originalSchema.allOf;
	if (originalSchema.anyOf !== undefined)
		newSchema.anyOf = originalSchema.anyOf;
	if (originalSchema.oneOf !== undefined)
		newSchema.oneOf = originalSchema.oneOf;
	if (originalSchema.not !== undefined) newSchema.not = originalSchema.not;
	if (originalSchema.definitions !== undefined)
		newSchema.definitions = originalSchema.definitions;
	if (originalSchema.$defs !== undefined)
		newSchema.$defs = originalSchema.$defs;
	if (originalSchema.$ref !== undefined) newSchema.$ref = originalSchema.$ref;
	if (originalSchema.$comment !== undefined)
		newSchema.$comment = originalSchema.$comment;
...
export function newProperty(type: JSONSchema7TypeName = "string"): JSONSchema7 {
	const baseProperty: Partial<JSONSchema7 & { propertyList?: any[] }> = {
		type,
	};
...
export function ensureObject(schema: JSONSchema7): JSONSchema7 {
	const newSchema: Partial<JSONSchema7 & { propertyList?: any[] }> = {
		type: "object",
	};
	carryOverCommonKeywords(schema, newSchema);

	newSchema.propertyList =
		(schema as any).propertyList && Array.isArray((schema as any).propertyList)
			? (schema as any).propertyList
			: [];
	...
	if (schema.minProperties !== undefined)
		newSchema.minProperties = schema.minProperties;
	if (schema.maxProperties !== undefined)
		newSchema.maxProperties = schema.maxProperties;
	if (schema.patternProperties !== undefined)
		newSchema.patternProperties = schema.patternProperties;
...
export function ensureArray(schema: JSONSchema7): JSONSchema7 {
	...
	if (schema.uniqueItems !== undefined)
		newSchema.uniqueItems = schema.uniqueItems;
	return newSchema as JSONSchema7;
}
...
export function ensureString(schema: JSONSchema7): JSONSchema7 {
	...
	if (
		...
		typeof newSchema.default !== "string"
	) {
...
export function ensureNumberOrInteger(
	schema: JSONSchema7,
	newType: "number" | "integer",
): JSONSchema7 {
	...
	if (schema.exclusiveMinimum !== undefined)
		newSchema.exclusiveMinimum = schema.exclusiveMinimum;
	if (schema.exclusiveMaximum !== undefined)
		newSchema.exclusiveMaximum = schema.exclusiveMaximum;
	if (schema.multipleOf !== undefined) newSchema.multipleOf = schema.multipleOf;

	if (
		newSchema.default !== undefined &&
		typeof newSchema.default !== "number"
	) {
		delete newSchema.default;
	}
...
export function ensureBoolean(schema: JSONSchema7): JSONSchema7 {
	...
	if (
		newSchema.default !== undefined &&
		typeof newSchema.default !== "boolean"
	) {
		delete newSchema.default;
	}
...
export function ensureEnum(schema: JSONSchema7): JSONSchema7 {
	...
	if (
		...
		schema.enum.every((val): val is string => typeof val === "string")
	) {
