...
export function convertSchemaToPropertyList(
	...
	return Object.entries(schemaProperties).map(([key, value]) => {
		...
		if (definitionForListItem.type === "object" && definition.properties) {
			nestedPropertyList = convertSchemaToPropertyList(
		...
		if (definitionForListItem.type === "array" && definition.items) {
			...
			if (itemType === "object" && items.properties) {
				const itemsPropertyList = convertSchemaToPropertyList(
					items.properties,
					items.required,
				);
...
export function convertPropertyListToSchema(
	...
	propertyList.forEach((item) => {
		...
		const currentDefinition = processPropertyDefinition(
			item.definition,
			item.isNullable,
		);
...
function processPropertyDefinition(
	...
	if (isObjectWithPropertyList(definitionAsObjectWithList)) {
		const nestedConversion = convertPropertyListToSchema(
	...
	if (isArrayWithObjectItems(currentDefinition)) {
		currentDefinition.items = processArrayItems(
			currentDefinition.items as JSONSchema7,
		);
	}
	...
	if (
		...
		currentDefinition.type !== "null"
	) {
		if (typeof currentDefinition.type === "string") {
			currentDefinition.type = [currentDefinition.type, "null"] as any;
		} else if (
			Array.isArray(currentDefinition.type) &&
			!currentDefinition.type.includes("null")
		) {
			currentDefinition.type = [...currentDefinition.type, "null"] as any;
		}
	}
...
function processArrayItems(items: JSONSchema7): JSONSchema7 {
	const itemsWithList = items as JSONSchema7 & {
		propertyList?: PropertyListItem[];
	};

	if (isObjectWithPropertyList(itemsWithList)) {
		const itemsConversion = convertPropertyListToSchema(
			itemsWithList.propertyList,
		);
		const { propertyList: _propertyList, ...itemsWithoutPropertyList } =
			itemsWithList;
...
function isObjectWithPropertyList(
	...
	return !!(
		...
		definition.propertyList &&
		definition.propertyList.length > 0
	);
}
