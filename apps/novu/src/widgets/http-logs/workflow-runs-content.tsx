...
import { ActivityTableRow } from "@/features/activity/components/activity-table-row";
import { useFetchActivities } from "@/features/activity/hooks/use-fetch-activities";
import { useFetchWorkflowRunsCount } from "@/features/workflows/hooks/use-fetch-workflow-runs-count";
...
export function WorkflowRunsContent({ log }: WorkflowRunsContentProps) {
	...
	const { data: workflowRunsCount, isLoading: isCountLoading } =
		useFetchWorkflowRunsCount({
			filters: activityFilters,
			staleTime: 30000,
			refetchOnWindowFocus: false,
		});
	...
	const handleLoadMore = async () => {
		...
		setDisplayedItemsCount((prev) =>
			Math.min(prev + ITEMS_PER_PAGE, activities.length),
		);
							...
							{isLoading ? (
								<div className="p-3">
									{Array.from({ length: 3 }).map((_, index) => (
										<div
											key={index}
											className="mb-3 flex items-center gap-2 rounded-lg border border-neutral-100 p-3"
										>
										...
										{activities.length === 0 ? (
											<div className="flex flex-col items-center gap-1">
												<p className="text-foreground-500 text-xs">
													No activities available in this environment
												</p>
											...
											<div className="flex flex-col items-center gap-1">
												<p className="text-foreground-500 text-xs">
													{activities.length} activities available, but none
													match your filters
												</p>
												<p className="text-foreground-400 text-xs">
												...
												>
													{isLoadingMore ? (
														...
														<>
															...
															Load more (
															{activities.length - displayedItemsCount}{" "}
															remaining)
														</>
													)}
												</Button>
