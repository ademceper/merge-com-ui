	...
	return (
		<div className="border-stroke-soft overflow-auto rounded-md border bg-white">
			<div
				...
				onClick={onToggle}
			>
				<span className="text-text-sub font-mono text-xs font-medium tracking-[-0.24px]">
					{title}
				</span>
				<div className="flex items-center gap-0.5">
					<CopyButton
						valueToCopy={textToCopy}
						className="text-text-soft size-7 p-1"
						size="2xs"
					/>
...
export function LogsDetailContent({ log }: LogsDetailContentProps) {
	...
	const hasRequestBody =
		log.requestBody &&
		log.requestBody !== "{}" &&
		log.requestBody.toString().trim() !== "";
	const hasResponseBody =
		log.responseBody &&
		log.responseBody !== "{}" &&
		log.responseBody.toString().trim() !== "";
			...
			<div className="space-y-2 px-3 py-2.5">
				<div className="mb-3">
					<div className="mb-3 flex items-center gap-2">
						...
						<span className="text-text-soft font-mono text-xs font-normal tracking-[-0.24px]">
							{log.method}
						</span>
					...
					<div className="space-y-3">
						<div className="flex items-center justify-between">
							<span className="text-text-soft font-mono text-xs font-medium tracking-[-0.24px]">
								Received at
							</span>
