...
import { DIGEST_VARIABLES } from "@/widgets/variable-editor/utils/digest-variables";
...
export const insertVariableToEditor = ({
	...
	const calculatedRange = range || {
		from: Math.max(
			0,
			editor.state.selection.from - queryWithoutSuffix.length - 4,
		), // -4 for '{{ }}'
	...
	const getVariables = () => {
		...
		const iterableProperties = [...namespaces, ...arrays, ...primitives]
			...
			.flatMap((variable) => {
				...
				return suffix
					? [{ name: `${REPEAT_BLOCK_ITERABLE_ALIAS}.${suffix}` }]
					: [];
			});
		...
		return [
			...baseVariables,
			...iterableProperties,
			{ name: REPEAT_BLOCK_ITERABLE_ALIAS },
		];
	};
...
export const calculateVariables = ({
	 ...
	 */
	if (
		from === VariableFrom.Content &&
		isAllowedVariable({ name: queryWithoutSuffix })
	) {
...
const getRepeatBlockEachVariables = (
	editor: TiptapEditor,
): Array<LiquidVariable> => {
	const iterableName = editor?.getAttributes("repeat")?.each;
...
const dedupAndSortVariables = (
	...
	const uniqueVariables = Array.from(
		new Map(filteredVariables.map((item) => [item.name, item])).values(),
	);
