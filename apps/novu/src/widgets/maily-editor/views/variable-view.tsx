...
import { VariableFrom } from "@/widgets/maily-editor/types";
import { EditVariablePopover } from "@/widgets/variable-editor/edit-variable-popover";
import { useVariableValidation } from "@/widgets/variable-editor/hooks/use-variable-validation";
import { validateEnhancedDigestFilters } from "@/widgets/variable-editor/utils";
import {
	DIGEST_VARIABLES_ENUM,
	getDynamicDigestVariable,
} from "@/widgets/variable-editor/utils/digest-variables";
import { VariablePill } from "@/widgets/variable-editor/variable-pill";
...
function parseVariableWithFallback(
	variable: string,
	fallbackName?: string,
	digestStepId?: string,
): ParsedVariableData {
...
export function NodeVariablePill(
	...
	const handleUpdate = useCallback(
		(newValue: string) => {
			const newParsedData = parseVariableWithFallback(
				newValue,
				undefined,
				digestStepName,
			);
			const newAliasFor = resolveRepeatBlockAlias(
				newParsedData.fullLiquidExpression,
				editor,
			);
...
export function createVariableNodeView(
	variables: LiquidVariable[],
	isAllowedVariable: IsAllowedVariable,
) {
