...
} from "@/widgets/variable-editor/variable-list";
		...
		const options = useMemo(
			() =>
				items.map((item) => {
					...
					const displayLabel = hasDisplayLabel(item)
						? item.displayLabel
						: (item as ExtendedVariable).name;
						...
						return {
							...
							preview:
								item.name in DIGEST_PREVIEW_MAP
									? DIGEST_PREVIEW_MAP[
											item.name as keyof typeof DIGEST_PREVIEW_MAP
										]
									: undefined,
						};
					}
...
function hasDisplayLabel(item: Variable): item is ExtendedVariable {
	return (
		...
		typeof (item as ExtendedVariable).displayLabel === "string"
	);
}
