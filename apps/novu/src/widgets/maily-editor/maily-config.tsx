...
import { createCards } from "@/widgets/maily-editor//blocks/cards";
import { createDigestBlock } from "@/widgets/maily-editor//blocks/digest";
import { createFooters } from "@/widgets/maily-editor/blocks/footers";
import { createHeaders } from "@/widgets/maily-editor/blocks/headers";
import { createHtmlCodeBlock } from "@/widgets/maily-editor/blocks/html";
import { ForView } from "@/widgets/maily-editor/views/for-view";
import { HTMLCodeBlockView } from "@/widgets/maily-editor/views/html-view";
import { useCreateTranslationExtension } from "@/pages/workflows/ui/workflow-editor/steps/email/translations";
import type { TranslationValueInputComponent } from "@/pages/workflows/ui/workflow-editor/steps/email/translations/edit-translation-popover/edit-translation-popover";
	...
	const blocksMap: Record<BlockType, () => BlockItem | null> = {
		...
		digest: () =>
			digestStepBeforeCurrent
				? createDigestBlock({ track, digestStepBeforeCurrent })
				: null,
...
const getAvailableBlocks = (
	...
	if (isInsideRepeat) {
		...
		return blocks.map((block) => ({
			...block,
			commands: block.commands.filter(
				(cmd) => !filteredBlocks.includes(cmd.title),
			),
		}));
	}
...
export const useCreateExtensions = ({
	...
	const translationExtension = useCreateTranslationExtension({
		...
		variables: props.parsedVariables.variables.filter(
			(v) => v.name !== LAYOUT_CONTENT_VARIABLE,
		),
		...
		const extensions: AnyExtension[] = [
			RepeatExtension.extend({
				...
				addAttributes() {
					...
					const payloadArrays = parsedVariables.arrays.filter((array) =>
						array.name.startsWith("payload."),
					);
					const firstArrayVariable =
						payloadArrays.length > 0 ? payloadArrays[0].name : "payload.items";
				...
				suggestion: {
					...
					items: ({ query, editor }) => {
						return searchSlashCommands(
							query,
							editor,
							getAvailableBlocks(blocks, editor),
						);
					},
				},
			}),
			VariableExtension.extend({
				addNodeView() {
					return ReactNodeViewRenderer(
						...
						),
						{
							...
							className:
								"relative inline-block text-[max(12px,calc(1em-3px))] h-5",
							as: "div",
						},
					);
				},
				...
				suggestion: {
					...
					command: ({ editor, range, props }) => {
						...
						const existsInSchema = parsedVariables.variables.some(
							(v) => v.name === props.id,
						);
						const isNewVariable =
							!existsInSchema &&
							!(props.id.startsWith("current.") || props.id === "current");
						...
						if (props.id === TRANSLATION_NAMESPACE_SEPARATOR) {
							...
							editor
								.chain()
								.focus()
								.insertContentAt(range, TRANSLATION_TRIGGER_CHARACTER)
								.run();
					...
					return {
						...
						updateButtonAttributes: (attrs: MailyButtonAttributes) => {
							...
							if (isTextVariable || isUrlVariable) {
								...
								return commands.updateButtonAttributes?.({
									...attrs,
					...
					return {
						...
						updateImageAttributes: (attrs) => {
							...
							if (isSrcVariable || isExternalLinkVariable) {
								...
								return commands.updateImageAttributes?.({
									...attrs,
									aliasFor: aliasFor ?? null,
								});
							}
					...
					return {
						...
						updateLinkAttributes: (attrs: MailyLinkAttributes) => {
							...
							if (isUrlVariable) {
								...
								return commands.updateLinkAttributes?.({
									...attrs,
									aliasFor: aliasFor ?? null,
								});
							}
