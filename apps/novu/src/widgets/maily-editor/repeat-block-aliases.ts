...
export const resolveRepeatBlockAlias = (
	...
	if (isAllowedAlias(nameRoot) && isInsideRepeatBlock(editor)) {
		...
		const replacedVariable = name.replace(
			nameRoot,
...
const getVariableFromNodeAttrs = (
	nodeType: string,
	...
	editor
		...
		.command(({ tr }) => {
			...
			block.content.descendants((node, pos) => {
				...
				const variableValue = getVariableFromNodeAttrs(
					node.type.name,
					node.attrs,
				);
				...
				tr.setNodeMarkup(repeatPos + pos + 1, null, {
					...node.attrs,
					aliasFor: newAlias,
				});
			});
