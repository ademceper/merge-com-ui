				...
				{hasNoChatHistory && messages.length === 0 ? (
					<div className="flex justify-start items-center h-full p-5">
						<div className="flex flex-col gap-1">
							...
							<span className="text-label-xs text-text-soft">
								Suggests improvements, fills gaps, and applies best practices as
								you build.{" "}
							</span>
						</div>
					</div>
					...
					<ConversationContent className="gap-4 py-4 px-4 -ml-4 -mr-3.5">
						{messages.map((chatMessage) => {
							...
							const hasReasoningContent = (chatMessage.parts ?? []).some(
								(p) => p.type === "reasoning",
							);
							const hasToolCallsContent = (chatMessage.parts ?? []).some((p) =>
							...
							const isLastMessage =
								chatMessage.id === messages[messages.length - 1].id;
						...
						{errorMessage && (
							<Message from="assistant">
								<div className="rounded-lg border border-red-200 bg-red-50 p-2 flex">
									<span className="text-label-xs text-red-700">
										Error: {errorMessage}
									</span>
								</div>
							</Message>
