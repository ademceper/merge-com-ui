...
export function IssuesPanel({
	...
	const firstIntegrationError = getFirstErrorMessage(
		issues || {},
		"integration",
	);
					...
					<HoverCard openDelay={200} closeDelay={100}>
						<HoverCardTrigger asChild>
							<div className="flex cursor-pointer items-center gap-2 transition-colors hover:text-red-700">
								...
								<span className="text-paragraph-xs font-medium text-red-600">
									{displayText}
								</span>
							</div>
						</HoverCardTrigger>
						...
						>
							<div className="flex items-center gap-2 pl-1.5">
								...
								<span className="text-label-xs font-medium text-red-600">
									Action required
								</span>
							</div>
								...
								<ul className="space-y-2">
									{allIssues.map((issue, index) => (
										...
										>
											...
											<span className="text-label-xs text-text-sub font-medium leading-4">
												{issue.message}
											</span>
										</li>
									))}
