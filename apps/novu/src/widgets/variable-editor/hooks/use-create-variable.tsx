...
import type { JSONSchema7TypeName } from "@/widgets/schema-editor-widget/json-schema";
...
import { useWorkflowSchema } from "@/pages/workflows/ui/workflow-editor/workflow-schema-provider";
			...
			<div className="flex min-w-[350px] items-center justify-between gap-1.5">
				<div className="flex items-center gap-3">
					...
					<span className="min-w-[100px] text-sm">
						Payload variable added to schema
					</span>
				</div>
			</div>
		),
...
export const useCreateVariable = () => {
	...
	const [isPayloadSchemaDrawerOpen, setIsPayloadSchemaDrawerOpen] =
		useState(false);
	const [highlightedVariableKey, setHighlightedVariableKey] = useState<
		string | null
	>(null);
	...
	const { savePersistedSubscriber, savePersistedContext } =
	...
	const handlePayloadVariable = useCallback(
		async (variableInfo: VariableInfo) => {
			...
			addSchemaProperty(
				{ keyName: variableInfo.key },
				"string" as JSONSchema7TypeName,
			);
	...
	const handleSubscriberVariable = useCallback(
		(variableInfo: VariableInfo) => {
			...
			const newVariable = variableInfo.key
				...
				.reduceRight(
					(value, key) => ({ [key]: value }),
					"example_value" as unknown,
				);

			const updatedSubscriberData = merge(
				{},
	...
	const handleCreateNewVariable = useCallback(
		async (variablePath: string) => {
			...
			try {
				...
				showErrorToast(
					`Failed to create ${variableInfo.type} variable: ${error}`,
				);
			}
		},
		[
