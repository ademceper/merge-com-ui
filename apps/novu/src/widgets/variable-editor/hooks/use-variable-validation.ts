...
export const isPayloadVariable = (variableName: string): boolean => {
	return (
		variableName?.startsWith("payload.") ||
		variableName?.startsWith("current.payload.")
	);
};

type VariableValidationState = {
...
export const useVariableValidation = (
	...
	return useMemo(() => {
		...
		const hasError =
			isPayload && !isInSchema && isPayloadSchemaEnabled ? true : !isAllowed;
