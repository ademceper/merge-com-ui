...
import { DigestCountSummaryPreview } from "@/widgets/variable-editor/components/digest-count-summary-preview";
import { DigestSentenceSummaryPreview } from "@/widgets/variable-editor/components/digest-sentence-summary-preview";
	...
	},
	{
		...
		info: () => {
			const dom = createInfoPanel({
				component: <DigestSentenceSummaryPreview />,
			});
			return {
				dom,
				destroy: () => {
...
export const DIGEST_VARIABLES_FILTER_MAP = {
	[DIGEST_VARIABLES_ENUM.COUNT_SUMMARY]:
		"| pluralize: 'notification', 'notifications'",
	[DIGEST_VARIABLES_ENUM.SENTENCE_SUMMARY]:
		"| toSentence: 'payload.name', 2, 'other'",
} as const;
