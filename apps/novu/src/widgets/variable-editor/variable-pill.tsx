...
import {
	...
	type validateEnhancedDigestFilters,
} from "./utils";
		...
		return (
			...
			>
				...
				>
					...
					<span
						className="text-label-xs text-text-sub max-w-[24ch] truncate"
			...
			{filters?.length > 0 && (
				<span className="flex items-center whitespace-nowrap">
					<span className="text-text-soft">
						{hasArgs ? `| ${firstFilterName}:\u00A0` : `| ${firstFilterName}`}
					</span>
					{hasArgs && (
						<span
							className="text-text-sub max-w-[24ch] truncate"
							title={finalParam}
						>
							{finalParam}
						</span>
					...
					{filters && filters?.length > 1 && (
						<Tooltip>
							...
							<TooltipPortal>
								<TooltipContent
									side="top"
									className="border-bg-soft bg-bg-weak border p-0.5 shadow-sm"
								>
