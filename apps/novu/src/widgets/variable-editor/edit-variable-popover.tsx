...
import type { JSONSchema7 } from "@/widgets/schema-editor-widget/json-schema";
...
const calculateAliasFor = (name: string, parsedAliasRoot: string): string => {
	...
	const normalizedVariableRest = variableRest.startsWith(".")
		? variableRest.substring(1)
		: variableRest;
	let aliasFor =
		parsedAliasRoot && normalizedVariableRest
			? `${parsedAliasRoot}.${normalizedVariableRest}`
			: parsedAliasRoot;
...
export const EditVariablePopover = ({
	...
	const { parsedName, parsedAliasForRoot, parsedDefaultValue, parsedFilters } =
	...
	const [filters, setFilters] = useState<FilterWithParam[]>(
		parsedFilters || [],
	);

	const aliasFor = useMemo(
		() => calculateAliasFor(name, parsedAliasForRoot),
		[name, parsedAliasForRoot],
	);
	...
	const filteredFilters = useMemo(
		() => getFilteredFilters(searchQuery),
		[getFilteredFilters, searchQuery],
	);
	...
	}, [onAddToSchemaClick, name, handleOpenChange, validation.variableName]);

	useEscapeKeyManager(
		id,
		handleClosePopover,
		EscapeKeyManagerPriority.POPOVER,
		open,
	);

	const showManageSchemaButton =
		isPayloadSchemaEnabled &&
		validation.isPayloadVariable &&
		validation.isInSchema;
	const showAddToSchemaButton =
		isPayloadSchemaEnabled &&
		validation.isPayloadVariable &&
		!validation.isInSchema;
	const showVariableTypeInput =
		isPayloadSchemaEnabled && validation.isPayloadVariable;
						...
						<div className="flex flex-col gap-1">
								...
								<FormControl>
									<div className="grid">
										<div className="mb-1 flex w-full flex-row items-center justify-between gap-1">
											<label className="text-text-sub text-label-xs items-start">
												Variable
											</label>
										...
										{validation.hasError && showAddToSchemaButton && (
											<FormMessagePure
												hasError={true}
												className="text-label-2xs mb-0.5 mt-0.5"
											>
							...
							{showVariableTypeInput && isPayloadSchemaEnabled && (
								<div className="text-label-2xs text-text-soft items-center gap-1.5 px-1 py-0.5 font-medium">
									💡 <b className="text-text-sub font-medium">Tip:</b> Edit
									variable type, mark as required field, and add validation via{" "}
									<LinkButton
										variant="gray"
										size="sm"
								...
								<FormControl>
									<div className="">
											...
											<Tooltip>
												...
												<TooltipContent side="top" className="max-w-xs">
													<p className="text-label-xs">
														LiquidJS filters modify the variable output in
														sequence, with each filter using the previous one's
														result. Reorder them by dragging and dropping.
													</p>
												</TooltipContent>
											</Tooltip>
										</label>
										...
										>
											...
											<PopoverContent
												className="min-w-[calc(275px-1rem)] max-w-[calc(275px-1rem)] p-0"
												align="start"
											>
												<Command>
													<div className="p-1">
														...
														{suggestedFilters.length > 0 && !searchQuery && (
															<>
																<CommandGroup heading="Suggested">
																	{suggestedFilters[0].filters.map(
																		(filterItem: Filters) => (
																			<CommandItem
