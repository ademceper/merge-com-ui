...
function parseDefaultValue(
	...
	type: JSONSchema7TypeName | "enum" | undefined,
) {
...
export const SchemaPropertySettingsPopover = forwardRef<
	HTMLDivElement,
	SchemaPropertySettingsPopoverProps
>((props, ref) => {
	const {
		definitionPath,
	...
	const currentDefinition = watch(
		definitionPath as Path<SchemaEditorFormValues>,
	...
	const isNumericType =
		effectiveType === "integer" || effectiveType === "number";
	...
	const deleteButton = (
		<Button
			...
			className={cn(
				"h-5 p-1",
				isVariableInUse && "cursor-not-allowed opacity-50",
			)}
					...
					<div className="flex w-full items-center justify-between gap-1">
						...
						{isVariableInUse ? (
							<Popover
								...
								onOpenChange={setShowUsagePopover}
								modal={false}
							>
								...
								>
									<div className="space-y-2">
										...
										<p className="text-xs">
											This variable can't be deleted as it's being used in the
											step content of this workflow.
										</p>
										{variableUsageInfo &&
											variableUsageInfo.usedInSteps.length > 0 && (
							...
							<FormItem>
								...
								<Controller
									name={
										(isArrayType
											? minItemsPath
											: minLengthPath) as Path<SchemaEditorFormValues>
									}
									control={control}
									render={({ field }) => (
										<FormControl>
							...
							<FormItem>
								...
								<Controller
									name={
										(isArrayType
											? maxItemsPath
											: maxLengthPath) as Path<SchemaEditorFormValues>
									}
									control={control}
									render={({ field }) => (
										<FormControl>
									...
									render={({ field }) => (
										<FormControl>
											<Select
												...
												onValueChange={(value) =>
													field.onChange(
														value === NONE_FORMAT_VALUE ? undefined : value,
													)
												}
											>
