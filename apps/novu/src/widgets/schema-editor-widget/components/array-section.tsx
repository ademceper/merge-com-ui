...
const ArrayItemProperty = memo<ArrayItemPropertyProps>(
	function ArrayItemProperty({
		...
		const itemVariableUsageInfo = useMemo(() => {
			...
			return onCheckVariableUsage && itemKeyName
				? onCheckVariableUsage(itemKeyName, arrayItemPath)
				: undefined;
		...
		return (
			<SchemaPropertyRow
				...
				pathPrefix={
					`${itemPropertiesListPath}.${itemNestedIndex}` as Path<SchemaEditorFormValues>
				}
...
export const ArraySection = memo<ArraySectionProps>(function ArraySection({
	...
	const itemSchemaObject = useWatch({ control, name: itemSchemaObjectPath }) as
		| JSONSchema7
		| undefined;
	...
	const { fields, append, remove } = useFieldArray({
		...
		name: itemIsObject
			? itemPropertiesListPath
			: `_unused_array_item_object_path_`,
	...
	return (
		<div className={cn("p-1", getMarginClassPx(indentationLevel + 1))}>
			...
			{itemIsObject && (
				<div
					className={cn("flex flex-col gap-1.5 pt-1.5", getMarginClassPx(1))}
				>
					...
					{isAtMaxDepth && (
						<div className="mt-1 rounded border border-red-200 bg-red-50 p-2 text-xs text-red-600">
							Maximum nesting depth of {MAX_NESTING_DEPTH} levels reached.
							Cannot add more item properties.
						</div>
					)}
					<div>
						<Button
