...
export function useSchemaForm({
	...
	onChange,
	onValidityChange,
}: UseSchemaFormProps): UseSchemaFormReturn {
	...
	const methods = useForm<SchemaEditorFormValues>({
		...
		resolver: standardSchemaResolver(SchemaEditorFormValuesSchema),
		mode: "onBlur",
	...
	const addProperty = useCallback(
		(
			propertyDataFromArg?: Partial<PropertyListItem>,
			typeFromArg?: JSONSchema7TypeName,
		) => {
...
// Helper functions
function createSchemaFromPropertyList(
...
function addRootLevelProperty(
	...
	const newProperty = createPropertyItem(
...
function addNestedProperty(
	...
	const currentRootPropertyList: PropertyListItem[] = JSON.parse(
		JSON.stringify(getValues().propertyList || []),
	);

	try {
		const targetList = findOrCreatePropertyPath(
			currentRootPropertyList,
			pathInfo.parentPath,
		);

		if (propertyExists(targetList, pathInfo.keyName)) {
			console.warn(
				`Property "${pathInfo.keyName}" already exists in "${pathInfo.parentPath.join(".")}".`,
			);
			return;
		}

		const newProperty = createPropertyItem(
			{ ...propertyData, keyName: pathInfo.keyName },
			defaultType,
		);
		...
		console.error(
			`Failed to add nested property: ${error instanceof Error ? error.message : String(error)}`,
		);
	}
}
