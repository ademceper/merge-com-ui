...
export const PublishButton = () => {
	...
	const otherEnvironments = environments.filter(
		(env) => env?._id && env._id !== currentEnvironment?._id,
	);
	...
	useEffect(() => {
		...
		window.addEventListener(
			"open-publish-modal",
			handleOpenPublishModal as EventListener,
		);
	...
	return (
		<>
			<DropdownMenu open={isDropdownOpen} onOpenChange={setIsDropdownOpen}>
				...
				<DropdownMenuContent
					align="end"
					className="max-h-[400px] min-w-[280px] overflow-y-auto"
				>
					{otherEnvironments.length === 0 ? (
						...
						otherEnvironments.map((environment) => (
							<EnvironmentOption
								...
								currentEnvironmentId={currentEnvironment?._id}
								onSelect={(hasChanges) =>
									handleEnvironmentSelect(environment, hasChanges)
								}
...
const calculateChangesCount = (
	diffData: IEnvironmentDiffResponse | undefined | null,
): number => {
...
const useInvalidateDiffOnWorkflowChange = (enabled: boolean = true) => {
	...
	useEffect(() => {
		...
		const unsubscribe = queryClient.getQueryCache().subscribe((event) => {
			if (
				event.type === "updated" &&
				event.query.queryKey.includes(QueryKeys.fetchWorkflows)
			) {
				queryClient.invalidateQueries({ queryKey: ["diff-environments"] });
			}
		});
		...
		<AnimatePresence mode="wait">
			...
			>
				<Badge
					...
					color="purple"
					size="sm"
					className="text-subheading-2xs h-4 min-w-4 p-0"
				>
					{safeCount}
				</Badge>
			</motion.div>
			...
			<div className="flex w-full items-center justify-between">
				<div className="flex items-center gap-2">
					...
					<span className="text-text-sub font-medium">
						Publish to{" "}
						<TruncatedText
							className="text-text-strong max-w-[20ch] font-bold"
							asChild
						>
