...
import {
	...
	FooterLink,
	SuggestionCard,
} from "./support-drawer-components";
import {
	BOOK_DEMO_URL,
	CHANGELOG_URL,
...
function SupportDrawerContent({
	...
	function handleTrackSuggestion(title: string) {
		telemetry(TelemetryEvent.SUPPORT_DRAWER_SUGGESTION_CLICKED, {
	...
	return (
		<div className="flex h-full flex-col">
			...
			<div className="flex items-center justify-between px-3 py-3.5">
				<span className="text-foreground-600 text-sm font-medium leading-5 tracking-[-0.084px]">
					Need a hand?
				</span>
			</div>

			<div className="px-3 pb-2">
				{hasInkeep ? <InkeepEmbeddedSearch {...inkeepConfig} /> : null}
			</div>
...
export function SupportDrawer({ children }: SupportDrawerProps) {
	...
	const drawerWidth = isViewingDocs
		? DRAWER_WIDTH_EXPANDED
		: DRAWER_WIDTH_DEFAULT;
