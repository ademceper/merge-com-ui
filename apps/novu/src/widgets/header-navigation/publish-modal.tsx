...
export function PublishModal({
	...
	const { workflows, layouts, dependencyMap, calculateDependencyState } =
		useResourceDependencies(diffData);
	...
	useEffect(() => {
		...
		diffData.resources.forEach((resource) => {
			const resourceId =
				resource.sourceResource?.id || resource.targetResource?.id;
		...
		const selectionWithDependencies =
			calculateDependencyState(initialSelection);
	...
	const getTotalSelectedCount = () => {
		return Object.values(resourceSelection).filter((state) => state.selected)
			.length;
	};

	const handleConfirm = () => {
		const selectedResources: ResourceToPublish[] = Object.entries(
			resourceSelection,
		)
			.filter(([_, state]) => state.selected)
			.map(([id, state]) => ({
				resourceType: state.resource
					.resourceType as ResourceToPublish["resourceType"],
				resourceId: id,
			}));
				...
				<div className="w-full max-w-[486px] space-y-1.5">
					{workflows.length > 0 && (
						...
						>
							{workflows.map((workflow) => {
								const id =
									workflow.sourceResource?.id || workflow.targetResource?.id;
								if (!id) return null;
					...
					{layouts.length > 0 && (
						...
						>
							{layouts.map((layout) => {
								const id =
									layout.sourceResource?.id || layout.targetResource?.id;
								if (!id) return null;
				...
				<CollapsibleContent>
					{count > 0 && (
						<div className="rounded-md border border-gray-200 bg-white">
							<div className="max-h-64 overflow-y-auto divide-y divide-gray-100 overflow-x-hidden">
								{children}
							</div>
						</div>
					)}
		...
		<div className="flex items-center gap-1.5 p-1 min-w-0">
			{disabled ? (
				<Tooltip>
					...
					>
						This resource is required by another selected resource and they must
			...
			<div className="min-w-0 flex-1">
				{resource.resourceType === "layout" ? (
					...
					<div className="leading-0 flex w-full min-w-0 items-center gap-1 text-left">
						<span className="min-w-0 shrink truncate text-xs font-medium leading-4 text-gray-900">
							{displayName}
						</span>
						{hasDependencies && (
							<Tooltip>
					...
					<>
						...
						<div className="truncate font-mono text-xs tracking-tight text-gray-400">
							{slug}
						</div>
					</>
				)}
			...
			<div className="flex flex-col items-end gap-1.5">
				...
				{updatedAt && (
					<span className="text-label-2xs text-text-sub">
...
function ResourceStatusBadge({ resource }: { resource: IResourceDiffResult }) {
	...
	if (summary.modified > 0) {
		...
		if (resource.resourceType === "workflow") {
			return (
				<WorkflowHoverCard workflowResource={resource}>
					{badge}
				</WorkflowHoverCard>
			);
		}
	...
	return (
		<div className="flex items-center justify-end gap-3">
			...
			>
				...
				<span className="text-[#E1E4EA]">({totalSelected})</span>
			</Button>
		</div>
	);
}
