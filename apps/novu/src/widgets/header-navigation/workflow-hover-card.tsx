...
export function WorkflowHoverCard({
	workflowResource,
	children,
	...
	const changeTypes = useMemo(() => {
		...
		return types;
	}, [workflowResource.changes, workflowResource]);
			...
			>
				<div className="flex flex-col gap-1">
					...
					<Badge
						variant="lighter"
						size="sm"
						color={getChangeColor(overallStatus.action)}
					>
					...
					<div className="flex flex-col gap-1.5">
						{changeTypes.map((changeType, index) => {
							...
							return (
								<div
									key={index}
									className="flex min-w-[175px] items-center gap-1.5 rounded p-1"
								>
