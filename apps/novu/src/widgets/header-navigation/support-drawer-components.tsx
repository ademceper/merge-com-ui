...
import { type SuggestionItem, toEmbedUrl } from "./support-drawer-constants";
	...
	return (
		...
		>
			...
			<div className="flex min-w-0 flex-1 flex-col">
				...
				<span className="text-foreground-400 text-xs leading-4">
					{item.description}
				</span>
			</div>
		</button>
...
export function DocsIframeView({
	url,
	onBack,
	onTrackBack,
	onTrackExternal,
	...
	useEffect(() => {
		const ensurePrefetch = () => {
			const existingPrefetch = document.querySelector(
				'link[rel="dns-prefetch"][href="https://docs.novu.co"]',
			);
			const existingPreconnect = document.querySelector(
				'link[rel="preconnect"][href="https://docs.novu.co"]',
			);
...
export function FooterLink({ icon: Icon, children, onClick }: FooterLinkProps) {
	return (
		...
		>
			...
			<span className="text-foreground-950 text-sm font-medium leading-5 tracking-[-0.28px]">
				{children}
			</span>
		</button>
	);
