...
const CONSTANTS = {
	SANITY_API_URL:
		"https://w2rl2099.api.sanity.io/v2025-02-19/data/query/production",
...
export function ChangelogStack() {
	...
	const updateDismissedChangelogs = async (changelogId: string) => {
		...
		const updatedDismissed = [...currentDismissed, changelogId].slice(
			-CONSTANTS.MAX_DISMISSED_IDS,
		);
	...
	const transformSanityData = (
		sanityPosts: SanityChangelogPost[],
	): Changelog[] => {
		const now = new Date();
	...
	const handleChangelogClick = async (changelog: Changelog) => {
		...
		window.open(
			`https://novu.co/changelog/${changelog.slug}`,
			"_blank",
			"noopener,noreferrer",
		);
