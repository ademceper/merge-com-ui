...
>(({ className, ...rest }, forwardedRef) => {
	return (
		<Command.List
			ref={forwardedRef}
			className={cn(
				"max-h-[400px] min-h-0 flex-1 overflow-auto",
				"py-1",
				className,
			)}
			{...rest}
...
const CommandItemIcon = React.forwardRef<
	HTMLDivElement,
...
>(({ className, ...rest }, forwardedRef) => {
	return (
		<div
			ref={forwardedRef}
			className={cn("size-5 shrink-0 text-foreground-600", className)}
			{...rest}
		/>
	);
});
...
>(({ className, ...rest }, forwardedRef) => {
	return (
		<Command.Empty
			ref={forwardedRef}
			className={cn(
				"flex items-center justify-center py-6 text-sm text-foreground-400",
				className,
			)}
			{...rest}
		/>
...
>(({ className, ...rest }, forwardedRef) => {
	return (
		<div
			ref={forwardedRef}
			className={cn(
				"flex h-12 items-center justify-between gap-3 px-3 border-t border-neutral-100",
				className,
			)}
			{...rest}
		/>
...
const CommandFooterKeyBox = React.forwardRef<
	HTMLDivElement,
	React.HTMLAttributes<HTMLDivElement>
>(({ className, ...rest }, forwardedRef) => {
	return (
