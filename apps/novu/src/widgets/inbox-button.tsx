...
import { useWorkflowEditorPage } from "@/features/workflows/hooks/use-workflow-editor-page";
		...
		<Popover onOpenChange={setOpen}>
			...
			<PopoverPortal>
				<PopoverContent
					side="bottom"
					align="end"
					className="h-[550px] w-[350px] overflow-hidden p-0"
				>
					<InboxContent />
				</PopoverContent>
			</PopoverPortal>
...
export const InboxButton = () => {
	...
	const isNovuProductionDashboard =
		window.location.hostname.includes("dashboard.novu.co");
	const isNovuStagingEnvironment =
		apiHostnameManager.getHostname() === "https://api.novu-staging.co";
	const shouldUseProductionApi =
		(isNovuProductionDashboard || isNovuStagingEnvironment) && !isTestPage;

	const subscriber = useMemo(
		() => ({
			subscriberId: isTestPage
				? (user?.externalId ?? "")
				: `org_${currentOrganization?.id}:user_${user?.externalId}`,
