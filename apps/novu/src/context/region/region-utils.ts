import {
	DEFAULT_REGION,
	getRegionCodeFromAws,
	getRegionConfig,
	REGIONS,
} from "./region-config";
import type { OrganizationMetadata, Region } from "./region-types";
...
export function detectRegionFromOrganization(
...
export function findOrganizationForRegion(
	region: Region,
	userMemberships: { data?: OrganizationMembershipLike[] },
) {
	...
	const found = userMemberships.data?.find((membership) => {
		const orgMetadata = membership.organization
			.publicMetadata as OrganizationMetadata;
		const awsRegion = orgMetadata?.region;
