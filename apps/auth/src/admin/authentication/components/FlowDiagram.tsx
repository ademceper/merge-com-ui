 ...
 *
 * $ npx keycloakify own --path "admin/authentication/components/FlowDiagram.tsx"
 *
...
type NodeType = "conditional" | "startSubFlow" | "endSubFlow" | "input" | "output";
...
const isBypassable = (execution: ExpandableExecution) =>
    execution.requirement === "ALTERNATIVE" || execution.requirement === "CONDITIONAL";

const createEdge = (fromNode: string, toNode: string, label?: ConditionLabel): Edge => ({
...
const borderStep = (node: Node, continuing: boolean = true): IntermediateFlowResult => ({
    startId: node.id,
    nodes: [node],
    edges: [],
...
const renderSubFlow = (execution: ExpandableExecution): IntermediateFlowResult => {
...
const createConcurrentGroupings = (
    ...
    return [steps, ...createConcurrentGroupings(executionList.slice(steps.length))];
};

const createGraph = (groupings: IntermediateFlowResult[][]): IntermediateFlowResult => {
    const nodes: Node[] = [];
    const edges: Edge[] = [];
    ...
    for (const group of groupings) {
        ...
        edges.push(
            ...
            ...pairwise(
                (prev, current) => createEdge(prev.startId, current.startId, "attempted"),
                group
            )
        );
...
function renderGraph(executionList: ExpandableExecution[]): [Node[], Edge[]] {
    ...
    const groupings = [
        ...
        [borderStep(createNode({ id: "end", displayName: "End" }, "output"), false)]
    ];
...
export const FlowDiagram = ({ executionList: { expandableList } }: FlowDiagramProps) => {
    const [expandDrawer, setExpandDrawer] = useState(false);
    const [initialNodes, initialEdges] = useMemo(() => renderGraph(expandableList), []);
