 ...
 *
 * $ npx keycloakify own --path "admin/user/form-state.ts"
 *
...
export function toUserRepresentation(data: UserFormFields): UIUserRepresentation {
    const username = data.username?.trim();
    const attributes = Array.isArray(data.attributes)
        ? keyValueToArray(data.attributes)
        : Object.fromEntries(
              Object.entries(data.attributes || {}).map(([k, v]) => [debeerify(k), v])
          );
