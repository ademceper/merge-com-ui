...
function AppleIcon({ className }: { className?: string }) {
    return (
        <svg className={cn("size-5 shrink-0", className)} viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                ...
                {providers.map((p) => {
                    ...
                    return (
                        <a
                            key={p.alias}
                            href={p.loginUrl}
                            className="flex items-center justify-center gap-2 w-full h-12 rounded-lg bg-muted hover:bg-muted/80 text-foreground font-medium text-sm transition-colors border-0"
                        >
                            {icon ? (
