...
export const labelAttribute = (t: TFunction, attribute: UserProfileAttributeMetadata) =>
    label(t, attribute.displayName, attribute.name);

const ROOT_ATTRIBUTES = ["username", "firstName", "lastName", "email"];

export const isRootAttribute = (attr?: string) => attr && ROOT_ATTRIBUTES.includes(attr);
...
export const beerify = <T extends string>(name: T) => name.replaceAll(".", "🍺");

export const debeerify = <T extends string>(name: T) => name.replaceAll("🍺", ".");
...
export function isRequiredAttribute({ required }: UserProfileAttributeMetadata): boolean {
